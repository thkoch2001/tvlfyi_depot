...
let
  ...
  check-crate-advisory =
    depot.nix.writeExecline "check-crate-advisory" { readNArgs = 3; } [
      "pipeline"
  ...
  test-parsing-all-security-advisories =
    depot.nix.runExecline "check-all-our-crates" { } [
      "pipeline"
  ...
  check-all-our-crates = depot.nix.runExecline "check-all-our-crates" {
    stdin = lib.concatStrings (map (crate:
      depot.nix.netstring.fromString
      (depot.nix.netstring.fromString crate.crateName
        + depot.nix.netstring.fromString crate.version)) our-crates);
  } [
    "if"
    [
      "forstdin"
      ...
      [
        depot.tools.eprintf
        ''
          No advisories found for crate %s
        ''
        "$crate"
      ]
      "importas"
...
in depot.nix.readTree.drvTargets {
  ...
  check-all-our-crates =
    depot.nix.drvSeqL [ test-parsing-all-security-advisories ]
    check-all-our-crates;
