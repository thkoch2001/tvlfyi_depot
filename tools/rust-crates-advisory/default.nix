...
let
  ...
  crate-advisories = "${pkgs.fetchFromGitHub {
    owner = "RustSec";
    repo = "advisory-db";
    # TODO: this will have to be updated regularly, how?
  ...
  check-security-advisory = depot.users.Profpatsch.writers.rustSimple {
  ...
  check-crate-advisory = depot.nix.writeExecline "check-crate-advisory" { readNArgs = 3; } [
    ...
    "if" [ depot.users.Profpatsch.lib.eprintf "advisory %s\n" "$advisory" ]
  ...
  check-all-our-crates = depot.nix.runExecline "check-all-our-crates" {
    # todo: move toNetstring into the nix/ subdir
    stdin = lib.concatStrings
      (map
        (crate:
          depot.users.Profpatsch.netstring.toNetstring
            ( depot.users.Profpatsch.netstring.toNetstring crate.crateName
            + depot.users.Profpatsch.netstring.toNetstring crate.version ))
        our-crates);
  } [
    "if" [
      "forstdin" "-o" "0" "-Ed" "" "crateNetstring"
      "multidefine" "-d" "" "$crateNetstring" [ "crate" "crate_version" ]
      "if" [ depot.users.Profpatsch.lib.eprintf "checking %s, version %s\n" "$crate" "$crate_version" ]

      "ifthenelse" [ bins.s6-test "-d" "${crate-advisories}/\${crate}" ]
          [ # also print the full advisory text if it matches
            "export" "PRINT_ADVISORY" "1"
            check-crate-advisory "${crate-advisories}/\${crate}" "$crate" "0.1.0"
          ]
        [ depot.users.Profpatsch.lib.eprintf "No advisories found for crate %s\n" "$crate" ]
        "importas" "-ui" "ret" "?"
        # put a marker in ./failed to read at the end
        "ifelse" [ bins.s6-test "$ret" "-eq" "1" ]
          [ bins.s6-touch "./failed" ]
        "if" [ depot.users.Profpatsch.lib.eprintf "\n" ]
        "exit" "$ret"
    ]
    "ifelse" [ bins.s6-test "-f" "./failed" ]
      [ "if" [ depot.users.Profpatsch.lib.eprintf "Error: Found active advisories!" ]
        "exit" "1"
      ]
    "importas" "out" "out"
    bins.s6-touch "$out"
