...
fn interpret(code: &str) {
    ...
    configure_nix_path(
        &mut eval,
        // The benchmark requires NIX_PATH to be set, so barf out early,
        // rather than benchmarking tvix returning an error
        &Some(env::var("NIX_PATH").expect("NIX_PATH must be set")),
    );
