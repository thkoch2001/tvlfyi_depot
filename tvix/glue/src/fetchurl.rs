...
pub(crate) fn fetchurl_derivation_to_fetch(drv: &Derivation) -> Result<(String, Fetch), Error> {
    ...
    // TODO: check if arguments really looks like this too?
    if drv.arguments != ["builtin:fetchurl"] {
    ...
    // TODO: check how empty names end up here.
    let name: String = drv
        .environment
        .get("name")
    ...
    match ca_hash {
        ...
        CAHash::Nar(hash) => {
            if drv.environment.get("executable").map(|v| v.as_slice()) == Some(b"true") {
