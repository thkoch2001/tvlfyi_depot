...
impl<P> ReferencePattern<P> {
    #[must_use] pub fn candidates(&self) -> &[P] {
        &self.inner.candidates
    }

    #[must_use] pub fn longest_candidate(&self) -> usize {
...
impl<P: AsRef<[u8]>> ReferencePattern<P> {
    ...
    /// candidates.
    #[must_use] pub fn new(candidates: Vec<P>) -> Self {
        ...
        Self {
            inner: Arc::new(ReferencePatternInner { candidates, longest_candidate, searcher }),
        }
    }
}
...
impl<P: AsRef<[u8]>> ReferenceScanner<P> {
    ...
    #[must_use] pub fn pattern(&self) -> &ReferencePattern<P> {
        &self.pattern
    }

    #[must_use] pub fn matches(&self) -> &[bool] {
        &self.matches
    }
...
impl<P: Clone + Ord + AsRef<[u8]>> ReferenceScanner<P> {
    ...
    #[must_use] pub fn finalise(self) -> BTreeSet<P> {
