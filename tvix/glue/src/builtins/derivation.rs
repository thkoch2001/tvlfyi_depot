...
pub(crate) mod derivation_builtins {
    ...
    async fn builtin_derivation_strict(
        ...
        let mut structured_attrs: Option<BTreeMap<String, serde_json::Value>> = input
        ...
        for (arg_name, arg_value) in input.clone().into_iter_sorted() {
            ...
            // TODO(raitobezarius): input_context.mimic for some of these values.
            match arg_name.as_str() {
                ...
                _ => {
                    ...
                    if let Some(ref mut structured_attrs) = structured_attrs {
                        ...
                        if arg_name.as_str() == STRUCTURED_ATTRS {
                            continue;
                        }
