...
> {
    ...
    while let Some(entry) = it.next() {
        ...
        let should_skip: bool = if let Some(filter) = filter {
    ...
    let blob_service = state.importer.blob_service.clone();
    let directory_service = state.importer.directory_service.clone();
    let root_node = state.tokio_runtime.block_on(async move {
        tvix_castore::import::ingest_entries(
            blob_service,
            &directory_service.as_ref(),
            entries_per_depths,
        )
        .await
