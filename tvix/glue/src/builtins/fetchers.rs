...
use tvix_eval::{CatchableErrorKind, ErrorKind, EvalIO, NixContextElement, NixString, Value};
...
impl FetchArgs {
    pub fn new(
        ...
        Ok(Self {
            ...
            url,
            hash: sha256
                .map(|h| nixhash::from_str(&h, Some("sha256")))
                .transpose()?,
            mode,
        })
    }

    fn store_path(&self) -> Result<StorePathRef, ErrorKind> {
        let Some(hash) = &self.hash else { todo!() };
        build_ca_path(
            &self.name,
            &match self.mode {
                HashMode::Flat => nixhash::CAHash::Flat(hash.clone()),
                HashMode::Recursive => nixhash::CAHash::Nar(hash.clone()),
            },
            Vec::<String>::new(),
            false,
        )
        .map_err(|e| {
            DerivationError::InvalidDerivation(
                nix_compat::derivation::DerivationError::InvalidOutputDerivationPath(
                    "out".into(),
                    e,
                ),
            )
...
async fn fetch(
    ...
    if state.store_path_exists(store_path).await? {
        return Ok(Value::from(NixString::new_context_from(
            NixContextElement::Plain(store_path.to_absolute_path()).into(),
            store_path.to_absolute_path(),
        )));
    }

    let resp = state.http().get(args.url).send().expect("TODO error types");
    let data = resp.bytes().expect("TODO error types");
    state.as_ref().import_path();
...
mod tests {
    ...
    fn fetchurl_store_path() {
        ...
        assert_eq!(
            args.store_path().unwrap().to_owned(),
