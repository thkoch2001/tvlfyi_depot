...
impl FetchArgs {
    ...
    fn ca_hash(&self) -> Option<nixhash::CAHash> {
        match self.mode {
            HashMode::Flat => Some(nixhash::CAHash::Flat(self.hash.clone()?)),
            HashMode::Recursive => Some(nixhash::CAHash::Nar(self.hash.clone()?)),
        }
    }
}
...
async fn fetch(
    ...
    let store_path = store_path.map(|sp| sp.to_owned());
    let ca_hash = args.ca_hash();
    let name = args.name;
    ...
    let store_path = Rc::clone(&state).tokio_handle.block_on(async move {
        let resp = state
            ...
            .send()
            .await
            .expect("TODO error types");
        let data = tokio_util::io::StreamReader::new(resp.bytes_stream().map_err(|e| {
            let e = e.without_url();
            warn!(%e, "failed to get response body");
            io::Error::new(io::ErrorKind::BrokenPipe, e.to_string())
        }));
        let sp = state
            .import_bytes_to_file(&name, store_path, data, ca_hash)
            .await?;
