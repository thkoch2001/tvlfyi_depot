        ...
        match self {
            Fetch::URL(url, nixhash) => {
                ...
                if let Some(nixhash) = nixhash {
                    ...
                    write!(f, "URL [url: {}, exp_hash: None]", redact_url(&url))
                }
            }
...
impl<BS, DS, PS> Fetcher<BS, DS, PS> {
    ...
    async fn download(&self, url: Url) -> Result<Box<dyn AsyncBufRead + Unpin>, FetcherError> {
        match url.scheme() {
            "file" => {
                let f = tokio::fs::File::open(url.path()).await?;
...
{
    /// Do a fetch.
    /// On success, return the root node, and a content digest and length.
    ...
    pub async fn fetch(&self, fetch: Fetch) -> Result<(Node, CAHash, u64), FetcherError> {
        use md5::Md5;
        use sha1::Sha1;
        ...
        match fetch {
            Fetch::URL(url, exp_nixhash) => {
                ...
                let (blob_size, actual_nixhash) = match exp_nixhash {
                    None => {
                        let mut hash = Sha256::new();
                        let blob_size = tokio::io::copy(
                            &mut InspectReader::new(&mut r, |d| hash.write_all(d).unwrap()),
                            &mut blob_writer,
                        )
                        .await?;

                        (blob_size, NixHash::Sha256(hash.finalize().into()))
                    }
                    Some(NixHash::Md5(_)) => {
                        ...
                        (blob_size, NixHash::Md5(hash.finalize().into()))
                    }
                    Some(NixHash::Sha1(_)) => {
                        ...
                        (blob_size, NixHash::Sha1(hash.finalize().into()))
                    }
                    Some(NixHash::Sha256(_)) => {
                        let mut hash = Sha256::new();
                        let blob_size = tokio::io::copy(
                            &mut InspectReader::new(&mut r, |d| hash.write_all(d).unwrap()),
                            &mut blob_writer,
                    ...
                    Some(NixHash::Sha512(_)) => {
                        ...
                        (blob_size, NixHash::Sha512(Box::new(hash.finalize().into())))
                    }
                };
    ...
    /// Does a fetch, persists the returned node in the PathInfoService, and
    /// returns the calculated StorePath, as well as the root node pointing to the contents.
    /// The root node can be used to descend into the data without doing the
    /// lookup to the PathInfoService again.
    pub async fn fetch_and_persist<'a>(
        &self,
        name: &'a str,
        fetch: Fetch,
    ) -> Result<(StorePathRef<'a>, Node), FetcherError> {
        // Fetch file, return the (unnamed) (File)Node of its contents, ca hash and filesize.
        let (mut node, ca_hash, size) = self.fetch(fetch).await?;
        ...
        // calculate_nar to calculate this representation, as it ends up in the PathInfo.
