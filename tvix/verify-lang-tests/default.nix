...
depot.nix.readTree.drvTargets {
  ...
  "nix-${lib.versions.majorMinor nix_last_verified.version}" =
    let
      nix_last_verified_version = lib.versions.majorMinor nix_last_verified.version;
    in
    lib.warnIf (nix_last_verified_version != (lib.versions.majorMinor pkgs.nixVersions.latest.version))
      "The last verified Nix version is out of date, consider updating the value of `nix_last_verified` to `pkgs.nixVersions.nix_${lib.replaceStrings ["."] ["_"] nix_last_verified_version} and verify that the tests still pass."
      runCppNixLangTests
      nix_last_verified;
}
