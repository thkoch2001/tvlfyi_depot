...
fn is_clean_path<P: AsRef<Path>>(p: P) -> bool {
    ...
    // manually assemble a new PathBuf, by iterating over components (which does
    // do some cleaning).
    let mut cleaned_p = PathBuf::new();
    for component in p.components() {
        if component.as_os_str().to_str() == Some("..") {
            return false;
        }
        cleaned_p.push(component);
    }

    // if the new path looks like the old one, we're good.
    if cleaned_p.into_os_string() != p.as_os_str() {
        return false;
    }
...
// fn is_sorted<B: AsRef<[u8]>>(lst: impl Iterator<Item = B>) -> bool {
fn is_sorted<'a>(lst: impl Iterator<Item = &'a [u8]>) -> bool {
    let mut prev_elem: Option<&[u8]> = None;
    for elem in lst {
        // let elem = elem.as_ref();
        if let Some(pn) = prev_elem {
            // check for being strictly greater
            if pn >= elem {
                return false;
            }
        }
...
mod tests {
    ...
    #[test_case("foo/./bar", false; "fail dot")]
