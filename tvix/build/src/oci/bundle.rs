...
fn find_path_in_scratchs<'a, 'b>(
    ...
    for mp in mountpoints.iter().rev() {
        match search_path.strip_prefix(mp) {
            Some(rest) => {
...
mod tests {
    ...
    #[test_case("nix/store/aaaa", &["nix/store".into()], Some(("nix/store", "aaaa")); "simple")]
    #[test_case("nix/store/aaaa", &["nix/sto".into()], None; "prefix but no separator")]
    #[test_case("nix/store/aaaa", &["build".into()], None; "not found")]
    fn test_find_path_in_scratchs(
        search_path: &str,
        mountpoints: &[String],
