...
use tvix_castore::proto::{self as castorepb, NamedNode};
    ...
    oci_spec::runtime::SpecBuilder::default()
        ...
        .mounts(configure_mounts(
            ...
            request
                .inputs
                .iter()
                .map(|e| e.node.as_ref().expect("node must be some")),
...
fn configure_process<'a>(
    ...
    let spec_builder = oci_spec::runtime::ProcessBuilder::default()
        ...
        .cwd(format!("/{}", cwd)) // relative to the bundle root, but at least runc wants it to also be absolute.
        .capabilities({
...
fn configure_mounts<'a>(
    ...
    inputs: impl Iterator<Item = &'a castorepb::node::Node>,
    inputs_dir: &str,
    ...
    for input in inputs {
        let input_name = std::str::from_utf8(input.get_name()).expect("invalid input name");

        mounts.push(configure_mount(
            Path::new("inputs").join(input_name).to_str().unwrap(),
            Path::new(inputs_dir).join(input_name).to_str().unwrap(),
            "none",
            &["bind", "ro"],
        )?);
    }
