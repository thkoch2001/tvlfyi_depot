...
use tvix_castore::{
    ...
    fs::fuse::FuseDaemon,
    import::ingest_path,
    proto::{self as castorepb, node::Node, NamedNode},
};
use uuid::Uuid;
...
impl<BS, DS> OCIBuildService<BS, DS> {
    pub fn new(bundle_root: PathBuf, blob_service: BS, directory_service: DS) -> Self {
        ...
        Self {
            ...
            concurrent_builds: tokio::sync::Semaphore::new(2), // TODO: make configurable
        }
    }
}
    ...
    DS: AsRef<dyn DirectoryService> + Send + Sync + Clone + 'static,
{
    ...
    async fn do_build(&self, request: BuildRequest) -> std::io::Result<Build> {
        let _ = self.concurrent_builds.acquire().await.unwrap();
        ...
        let mut fuse_daemon = tokio::task::spawn_blocking({
            ...
            move || {
                ...
                FuseDaemon::new(fs, dest, 4).context("failed to start fuse daemon")
            }
        })
        .await?
        .context("mounting")
        ...
        // impl Drop for FuseDaemon unmounts, so if the call is cancelled, we also unmount.
        ...
        if !child_output.status.success() {
            return Err(std::io::Error::new(
                ...
                format!("nonzero exit code: {:?}", child_output.status),
            ));
        }

        // Explicitly unmount after the build command returned.
        fuse_daemon
            .unmount()
            .context("unable to unmount filesystem")
            .map_err(std::io::Error::other)?;
        ...
        let outputs = futures::future::try_join_all(host_output_paths.into_iter().enumerate().map(
            |(i, p)| {
                ...
                async move {
                    ...
                    Ok::<_, std::io::Error>(castorepb::Node {
...
fn spawn_bundle(
    ...
    command
        ...
        // .stderr(Stdio::piped())
        // .stdout(Stdio::piped())
        .stdin(Stdio::inherit()); // TODO: update this to null
