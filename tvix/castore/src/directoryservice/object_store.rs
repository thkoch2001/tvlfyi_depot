...
impl ObjectStoreDirectoryService {
    ...
    pub fn from_config(config: ObjectStoreDirectoryServiceConfig) -> Result<ObjectStoreDirectoryService, Box<dyn std::error::Error>> {
        let (object_store, path) = object_store::parse_url_opts(&config.object_store_url.parse()?, config.object_store_options)?;
        Ok(Self {
            object_store: Arc::new(object_store),
            base_path: path,
        })
