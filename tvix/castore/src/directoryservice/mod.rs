...
/// While a Node by itself may have any name, the names of entries in a Directory must:
...
impl Directory {
    ...
    ///
    /// If the sum overflows u64:
    /// In debug mode, this will panic
    /// In release mode, this will return u64::MAX
    pub fn size(&self) -> u64 {
    ...
    /// the number of directory elements, and their size fields.
    ///
    /// This will return None if the sum overflows u64.
    pub fn size_checked(&self) -> Option<u64> {
        checked_sum([
            self.nodes.len().try_into().ok()?,
            checked_sum(self.directories().map(|e| e.size))?,
        ])
    }
