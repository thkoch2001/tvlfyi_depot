...
use super::{
    utils::{traverse_directory, SimplePutter},
    DirectoryPutter, DirectoryService,
};
...
impl DirectoryService for BigtableDirectoryService {
    ...
    async fn get(&self, digest: &B3Digest) -> Result<Option<proto::Directory>, Error> {
        ...
        assert!(row_key == directory_key.as_bytes(), "row key must match");

        if let Some(row_cell) = row_cells.pop() {
            // Ensure there's only one cell. We filter out others above.
            assert!(row_cells.is_empty(), "only one cell allowed");
            // We also require the qualifier to be correct in the filter above.
            assert_eq!(
                directory_key.as_bytes(),
                &row_cell.qualifier,
