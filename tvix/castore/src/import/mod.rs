    ...
    let root_node = loop {
        ...
        let node = match &mut entry {
            IngestionEntry::Dir { .. } => {
                ...
                Node::Directory(
                    DirectoryNode::new(directory_digest, directory_size).map_err(|e| {
...
mod test {
    ...
    #[case::single_dir(vec![IngestionEntry::Dir {
        ...
        Node::Directory(DirectoryNode::new(Directory::default().digest(), Directory::default().size()).unwrap())
    )]
    #[case::dir_with_keep(vec![
        ...
        Node::Directory(DirectoryNode::new(DIRECTORY_WITH_KEEP.digest(), DIRECTORY_WITH_KEEP.size()).unwrap())
    )]
    ...
    Node::Directory(DirectoryNode::new(DIRECTORY_COMPLICATED.digest(), DIRECTORY_COMPLICATED.size()).unwrap())
    )]
    #[tokio::test]
