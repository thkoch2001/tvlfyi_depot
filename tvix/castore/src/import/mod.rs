...
mod test {
    ...
    #[case::single_dir(vec![IngestionEntry::Dir {
        ...
        Node::Directory(DirectoryNode::new("foo".into(), Directory::default().digest().into(), Directory::default().size()).unwrap())
    )]
    #[case::dir_with_keep(vec![
        ...
        Node::Directory(DirectoryNode::new("foo".into(), DIRECTORY_WITH_KEEP.digest().into(), DIRECTORY_WITH_KEEP.size()).unwrap())
    )]
    ...
    Node::Directory(DirectoryNode::new("blub".into(), DIRECTORY_COMPLICATED.digest().into(), DIRECTORY_COMPLICATED.size()).unwrap())
    )]
    #[tokio::test]
