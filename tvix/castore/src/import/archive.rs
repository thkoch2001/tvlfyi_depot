...
/// The maximum number of small blob uploads that are allowed to run concurrently while
...
mod test {
    ...
    lazy_static! {
        ...
        pub static ref FILE_B: IngestionEntry = IngestionEntry::Regular {
            path: "b".into(),
            size: 0,
            executable: false,
            digest: EMPTY_DIGEST.clone(),
    ...
    #[case::inaccesible_tree(&[&*DIR_A, &*DIR_A_B, &*FILE_A], &[&*FILE_A])]
    ...
    #[rstest]
    #[case::no_top_level_entries(&[], "unexpected number of top level entries")]
    #[case::multiple_top_level_dirs(&[&*DIR_A, &*DIR_B], "unexpected number of top level entries")]
    #[case::multiple_top_level_files(&[&*FILE_A, &*FILE_B], "unexpected number of top level entries")]
    fn node_ingestion_error(#[case] in_entries: &[&IngestionEntry], #[case] exp_error: &str) {
        let mut nodes = IngestionEntryGraph::new();
        ...
        assert_eq!(error.to_string(), exp_error);
    }
}
