    ...
    S: Stream<Item = DirEntry> + std::marker::Unpin,
{
    ...
    let root_node = loop {
        ...
        let node = if file_type.is_dir() {
            ...
            {
                let mut directory_putter = std::mem::take(&mut maybe_directory_putter)
                    .unwrap_or_else(|| directory_service.as_ref().put_multiple_start());

                directory_putter.put(directory).await?;
                maybe_directory_putter = Some(directory_putter);
            }
