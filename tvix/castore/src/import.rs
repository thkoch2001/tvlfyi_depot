    ...
    BS: AsRef<dyn BlobService> + Clone + Send + 'a,
{
    ...
    dir_entry_iter_to_ingestion_stream(blob_service, iter, prefix.into())
}
...
async fn upload_blob_at_path<'a, BS>(blob_service: BS, path: PathBuf) -> Result<B3Digest, Error>
where
...
/// path. Once we receive an [IngestionEntry] for the directory itself, we upload it to the
/// [DirectoryService] through a lazily created [DirectoryPutter].
///
