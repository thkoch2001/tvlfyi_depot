...
async fn process_entry<'a, R: AsyncRead + Unpin, P: AsRef<Path> + Debug>(
    ...
    let name = path.as_ref().file_name().expect("Entry path {path:?} should have a file name");
    match entry {
        ...
        IngestionEntry::File { ref mut reader, executable } => {
            let mut writer = blob_service.open_write().await;

            let size = tokio::io::copy(reader, &mut writer).await.map_err(|e| {
                Error::UnableToRead(path.as_ref().to_path_buf(), e)
            })?;
