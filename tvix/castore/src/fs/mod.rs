    ...
    RN: RootNodes + Clone + 'static,
{
    ...
    fn getxattr(
        ...
        let digest_str = if inode == ROOT_ID {
            warn!("got getxattr call on root node");
            ...
            match *self.inode_tracker.read().get(inode).unwrap() {
                ...
                InodeData::Regular(ref digest, _, _) => {
                    if name.to_bytes() != XATTR_NAME_BLOB_DIGEST {
                        return Err(io::Error::from_raw_os_error(libc::ENODATA));
                    }
                    format!("{digest}")
                }
                InodeData::Symlink(..) => {
                    warn!("got getxattr call on symlink");
                    return Err(io::Error::from_raw_os_error(libc::ENODATA));
                }
            }
        };
    ...
    fn listxattr(
        ...
        let xattrs_names = if inode == ROOT_ID {
            vec![]
        } else {
            let mut out = Vec::new();
            match *self.inode_tracker.read().get(inode).unwrap() {
                InodeData::Directory(_) => out.extend_from_slice(XATTR_NAME_DIRECTORY_DIGEST),
                InodeData::Regular(..) => out.extend_from_slice(XATTR_NAME_BLOB_DIGEST),
                InodeData::Symlink(..) => {}
            }
            out.push_byte(b'\x00');
            out
        ...
        if size == 0 {
            // NOTE: We only have a single key or no keys present, so this is ok.
            Ok(ListxattrReply::Count(if xattrs_names.is_empty() {
                0
            } else {
                xattrs_names.len() as u32
            }))
