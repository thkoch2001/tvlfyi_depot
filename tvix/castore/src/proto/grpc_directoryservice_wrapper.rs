use crate::directoryservice::DirectoryClosureValidator;
use crate::proto;
    ...
    T: Deref<Target = dyn DirectoryService> + Send + Sync + 'static,
{
    ...
    async fn put(
        ...
        // We put all Directory messages we receive into DirectoryClosureValidator first.
        let mut validator = DirectoryClosureValidator::default();
        while let Some(directory) = req_inner.message().await? {
            validator.add(directory)?;
        }

        // drain, which validates connectivity too.
        let directories = validator.drain()?;
