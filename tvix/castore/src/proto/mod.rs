...
impl TryFrom<Directory> for crate::Directory {
    ...
    fn try_from(value: Directory) -> Result<Self, Self::Error> {
        ...
        // [crate::Directory::from_iter] iterating over all three and doing all checks inline.
        value
            .directories
            .iter()
            .try_fold(&b""[..], |prev_name, e| {
                if e.name.as_ref() < prev_name {
                    return Err(DirectoryError::WrongSorting(e.name.to_owned()));
                } else if e.name.as_ref() == prev_name {
                    return Err(DirectoryError::DuplicateName(e.name.to_owned().try_into()?));
                } else {
                    Ok(e.name.as_ref())
                }
            })?;
        ...
        crate::Directory::from_iter(elems)
    }
}
