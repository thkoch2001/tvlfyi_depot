...
struct Builder {
    configs: std::sync::Mutex<HashMap<String, Box<dyn BlobServiceBuilder>>>,
    instantiated_stores: std::sync::Mutex<HashMap<String, Arc<dyn BlobService>>>,
}
...
impl Builder {
    fn build_composition(
        self: Arc<Builder>,
        entrypoint: String,
    ) -> BoxFuture<'static, anyhow::Result<Arc<dyn BlobService>>> {
        Box::pin(async move {
            let entrypoint_config = {
                if let Some(built) = self.instantiated_stores.lock().unwrap().get(&entrypoint) {
                ...
                self.configs
                    ...
                    .unwrap()
                    .remove(&entrypoint)
                    .ok_or(anyhow::anyhow!("store not found or recursive reference: {}", &entrypoint))?
            };

            let store = entrypoint_config
                .build(&entrypoint, &|name| {
                    let builder = self.clone();
                    Box::pin(async { builder.build_composition(name).await })
                })
                .await?;

            self.instantiated_stores
                .lock()
                .unwrap()
                .insert(entrypoint, store.clone());
