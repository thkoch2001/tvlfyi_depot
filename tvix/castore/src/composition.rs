...
/// When deserializing store configs, the registry provides a stateful mapping from the
/// `type` tag of an internally tagged enum to a Config struct which is deserialized into a
/// `Box<dyn ServiceBuilder<Output = Arc<dyn BlobService>>>` (the same for DirectoryService
/// instead of BlobService etc).
//
