...
impl<T: ?Sized> Registry<T> {
    ...
    pub fn register<C: DeserializeOwned, S: IntoDynBox<T>>(
        &mut self,
        name: &'static str,
        from_config: &'static (dyn Fn(C) -> Result<S, Box<dyn std::error::Error>> + Send + Sync + 'static),
    ) {
        let from_config = &*from_config;
        self.0.insert(
            name,
            BoxFnSeed(Box::new(move |de| {
                let cfg: C = deserialize(de)?;
                Ok(from_config(cfg).map_err(|err| serde::de::Error::custom(err))?.into())
            })),
        );
    }
}
...
lazy_static! {
    pub static ref BLOB_REG: Mutex<Registry<dyn BlobService>> = {
        ...
        reg.register("objectstore", &super::blobservice::ObjectStoreBlobService::from_config);
        reg.register("memory", &|_: NoConfig| Ok(super::blobservice::MemoryBlobService::default()));
        Mutex::new(reg)
    };
    pub static ref DIRECTORY_REG: Mutex<Registry<dyn DirectoryService>> = {
        let mut reg = Registry(Default::default());
        reg.register("objectstore", &super::directoryservice::ObjectStoreDirectoryService::from_config);
        reg.register("memory", &|_: NoConfig| Ok(super::directoryservice::MemoryDirectoryService::default()));
        Mutex::new(reg)
    };
}
