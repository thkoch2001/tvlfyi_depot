...
impl<'a> CompositionContext<'a> {
    ...
    fn build_internal<T: ?Sized + Send + Sync + 'static>(
        ...
        if entrypoint.contains("://") {
            // This is a url. we are building an anonymous store
...
impl Composition {
    ...
    /// The entrypoint is the name of the store which should be instantiated.
    ///
    /// EXPERIMENTAL: The entrypoint can also be a URL string, which is created as an
    /// anonymous store. Instantiations of the same URL will
