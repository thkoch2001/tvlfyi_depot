...
pub struct CompositionContext<T> {
    stack: Vec<String>,
    composition: Composition<T>,
}

impl<T: Clone + Send + Sync + 'static> CompositionContext<T> {
    pub async fn resolve(
        ...
        if self.stack.contains(&entrypoint) {
            return Err(CompositionError::Recursion.into());
        }
        Ok(self
            .composition
...
enum InstantiationState<T> {
    ...
    InProgress(tokio::sync::watch::Receiver<Option<Result<T, CompositionError>>>),
    Done(Result<T, CompositionError>),
}

#[derive(Clone)]
pub struct Composition<T> {
    stores: Arc<std::sync::Mutex<HashMap<String, InstantiationState<T>>>>,
}
...
impl<T: Clone + Send + Sync + 'static> Composition<T> {
    ...
    pub fn from_configs_iter(
        ...
        Composition {
            stores: Arc::new(std::sync::Mutex::new(
                configs
                    .into_iter()
    ...
    pub async fn build(&self, entrypoint: &str) -> Result<T, CompositionError> {
    ...
    fn build_internal(
        ...
        stack: Vec<String>,
        entrypoint: String,
    ) -> BoxFuture<'_, Result<T, CompositionError>> {
