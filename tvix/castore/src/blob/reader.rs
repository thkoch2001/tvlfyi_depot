...
use crate::blobservice::ChunkMeta;
use crate::{chunkstore::ChunkStore, B3Digest};
use std::io;
use std::io::Cursor;
...
pin_project! {
    ...
    pub struct BlobReader<CS> {
        chunked_blob: ChunkedBlob<CS>,
    ...
    CS: ChunkStore + Clone + 'static,
{
    ...
    where
        BS: BlobService,
    {
        let Some(chunks) = blob_service.chunks(digest).await? else {
            return Ok(None);
        };

        Ok(Some(Self::from_chunks(chunks.into_iter(), chunk_store)))
    }

    /// Construct a new [ChunkedReader], by retrieving a list of chunks (their
    /// blake3 digests and chunk sizes)
    pub fn from_chunks(chunks_it: impl Iterator<Item = ChunkMeta>, chunk_store: CS) -> Self {
        let chunked_blob = ChunkedBlob::from_iter(chunks_it, chunk_store);
...
struct ChunkedBlob<CS> {
    chunk_store: CS,
    chunks: Vec<(u64, u64, B3Digest)>,
}

impl<CS> ChunkedBlob<CS>
where
    CS: ChunkStore + Clone + 'static,
{
    ...
    fn from_iter(chunks_it: impl Iterator<Item = ChunkMeta>, chunk_store: CS) -> Self {
        let mut chunks = Vec::new();
        let mut offset: u64 = 0;

        for chunk in chunks_it {
            chunks.push((offset, chunk.size, chunk.digest));
            offset += chunk.size;
        }
        ...
        let readers_stream = tokio_stream::iter(chunks.into_iter().enumerate()).map(
            move |(nth_chunk, (_chunk_start_offset, chunk_size, chunk_digest))| {
                ...
                async move {
                    ...
                    Ok::<_, std::io::Error>(chunk_reader)
                }
            },
        );
...
mod test {
    ...
    use crate::{
        blob::BlobWriter,
        blobservice::{BlobService, ChunkMeta, MemoryBlobService},
    ...
    lazy_static! {
        ...
        pub static ref BLOB_1_LIST: [ChunkMeta; 5] = [
            ChunkMeta { digest: CHUNK_1_DIGEST.clone(), size: 2},
            ChunkMeta { digest: CHUNK_2_DIGEST.clone(), size: 4},
            ChunkMeta { digest: CHUNK_3_DIGEST.clone(), size: 1},
            ChunkMeta { digest: CHUNK_4_DIGEST.clone(), size: 2},
            ChunkMeta { digest: CHUNK_5_DIGEST.clone(), size: 7},
        ];
    }
    ...
    async fn gen_services_blob1() -> (Arc<dyn BlobService>, Arc<dyn ChunkStore>) {
        ...
        (blob_service, chunk_store)
    }

    #[tokio::test]
    async fn test_read() {
        let (blob_service, chunk_store) = gen_services_blob1().await;
        let mut chunked_reader =
            BlobReader::from_chunks(BLOB_1_LIST.clone().into_iter(), chunk_store);
    ...
    async fn test_read_missing_chunks() {
        ...
        for blob_contents in [CHUNK_1.to_vec(), CHUNK_2.to_vec()] {
            let mut bw = BlobWriter::new(blob_service, chunk_store);
            tokio::io::copy(&mut std::io::Cursor::new(blob_contents), &mut bw)
                .await
                .expect("writing blob");
