    ...
    async fn open_read(&self, digest: &B3Digest) -> io::Result<Option<Box<dyn BlobReader>>> {
        ...
        Ok(match self.object_store.get(&p).await {
            Ok(res) => {
                ...
                // TODO: decompress and validate b3 digest matches!
...
async fn chunk_and_upload<R: AsyncRead + Unpin>(
                ...
                match object_store.head(&chunk_path).await {
                    ...
                    Ok(_) => {
                        trace!(
                            ...
                            chunk.size = chunk_data.length,
                            chunk.path = %chunk_path,
                            "chunk already exists on backend"
                        );
                    }
    ...
    match object_store.head(&blob_path).await {
        ...
        Ok(_) => {
            trace!(
                blob.digest = BASE64.encode(&blob_digest.as_slice()),
                blob.path = %blob_path,
    ...
    Fut: Future,
{
    fn poll_write(
        ...
        // poll the future, ensure it stays pending
        let fut = this.fut.as_pin_mut().expect("not future");
        let fut_p = Box::pin(fut).poll_unpin(cx);
        debug_assert!(fut_p.is_pending(), "fut must keep pending");
    ...
    Fut: Future<Output = io::Result<B3Digest>> + Send + Unpin,
{
    async fn close(&mut self) -> io::Result<B3Digest> {
        match self.writer.take() {
            Some(writer) => {
                // drop the writer explicitly, so the other side will read EOF.
                drop(writer);
                ...
                // await it.
                let fut = pin!(fut);
                let fut_output = fut.await;

                // store it in fut_output, so we won't pull the future
                // after completion.
                // TODO: cloneable error, so we can store it
                // self.fut_output = Some(fut_output);
                fut_output
            }
            None => {
                // called a second time, return self.fut_output.
                // self.fut_output.expect("fut_output must be some").clone()
                todo!()
            }
        }
    }
...
mod test {
    ...
    async fn upload_blob(fix_blob_contents: &[u8], fix_blob_digest: &B3Digest) {
        ...
        assert_eq!(blob_digest, blob_digest);
