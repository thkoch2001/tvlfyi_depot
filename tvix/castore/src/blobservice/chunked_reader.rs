    ...
    BS: AsRef<dyn BlobService> + Clone + 'static,
{
    ...
    fn get_chunk_idx_for_position(&self, pos: u64) -> Option<usize> {
        self.chunks
            .binary_search_by(|(chunk_start_offset, chunk_size, _)| {
                if *chunk_start_offset > pos {
                    return Ordering::Greater;
                }
                if chunk_start_offset + chunk_size <= pos {
    ...
    pub fn reader_skipped_offset<'a>(&'a self, offset: u64) -> Box<dyn AsyncRead + Unpin + 'a> {
        // construct a stream of all chunks starting with the given offset
        let start_chunk_idx = self
            .get_chunk_idx_for_position(offset)
            .expect("outside of blob"); // TODO: error handling. Should this just be a reader returning EOF?

        let skip_first_chunk_bytes = (offset - self.chunks[start_chunk_idx].0) as usize;

        let blob_service = self.blob_service.clone();
        let readers_stream = tokio_stream::iter(self.chunks[start_chunk_idx..].iter()).map(
        ...
        // skip inside the first chunk
        let bytes_stream = bytes_stream.skip(skip_first_chunk_bytes);
    ...
    BS: AsRef<dyn BlobService> + Clone + 'static,
{
    fn start_seek(self: Pin<&mut Self>, position: std::io::SeekFrom) -> std::io::Result<()> {
        ...
        *pos = new_position;
        this.r = pin!(this.chunked_blob.reader_skipped_offset(new_position));
...
mod test {
    ...
    const CHUNK_5: [u8; 7] = hex!("09101112131415");
    ...
    async fn test_read() {
        ...
        for blob_contents in [
            ...
            bw.close().await.expect("failed to close writing blob");
        }

        let cb = ChunkedBlob::from_iter(BLOB_1_LIST.clone().into_iter(), blob_service.clone());
        ...
        assert_eq!(
            hex!("00010203040506070809101112131415").to_vec(),
            buf,
