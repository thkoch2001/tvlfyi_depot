    ...
    BS: AsRef<dyn BlobService> + Clone + 'static,
{
    ...
    pub fn reader_skipped_offset(&self, offset: u64) -> Box<dyn AsyncRead + Unpin> {
        ...
        eprintln!("start_chunk_idx: {}", start_chunk_idx);
        eprintln!("chunk: {:?}", self.chunks[start_chunk_idx]);

        let skip_first_chunk_bytes = (offset - self.chunks[start_chunk_idx].0) as usize;
        eprintln!("skip_first_chunk_bytes: {}", skip_first_chunk_bytes);
        ...
        let bytes_stream = bytes_stream.skip(skip_first_chunk_bytes); // wat?
    ...
    BS: AsRef<dyn BlobService> + Clone + 'static,
{
    fn start_seek(self: Pin<&mut Self>, position: std::io::SeekFrom) -> std::io::Result<()> {
        ...
        eprintln!("new_position: {}", new_position);
...
mod test {
    ...
    async fn test_seek() {
        ...
        // seek one bytes before the end
        {
            ...
            assert_eq!(hex!("15").to_vec(), buf);
        }
    }

    // TODO: more tests for seeking, tests for missing chunks, …
}
