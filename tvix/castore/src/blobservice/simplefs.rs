...
/// Connects to a tvix-store BlobService on an existing mountpoint backed by a POSIX-compliant
/// filesystem.
///
/// It takes an existing mountpoint, builds a `tmp` directory inside of it. All blobs received are
/// staged in that `tmp` directory, then they are moved into `B3DIGEST[:2]/B3DIGEST[2:]` in a
/// sharding style, e.g. `abcdef` gets turned into `ab/cdef`
///
