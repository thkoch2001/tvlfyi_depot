use std::{path::{PathBuf, Path}, task::Poll, pin::pin};
...
use super::{BlobService, BlobReader, BlobWriter};
...
impl BlobService for SimpleFilesystemBlobService {
    ...
    async fn open_read(&self, digest: &B3Digest) -> Result<Option<Box<dyn BlobReader>>, Error> {
        ...
        let reader = match tokio::fs::File::open(dst_path).await {
            Ok(file) => {
                debug!("{:?}", file);
                let reader: Box<dyn BlobReader> = Box::new(
...
impl tokio::io::AsyncWrite for SimpleFilesystemBlobWriter {
    fn poll_write(
        ...
        if let Err(e) = self.file.as_mut() {
            return Poll::Ready(Err(std::mem::replace(e, std::io::Error::new(std::io::ErrorKind::NotConnected, "this file is already closed"))));
        }
...
impl BlobWriter for SimpleFilesystemBlobWriter {
    async fn close(&mut self) -> Result<B3Digest, Error> {
        if let Err(e) = self.file.as_mut() {
            return Err(std::mem::replace(e, std::io::Error::new(std::io::ErrorKind::NotConnected, "this file is already closed")).into());
        }
