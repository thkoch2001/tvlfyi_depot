...
impl<B: BlobService + 'static> tokio::io::AsyncSeek for Reader<B> {
    fn start_seek(self: Pin<&mut Self>, pos: io::SeekFrom) -> io::Result<()> {
        ...
        *this.position_index = match this
        ...
        // TODO re-use the existing blobreader if we are seeking within the same blob
        if prev_position_bytes != *this.position_bytes {
            *this.current_blob =
                match this.segments.get(*this.position_index) {
                    Some((offset, Data::Blob(BlobRef { digest, .. }))) => {
                        ...
                        futures::future::try_maybe_done(
                            (async move {
                                let mut reader = blob_service.open_read(&digest).await?.ok_or(
                                    io::Error::new(
                                        io::ErrorKind::Other,
                    ...
                    // Not seeking into a blob, close the blob reader
                    _ => TryMaybeDone::Gone,
                };
        }
