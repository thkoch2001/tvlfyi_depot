    ...
    fn poll_read(
        ...
        match segment {
            Data::Literal(data) => {
                ...
                buf.put(data.slice(offset_in_segment..offset_in_segment + read_size));
            }
            Data::Blob(BlobRef { .. }) => {
                futures::ready!(this.current_blob.poll_unpin(cx))?;
                let reader = Pin::new(&mut this.current_blob).output_mut().unwrap();
                futures::ready!(Pin::new(reader).poll_read(cx, buf))?;
            }
        };
