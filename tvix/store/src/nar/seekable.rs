...
impl Reader {
    ...
    fn flush_segment(&mut self, offset: &mut u64, cur_segment: &mut Vec<u8>) {
    ...
    /// Used during construction.
    /// Recursively walks the node and its children, and fills `self.segments` with the appropriate
    ...
    fn walk_node(
        ...
        match node {
            ...
            tvix_castore::Node::File {
                ...
                self.flush_segment(offset, cur_segment);
            ...
            tvix_castore::Node::Directory { digest, .. } => {
                ...
                for (name, node) in directory.nodes() {
                    ...
                    self.walk_node(offset, directories, node.clone(), child_node)?;
                }
    ...
    pub async fn new(
        ...
        Self::new_with_directory_closure(maybe_directory_closure, root_node)
    }
    ...
    pub fn new_with_directory_closure(
        ...
        reader.walk_node(&mut offset, &directories, root_node, nar_node)?;
        reader.flush_segment(&mut offset, &mut cur_segment);
