    ...
    DS: AsRef<dyn DirectoryService>,
{
    ...
    let entries = try_stream! {
        ...
        loop {
            ...
            while let Some(ReaderEntry { name, node }) = dir_reader.next().await? {
                let path = parent_path.try_join(&name).expect("Tvix bug: failed to join name");
                match node {
                    ReaderNode::Directory(child_dir_reader) => {
                        // another child directory. push the child to the stack, then continue
                        // (without the stack popping at the end)
                        // drop(parent_path);
                        // drop(dir_reader);
