...
/// blob_meta already.
pub struct BlobReader<CS: ChunkService> {
    ...
    // a cusor to its contents are stored here.
    current_chunk: Option<Cursor<Vec<u8>>>,
}

impl<'items, CS: ChunkService> BlobReader<CS> {
    pub fn open(chunk_service: CS, blob_meta: proto::BlobMeta) -> Self {
        Self {
            chunk_service,
    ...
    fn read_from_current_chunk<W: std::io::Write>(
        ...
        // If there's still something in partial_chunk, read from there and return
        if let Some(current_chunk) = &mut self.current_chunk {
            let result = io::copy(&mut current_chunk.take(m as u64), buf);

            return match result {
                Ok(n) => {
                    // if we were not able to read less than m,
...
impl<'items, CS: ChunkService> std::io::Read for BlobReader<CS> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        loop {
            ...
            let n = self.read_from_current_chunk(read_max, &mut buf_w)?;
            bytes_read += n;
            ...
            // if we filled buf entirely with that, we're done.
            ...
            match self.chunks_iter.next() {
                ...
                Some(chunk_meta) => match self.chunk_service.get(&chunk_meta.digest) {
                    ...
                    // Error occured while fetching the next chunk
                    Err(e) => {
                        break Err(std::io::Error::new(
                            io::ErrorKind::InvalidData,
                            format!("chunk_service returned error: {}", e.to_string()),
                        ));
                    }
                },
            }
...
mod tests {
    ...
    fn empty_blobmeta() -> anyhow::Result<()> {
        let cs = SledChunkService::new(TempDir::new()?.path().to_path_buf())?;
        ...
        let mut blob_reader = BlobReader::open(cs.clone(), blobmeta);
    ...
    fn empty_chunk() -> anyhow::Result<()> {
        ...
        let dgst = cs.put(vec![]).expect("must succeed");
    ...
    fn single_chunk() -> anyhow::Result<()> {
        ...
        // insert a single chunk
        let dgst = cs.put(DUMMY_DATA_1.clone()).expect("must succeed");
    ...
    fn wrong_size_fail() -> anyhow::Result<()> {
        ...
        // insert chunks
        let dgst_1 = cs.put(DUMMY_DATA_1.clone()).expect("must succeed");
    ...
    fn multiple_chunks() -> anyhow::Result<()> {
        ...
        // insert chunks
        let dgst_1 = cs.put(DUMMY_DATA_1.clone()).expect("must succeed");
        let dgst_2 = cs.put(DUMMY_DATA_2.clone()).expect("must succeed");
        ...
        // read via io::copy
        {
            let mut blob_reader = BlobReader::open(cs.clone(), blobmeta.clone());
