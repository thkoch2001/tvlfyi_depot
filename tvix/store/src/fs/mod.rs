...
use crate::{lookupservice::LookupService, proto::lookup_node::Contents};
...
pub struct TvixStoreFs {
    ...
    lookup_service: Arc<dyn LookupService>,
...
impl TvixStoreFs {
    ...
    /// [self.inode_tracker].
    fn lookup_path(&self, path: &Path) -> Result<Option<(u64, Arc<InodeData>)>, Error> {
        let ino = {
            ...
            match self.tokio_handle.block_on(task).unwrap()? {
                ...
                Some(contents) => {
                    let inode_data: InodeData = match contents {
                        ...
                        Contents::Directory(_) => InodeData::LookupDirectory(path.to_path_buf()),
                    };

                    // Let's check if someone else beat us to updating the inode tracker and
                    // store_paths map.
                    let mut paths = self.paths.write();
                    if let Some(ino) = paths.get(path).cloned() {
                        return Ok(Some((
                            ino,
                            self.inode_tracker.read().get(ino).expect("must exist"),
                        )));
                    ...
                    let (ino, inode) = {
                        ...
                        let ino = inode_tracker.put(inode_data);
...
impl FileSystem for TvixStoreFs {
    ...
    fn lookup(
        ...
        if parent == ROOT_ID {
            return match self.lookup_path(Path::new(name.to_str().expect("invalid UTF-8"))) {
                Err(e) => {
                    warn!("{}", e);
        ...
        let parent_data = match *parent_data {
            ...
            InodeData::LookupDirectory(ref base_dir) => {
                let path = from_utf8(name.to_bytes()).expect("invalid UTF-8");
                let (child_ino, child_inode_data) = match self.lookup_path(&base_dir.join(path))? {
                    None => return Err(io::Error::from_raw_os_error(libc::ENOENT)),
                    Some(ino) => ino,
                };
    ...
    fn readdir(
        ...
        if inode == ROOT_ID {
            ...
            //     while let Some((i, entry)) = rx.blocking_recv() {
            ...
            //         let path = Path::new(from_utf8(&entry.name).expect("invalid UTF-8"));
            //         let ino = {
            //             // This extra scope makes sure we drop the read lock
            //             // immediately after reading, to prevent deadlocks.
            //             let paths = self.paths.read();
            ...
            //             Some(ino) => ino,
            //             None => {
            //                 let inode_data = match entry.node.unwrap().contents.unwrap() {
            //                     Contents::Canode(node) => {
            //                         // We know the root node exists and the store_path can be parsed because clients MUST validate.
            //                         (&node.node.unwrap()).into()
            //                     }
            //                     Contents::Directory(_) => {
            //                         InodeData::LookupDirectory(path.to_path_buf())
            //                     }
            //                 };
            ...
            //                 self.paths.write().insert(path.to_path_buf(), ino);
            //                 ino
            //             }
            //         };

            //         let ty = match entry.node.unwrap().contents.unwrap() {
            //             Contents::Canode(node) => match node.node.unwrap() {
            //                 Node::Directory(_) => libc::S_IFDIR,
            //                 Node::File(_) => libc::S_IFREG,
            //                 Node::Symlink(_) => libc::S_IFLNK,
            //             },
            //             Contents::Directory(..) => libc::S_IFDIR,
            //         };
        ...
        let dir_inode_data = match *dir_inode_data {
            ...
            // Listing is not supported here yet.
            InodeData::LookupDirectory(_) => return Ok(()),
        };

        match *dir_inode_data {
    ...
    fn open(
        ...
        match *self.inode_tracker.read().get(inode).unwrap() {
            ...
            InodeData::Directory(..) | InodeData::Symlink(_) | InodeData::LookupDirectory(_) => {
                warn!("is directory");
