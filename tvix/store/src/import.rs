...
impl From<super::Error> for Error {
    fn from(value: super::Error) -> Self {
        match value {
            ...
            crate::Error::StorageLoadError(_) => panic!("error"),
            crate::Error::StoragePutError(_) => panic!("tvix bug"),
            // ^ this might be a sign super::Error might not yet be well specced out
        }
    }
}
...
#[instrument(skip(blob_service, chunk_service, directory_service))]
...
>(
    ...
    for entry in WalkDir::new(p).follow_links(false).contents_first(true) {
        ...
        if entry.depth() == 0 {
            ...
            // TODO: is it ok to unwrap here in all cases?
