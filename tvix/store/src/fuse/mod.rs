...
use self::inodes::{DirectoryInodeData, InodeTracker};
...
impl FUSE {
    ...
    fn directory_digest_to_inode_data(
        &self,
...
impl fuser::Filesystem for FUSE {
        ...
        if parent_ino == fuser::FUSE_ROOT_ID {
            ...
            let parent_data = match *parent_data {
                ...
                InodeData::Directory(DirectoryInodeData::Sparse(ref parent_digest, _)) => {
                    // TODO: add fetching code here
                    // TODO: handle error!
                    match self.directory_digest_to_inode_data(parent_digest) {
                        Ok(new_data) => {
    ...
    fn readdir(
        ...
        // lookup the node by inode. We rely on the previous `lookup` populating
        // sparse directories, so no need for fetching here..
        match *self.inode_tracker.get(ino).unwrap() {
            InodeData::Regular(..) | InodeData::Symlink(..) => {
                warn!("Not a directory");
                reply.error(libc::ENOTDIR);
            }
            // This shouldn't happen, `lookup` should have populated it.
