...
async fn put_get_multiple() -> anyhow::Result<()> {
    ...
    let mock_directory_a = Directory::default();
    let mock_directory_b = Directory {
        directories: vec![DirectoryNode {
            name: "a".to_string(),
            digest: mock_directory_a.digest(),
            size: 1,
        }],
        ..Default::default()
    };
    ...
    let put_resp = service
        .put(tonic_mock::streaming_request(
            vec![mock_directory_b.clone()],
        ))
        .await
    ...
    let put_resp = service
        .put(tonic_mock::streaming_request(vec![
            mock_directory_a.clone(),
            mock_directory_b.clone(),
        ]))
        .await
        .expect("must succeed");

    assert_eq!(mock_directory_b.digest(), put_resp.into_inner().root_digest);
    ...
    let items = get_directories(
        ...
        GetDirectoryRequest {
            recursive: false,
            by_what: Some(ByWhat::Digest(mock_directory_b.digest())),
        },
    )
    .await
    .expect("must not error");
    ...
    assert_eq!(vec![mock_directory_b.clone()], items);
    ...
    assert_eq!(
        vec![mock_directory_b.clone(), mock_directory_a.clone()],
        items
    );
