    ...
    TvixStoreIO<BS, DS, PS, NCS>
{
    ...
    /// [crate::proto::PathInfo] describing the path, without inserting it into
    /// [PathInfoService].
    pub fn import_path_with_pathinfo(
        &mut self,
        path: &std::path::Path,
    ) -> Result<crate::proto::PathInfo, ErrorKind> {
        ...
        // assemble (and return) the [crate::proto::PathInfo] object.
        Ok(crate::proto::PathInfo {
    ...
    > EvalIO for TvixStoreIO<BS, DS, PS, NCS>
{
    ...
    fn import_path(&mut self, path: &std::path::Path) -> Result<std::path::PathBuf, ErrorKind> {
        ...
        // put into [PathInfoService].
        self.path_info_service.put(path_info.clone()).unwrap(); // TODO: error handling
