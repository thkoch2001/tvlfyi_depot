...
fn gen_grpc_service(p: PathBuf) -> GRPCDirectoryServiceWrapper<SledDirectoryService> {
    let directory_service = SledDirectoryService::new(p.join("directories").to_path_buf()).unwrap();
...
async fn not_found() -> anyhow::Result<()> {
    let service = gen_grpc_service(TempDir::new()?.path().to_path_buf());
