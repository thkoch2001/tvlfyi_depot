...
fn gen_grpc_directory_service(
    p: PathBuf,
) -> anyhow::Result<GRPCDirectoryServiceWrapper<SledDirectoryService>> {
    let directory_service = SledDirectoryService::new(p.join("directories").to_path_buf())?;
    Ok(GRPCDirectoryServiceWrapper::from(directory_service))
}
...
async fn not_found() -> anyhow::Result<()> {
    let service = gen_grpc_directory_service(TempDir::new()?.path().to_path_buf())?;
