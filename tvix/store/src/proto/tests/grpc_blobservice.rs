...
// fn gen_grpc_blob_service<GBS: GRPCBlobService>(p: PathBuf) -> anyhow::Result<GBS> {
fn gen_grpc_blob_service(
    p: PathBuf,
) -> anyhow::Result<GRPCBlobServiceWrapper<SledBlobService, SledChunkService>> {
    let blob_service = SledBlobService::new(p.join("blobs").to_path_buf())?;
    let chunk_service = SledChunkService::new(p.join("chunks").to_path_buf())?;
    Ok(GRPCBlobServiceWrapper::new(blob_service, chunk_service))
}

/// Trying to read a non-existent blob should return a not found error.
#[tokio::test]
async fn not_found_read() -> anyhow::Result<()> {
    let service = gen_grpc_blob_service(TempDir::new()?.path().to_path_buf())?;
...
#[tokio::test]
async fn not_found_stat() -> anyhow::Result<()> {
    let service = gen_grpc_blob_service(TempDir::new()?.path().to_path_buf())?;
...
#[tokio::test]
async fn put_read_stat() -> anyhow::Result<()> {
    let service = gen_grpc_blob_service(TempDir::new()?.path().to_path_buf())?;
...
/// `read()` method.
#[tokio::test]
async fn put_read_stat_large() -> anyhow::Result<()> {
    let service = gen_grpc_blob_service(TempDir::new()?.path().to_path_buf())?;
