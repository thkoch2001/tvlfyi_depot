...
#[test]
fn convert_valid() {
    let path_info = PATH_INFO_WITH_NARINFO.clone();
...
fn convert_fail_node(#[case] node: castorepb::node::Node, #[case] exp_err: ValidatePathInfoError) {
    ...
    let mut p = PATH_INFO_WITH_NARINFO.clone();
...
fn convert_without_narinfo_fail() {
    assert_eq!(
        ...
        PathInfo::try_from(PATH_INFO_WITHOUT_NARINFO.clone()).expect_err("must fail"),
    );
}
