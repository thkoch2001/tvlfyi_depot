    ...
    async fn get(
        ...
        task::spawn(async move {
            ...
            while let Some(ref digest) = deq.pop_front() {
                // TODO: add the digest_b64 to a span, but pay attention to
                ...
                let res: Result<proto::Directory, Status> = match client.get(
                    &proto::get_directory_request::ByWhat::Digest(digest.to_vec()),
                ) {
                    ...
                    Ok(None) => {
                        if !sent_directory_dgsts.is_empty() {
                            ...
                            // parents. Log this out. The node we started with is part of the span.
                            warn!("consistency issue: {} was not found!", digest_b64)
                        }
    ...
    async fn put(
        ...
        while let Some(directory) = req_inner.message().await? {
                ...
                .get(&proto::get_directory_request::ByWhat::Digest(dgst.to_vec()))
            {
                ...
                Ok(None) => {
                    if let Err(e) = self.client.put(directory) {
                        return Err(e.into());
                    }
                }
            }
