    ...
    > super::blob_service_server::BlobService for GRPCBlobServiceWrapper<BS, CS>
{
    ...
    async fn read(
        ...
        // query the chunk service for more detailled blob info
        let chunks_resp = self.blob_service.stat(&super::StatBlobRequest {
                ...
                task::spawn(async move {
                    for chunkmeta in blobmeta.chunks {
                        ...
                        let res = match chunk_client.get(&chunkmeta.digest) {
                            Err(e) => Err(e.into()),
                            Ok(None) => Err(Error::StorageLoadError(format!(
                            ...
                            Ok(Some(data)) => {
                                ...
                                if data.len() as u32 != chunkmeta.size {
                                    Err(Error::StorageLoadError(format!(
    ...
    async fn put(
        ...
        for chunk in chunker {
            ...
            match self.chunk_service.has(chunk_digest.as_bytes()) {
                Err(e) => {
                    return Err(Error::StorageLoadError(format!(
                ...
                Ok(has_chunk) => {
                    if !has_chunk {
                        if let Err(e) = self.chunk_service.put(chunk_data.to_vec()) {
                            return Err(Error::StoragePutError(format!(
