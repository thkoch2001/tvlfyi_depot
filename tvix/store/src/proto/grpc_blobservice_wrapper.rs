...
use std::{collections::VecDeque, io, pin::Pin, sync::Arc, ops::{DerefMut, Deref}};
use tokio::task;
...
impl<const N: usize> Default for BytesMutWithDefaultCapacity<N> {
    fn default() -> Self {
        BytesMutWithDefaultCapacity { inner: bytes::BytesMut::with_capacity(N) }
    }
}
...
impl super::blob_service_server::BlobService for GRPCBlobServiceWrapper {
    ...
    async fn read(
        ...
        match self.blob_service.open_read(&req_digest) {
            Ok(Some(reader)) => {
                let async_reader: SyncReadIntoAsyncRead<_, BytesMutWithDefaultCapacity<{100 * 1024}>> = reader.into();
