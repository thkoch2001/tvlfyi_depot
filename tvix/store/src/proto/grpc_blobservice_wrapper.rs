use crate::blobservice::{BlobService, BlobWriter};
use data_encoding::BASE64;
use std::{collections::VecDeque, io, pin::Pin};
use tokio::task;
use tokio_stream::StreamExt;
use tokio_util::io::SyncReadStream;
    ...
    for GRPCBlobServiceWrapper<BS>
{
    ...
    async fn read(
        ...
        match self.blob_service.open_read(&req_digest) {
            Ok(Some(reader)) => {
                let stream: SyncReadStream<_> = reader.into();
                ...
                let chunks_stream = stream.map(stream_mapper);
