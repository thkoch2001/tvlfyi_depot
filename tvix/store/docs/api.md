...
### PathInfoService
The PathInfo service provides lookups from an store path hash to a `PathInfo`
message.
...
If something other than the root path (like `pkgs/top-level/all-packages.nix`)
is requested, the root_node Directory is inspected and potentially a chain of
`Directory` objects requested from *DirectoryService*. [^n+1query].
