...
mod builtins {
    ...
    #[builtin("identity")]
    pub async fn builtin_identity(co: GenCo, x: Vec<Value>) -> Result<Value, ErrorKind> {
        Ok(todo!())
    }

    #[builtin("tryEval")]
    pub async fn builtin_try_eval(co: GenCo, #[lazy] _x: Vec<Value>) -> Result<Value, ErrorKind> {
        todo!()
    }
}
