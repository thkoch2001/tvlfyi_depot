    ...
    for item in items.iter_mut() {
        if let Item::Fn(f) = item {
                ...
                .position(|attr| attr.path.get_ident().iter().any(|id| *id == "builtin"))
            {
                ...
                let builtin_arguments = value_args
                    ...
                    .map(|arg| {
                        ...
                        let name = match arg {
                            ...
                            FnArg::Typed(PatType { mut attrs, pat, .. }) => {
                                attrs.retain(|attr| {
                        ...
                        Ok(BuiltinArgument { strict, span, name })
                    })
                ...
                rewritten_args.push(parse_quote_spanned!(sig_span=> values: Vec<Value>));
                f.sig.inputs = rewritten_args.into_iter().collect();

                // Rewrite the body of the function to do said argument forcing.
                // The order of arguments is reversed, as the stack slice passed
                // to the function is also in reverse order.

                let fn_name = f.sig.ident.clone();
                let num_args = builtin_arguments.len();
                let args = (0..num_args)
