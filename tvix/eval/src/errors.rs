...
impl Error {
    pub fn fancy_format_str(&self, codemap: &CodeMap) -> String {
        let mut out = vec![];
        Emitter::vec(&mut out, Some(codemap)).emit(&vec![self.diagnostic(codemap)]);
    ...
    fn message(&self, codemap: &CodeMap) -> String {
        match &self.kind {
            ...
            ErrorKind::AssertionFailed => format!("assertion failed"),
            ...
            ErrorKind::DynamicKeyInLet => {
                format!("dynamically evaluated keys are not allowed in let-bindings")
            }

            ErrorKind::UnknownStaticVariable => format!("variable not found"),
...
to a missing value in the attribute set(s) included via `with`."#,
            ...
            ErrorKind::VariableAlreadyDefined(_) => format!("variable has already been defined"),

            ErrorKind::NotCallable => {
                format!("this value is not callable (i.e. not a function or builtin)")
            }

            ErrorKind::InfiniteRecursion => format!("infinite recursion encountered"),
