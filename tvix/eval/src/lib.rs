...
/// fields is optional.
pub struct Evaluation<'co, 'ro, 'ctx, IO> {
    ...
    pub context: Option<&'ctx HashMap<String, Value>>,
...
impl<'co, 'ro, 'ctx, IO> Evaluation<'co, 'ro, 'ctx, IO>
where
...
impl<'co, 'ro, 'ctx> Evaluation<'co, 'ro, 'ctx, Box<dyn EvalIO>> {
