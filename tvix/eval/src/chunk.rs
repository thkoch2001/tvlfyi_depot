...
impl Chunk {
    ...
    pub unsafe fn patch_jump(&mut self, idx: CodeIdx) {
        let offset = JumpOffset(
            // self.last_op_idx().unwrap().0 - idx.0, //
            self.code.len() - idx.0 - (1 + std::mem::size_of::<usize>()),
        );
    ...
    pub fn disassemble_op<W: Write>(
        ...
        match unsafe { OpCode::read(&self.code[idx.0..]).0 } {
            ...
            op @ (OpCode::OpJump(off)
            | OpCode::OpJumpIfFalse(off)
            | OpCode::OpJumpIfTrue(off)
            | OpCode::OpJumpIfCatchable(off)
            | OpCode::OpJumpIfNotFound(off)
            | OpCode::OpJumpIfNoFinaliseRequest(off)) => {
                let jump_target = CodeIdx(idx.0 + off.0);
                writeln!(writer, "{op:?} → {jump_target:?}")
            }
    ...
    pub(crate) unsafe fn read_and_inc(&self, ip: CodeIdx) -> (OpCode, CodeIdx) {
        dbg!(self.op_codes().collect::<Vec<_>>());
        dbg!(self.code.len());
