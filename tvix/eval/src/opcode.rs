...
pub enum OpCode {
    ...
    /// Pop a value `val` off of the stack, then pop a
    /// Thunk(Blackhole) off the stack; change the Blackhole and the
    /// rest of its Blackhole-chain to ThunkRepr::Evaluated(val,
    /// depth).  Finally, push `val` back onto the stack.
    OpUpdateBlackholeChain(ForcingDepth),
