...
pub fn interpret(code: &str, location: Option<PathBuf>, options: Options) -> EvalResult<Value> {
    ...
    let builtins = {
        ...
        // SAFETY: We need to insert import into the builtins, but the
        // builtins passed to import must have import *in it*. We know
        // that during construction nothing else can access the
        // builtins, so mutating the shared value is safe in this
        // block (for now). We do need to get rid of this, though.
        unsafe {
