...
impl<'o> VM<'o> {
    ...
    fn run_op(&mut self, op: OpCode) -> EvalResult<()> {
        match op {
            ...
            OpCode::OpClosure(idx) => {
                ...
                let thunk = Thunk::from(Closure::new(blueprint));
            ...
            OpCode::OpFinalise(StackIdx(idx)) => {
                match &self.stack[self.frame().stack_base + idx] {
                    Value::Closure(_) => panic!("this should not happen"),

                    Value::Thunk(thunk) => {
                        thunk
                            .upvalues_mut()
                            .resolve_deferred_upvalues(&self.stack[self.frame().stack_base..]);
    ...
    fn populate_upvalues(
        &mut self,
        count: usize,
        mut upvalues: RefMut<Upvalues>,
    ) -> EvalResult<()> {
        for _ in 0..count {
            match self.inc_ip() {
                OpCode::DataLocalIdx(StackIdx(local_idx)) => {
                    let idx = self.frame().stack_base + local_idx;
