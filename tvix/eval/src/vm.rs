...
macro_rules! cmp_op {
    ...
    ( $self:ident, $frame:ident, $op:tt ) => {{
        ...
        let ordering = a.nix_cmp(&b, todo!("vm")).map_err(|kind| $frame.error(kind))?;
        let result = Value::Bool(cmp_op!(@order $op ordering));
        ...
        loop {
            ...
            match op {
                ...
                OpCode::OpCoerceToString => {
                    ...
                    let string = self
                        .stack_pop()
                        .coerce_to_string(CoercionKind::Weak, todo!())
                        .with_span(&frame)?;
...
type Continuation = Box<dyn FnOnce(&mut NeoVM) -> EvalResult<Trampoline>>;
        ...
        match op {
            ...
            OpCode::OpAdd => {
                ...
                let result = match (&a, &b) {
                    ...
                    _ => todo!(), // fallible!(self, arithmetic_op!(&a, &b, +)),
                };
            ...
            OpCode::OpForce => {
                if let Some(Value::Thunk(_)) = self.stack.last() {
                    let value = self.pop();
                    let trampoline = fallible!(self, Thunk::force_trampoline(todo!(), value));
...
pub fn neo_run_lambda(
    ...
    let mut vm = NeoVM::new(nix_search_path, io_handle, observer, globals);
