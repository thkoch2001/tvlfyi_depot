...
impl<'o> VM<'o> {
    ...
    pub(crate) fn nix_eq(
        ...
        v1: Value,
        v2: Value,
        allow_top_level_pointer_equality_on_functions: bool,
    ) -> EvalResult<bool> {
        self.push(v1);
        self.push(v2);
        let res = self.nix_op_eq(allow_top_level_pointer_equality_on_functions);
    ...
    pub(crate) fn nix_op_eq(
        ...
        let mut allow_pointer_equality_on_functions = allow_top_level_pointer_equality_on_functions;
        ...
        let res = 'outer: loop {
            ...
            match (v1, v2) {
                (Value::List(l1), Value::List(l2)) => {
                    allow_pointer_equality_on_functions = true;
                    if l1.ptr_eq(&l2) {
                        continue;
                    }
                    if l1.len() != l2.len() {
                ...
                (v1, v2) => {
                    if allow_pointer_equality_on_functions {
