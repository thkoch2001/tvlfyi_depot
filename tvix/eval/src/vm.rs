...
impl<'o> VM<'o> {
    ...
    pub fn call_value(&mut self, callable: &Value) -> EvalResult<Value> {
        match callable {
            ...
            Value::Attrs(attrs) => match attrs.select("__functor") {
                None => Err(self.error(ErrorKind::NotCallable(callable.type_of()))),
                Some(functor) => {
                    println!("foo");
                    // The functor receives the set itself as its first
                    // argument.
    ...
    pub fn tail_call_value(&mut self, callable: Value) -> EvalResult<()> {
        match callable {
            ...
            Value::Attrs(ref attrs) => match attrs.select("__functor") {
                ...
                Some(functor) => {
                    ...
                    // argument.
                    self.push(callable.clone());
                    self.tail_call_value(functor.clone())
                }
            },
