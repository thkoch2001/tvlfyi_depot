...
macro_rules! cmp_op {
    ...
    ( $self:ident, $frame:ident, $op:tt ) => {{
        ...
        let ordering = a.nix_cmp(&b, $self).map_err(|kind| $frame.error(kind))?;
        let result = Value::Bool(cmp_op!(@order $op ordering));
...
enum Frame {
    ...
    Generator {
        ...
        generator: Gen<Value, (), Pin<Box<dyn Future<Output = Result<Value, ErrorKind>>>>>,
    },
}
            ...
            match frame {
                ...
                Frame::Generator {
                    ...
                    match generator.resume() {
                        // If the generator yields a value, it is requesting
                        // that value to be forced.
                        genawaiter::GeneratorState::Yielded(value) => {
                            // This generator is not done yet, put it back on
                            // the frame stack.
                            self.frames.push(Frame::Generator { span, generator });
                        }
    ...
    fn execute_bytecode(&mut self, span: LightSpan, mut frame: CallFrame) -> EvalResult<()> {
        loop {
            ...
            match op {
                ...
                OpCode::OpLess => cmp_op!(self, frame, <),
                OpCode::OpLessOrEq => cmp_op!(self, frame, <=),
                OpCode::OpMore => cmp_op!(self, frame, >),
                OpCode::OpMoreOrEq => cmp_op!(self, frame, >=),
                ...
                OpCode::OpResolveWith => {
                    ...
                    let value = todo!("resolve_with but flat"); // self.resolve_with(ident.as_str())?;
                    self.stack.push(value)
                }
...
impl<'o> VM<'o> {
    ...
    pub fn call_value(&mut self, callable: &Value) -> EvalResult<()> {
        todo!("VM::call_value") // probably unnecessary?
    }
...
impl<'o> OldVM<'o> {
        ...
        let res = 'outer: loop {
            ...
            match (v1, v2) {
                    ...
                    match (a1.select("type"), a2.select("type")) {
                        (Some(v1), Some(v2))
                            if "derivation"
                                == fallible!(
                                    self,
                                    v1.coerce_to_string(CoercionKind::ThunksOnly, todo!())
                                )
                                .as_str()
                                && "derivation"
                                    == fallible!(
                                        self,
                                        v2.coerce_to_string(CoercionKind::ThunksOnly, todo!())
                                    )
                                    .as_str() =>
                        {
                            if fallible!(
                                ...
                                a1.select("outPath")
                                    ...
                                    .coerce_to_string(CoercionKind::ThunksOnly, todo!())
                            ) == fallible!(
                                self,
                                a2.select("outPath")
                                    .expect("encountered a derivation with no `outPath` attribute!")
                ...
                (v1, v2) => {
                    ...
                    if !fallible!(self, v1.nix_eq(&v2, todo!())) {
                        break false;
                    }
                }
            }
    ...
    pub fn call_builtin(&mut self, builtin: Builtin) -> EvalResult<()> {
        ...
        let arg = self.pop();
        let result = fallible!(self, builtin.apply(todo!(), arg));
...
pub fn run_lambda(
    ...
    let mut vm = OldVM::new(nix_search_path, io_handle, observer, globals);
