...
impl NixAttrs {
    ...
    pub fn keys(&self) -> Keys {
        Keys(match self.0.as_ref() {
            ...
            AttrsRep::Map(m) => {
                let mut keys = m.keys().collect::<Vec<&NixString>>();
                keys.sort();
                KeysInner::Map(keys.into_iter())
            }
        })
    }
...
impl IntoIterator for NixAttrs {
    ...
    fn into_iter(self) -> Self::IntoIter {
        match Rc::unwrap_or_clone(self.0) {
            ...
            AttrsRep::Map(map) => {
                let mut as_tuples = map.into_iter().collect::<Vec<(NixString, Value)>>();
                as_tuples.sort_by(|(klhs, _), (krhs, _)| klhs.cmp(krhs));
                OwnedAttrsIterator(IntoIterRepr::Finite(as_tuples.into_iter()))
            }
        }
    }
}
...
enum KeysInner<'a> {
    Empty,
    KV(IterKV),
    Map(std::vec::IntoIter<&'a NixString>),
}
