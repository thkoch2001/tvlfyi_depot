...
impl NixAttrs {
    ...
    pub fn select<'a, K: 'a>(&self, key: K) -> Option<&Value>
    where
        &'a BStr: From<K>,
    {
        self.0.select(key.into())
    }

    /// Select a required value from an attribute set by key, return
    /// an `AttributeNotFound` error if it is missing.
    pub fn select_required<'a, K: 'a>(&self, key: K) -> Result<&Value, ErrorKind>
    where
        &'a BStr: From<K>,
    {
        self.select(key)
            .ok_or_else(|| ErrorKind::AttributeNotFound {
                name: <&'a BStr>::from(key).to_string(),
            })
    }
