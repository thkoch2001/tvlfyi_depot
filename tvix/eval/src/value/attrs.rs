...
impl NixAttrs {
    ...
    /// Same as `into_iter()`, but indicates call sites that rely on
    /// the fact that the iteration is in lexicographic order.
    ...
    /// Wrapper around `from_map()` which avoids leaking
    /// implementation details (`BTreeSet`) beyond this module.
    pub(crate) fn from_iter<I>(iter: I) -> Self
    where
        I: Iterator<Item = (NixString, Value)>,
    {
        Self::from_map(BTreeMap::from_iter(iter))
    }
