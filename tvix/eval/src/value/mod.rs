...
impl Value {
    ...
    where
        T: Into<NixString>,
    {
        Self(BoxedNan::ptra(Box::new(s.into())))
    }
...
impl Value {
    pub fn into_string(self) -> Result<NixString, Self> {
        self.0.into_ptra().map(|s| *s).map_err(Self)
    }
...
impl Value {
    ...
    pub fn explain(&self) -> String {
        match self.match_ref() {
            ...
            VRef::String(s) if s.has_context() => format!("the contextful string '{}'", s),
            VRef::String(s) => format!("the contextless string '{}'", s),
