...
impl TotalDisplay for Value {
    fn total_fmt(&self, f: &mut std::fmt::Formatter<'_>, set: &mut ThunkSet) -> std::fmt::Result {
        match self {
            ...
            Value::Catchable(catchable) => match **catchable {
                CatchableErrorKind::UnimplementedFeature(_) => {
                    f.write_str("internal[unimplemented]")
                }
                _ => panic!("total_fmt() called on a CatchableErrorKind"),
            },
        }
    }
}
