    ...
    pub fn match_ref(&self) -> VRef {
        match self.0.kind() {
            ...
            BoxedNanKind::PtrA => VRef::String(unsafe {
                NixString::string_ref_from_raw(self.0.as_ptr_unchecked().cast())
            }),
    ...
    pub fn match_mut(&mut self) -> VMut {
        match self.0.kind() {
            ...
            BoxedNanKind::PtrA => VMut::String(unsafe {
                NixString::string_ref_mut_from_raw(self.0.as_ptr_unchecked().cast())
            }),
...
mod tests {
    ...
    fn clone_string_value() {
        ...
        let s2 = s.clone();
        assert_eq!(s.to_str().unwrap(), s2.to_str().unwrap());
    }
