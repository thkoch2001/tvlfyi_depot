...
pub use builtin::{Builtin, BuiltinArgument, BuiltinResult};
...
impl Value {
    ...
    // TODO: we need to retain the thunkset, but it must become shareable ...
    ...
    pub async fn neo_coerce_to_string(
        self,
        co: GenCo,
        kind: CoercionKind,
    ...
    pub fn coerce_to_string(
        ...
        todo!("remove sync Value::coerce_to_string");
    }
    ...
    pub async fn neo_force(self, co: GenCo) -> Result<Value, ErrorKind> {
        if let Value::Thunk(thunk) = self {
            return thunk.neo_force(co).await;
        }
