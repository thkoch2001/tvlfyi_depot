    ...
    pub fn nix_eq(&self, other: &Self, vm: &mut VM) -> Result<bool, ErrorKind> {
        match (self, other) {
            ...
            (Value::Thunk(lhs), Value::Thunk(rhs)) => {
                ...
                // TODO: determine whether thunk comparison in nix can
                // short circuit on thunk equality (without forcing)

                // TODO: this cloning is done because there is a
                // potential issue with keeping borrows into both
                // thunks around while recursing, as they might
                // recurse themselves, leading to a borrow error when
                // they are later being forced.
