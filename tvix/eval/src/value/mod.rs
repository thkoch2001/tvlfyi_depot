...
impl Value {
    ...
    pub async fn force(self, co: &GenCo, _span: LightSpan) -> Result<Value, ErrorKind> {
    ...
    pub async fn force_owned_genco(self, co: GenCo, _span: LightSpan) -> Result<Value, ErrorKind> {
        if let Value::Thunk(thunk) = self {
            // TODO(amjoseph): use #[tailcall::mutual]
            return Ok(generators::request_force(&co, Value::Thunk(thunk)).await);
        }
