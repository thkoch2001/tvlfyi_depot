...
macro_rules! gen_is {
    ( $name:ident, $variant:pat ) => {
        #[must_use] pub fn $name(&self) -> bool {
            match self {
                $variant => true,
                Value::Thunk(thunk) => Self::$name(&thunk.value()),
                _ => false,
...
impl Value {
    ...
    #[must_use] pub fn attrs(attrs: NixAttrs) -> Self {
    ...
    pub async fn coerce_to_string_(
        ...
        loop {
            ...
            let coerced: Result<BString, _> = match (value, kind) {
                ...
                // strong coercions
                (Self::Null | Self::Bool(false), CoercionKind { strong: true, .. }) => Ok("".into()),
                ...
                val @
(Self::Closure(_) | Self::Builtin(_) | Self::Null | Self::Bool(_) |
Self::Integer(_) | Self::Float(_) | Self::List(_), _) => Err(ErrorKind::NotCoercibleToString {
                    from: val.0.type_of(),
                    kind,
                }),
                ...
                (Self::AttrNotFound | Self::Blueprint(_) | Self::DeferredUpvalue(_) |
Self::UnresolvedPath(_) | Self::Json(..) | Self::FinaliseRequest(_), _) => {
    ...
    #[must_use] pub fn type_of(&self) -> &'static str {
        match self {
            Self::Null => "null",
            Self::Bool(_) => "bool",
            Self::Integer(_) => "int",
    ...
    ///
    /// [`Thunk`]: Value::Thunk
    #[must_use] pub fn is_thunk(&self) -> bool {
    ...
    #[must_use] pub fn explain(&self) -> String {
        match self {
