...
//!
//! Nix language strings are arbitrary (not necessarily UTF-8!) sequences of bytes, and are
...
impl NixString {
    ...
    pub fn into_parts(self) -> (BString, Option<NixContext>) {
        (self.0, self.1)
    }

    pub fn from_parts(s: BString, ctx: Option<NixContext>) -> Self {
        Self(s, ctx)
    }
