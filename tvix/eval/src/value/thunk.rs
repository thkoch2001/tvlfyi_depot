...
impl Thunk {
    ...
    pub async fn force_(
        ...
        // thunks.  Previously, we kept a HashSet with all of these
        // thunks; however it turns out that isn't necessary!  We
        // need only keep a copy of the *first* thunk in the chain;
        // after forcing the whole chain we can then simply walk the
        // chain again, head-to-tail, updating as we go.  No
        // auxiliary set is required.
        //
        // The value `head_of_chain_of_thunks` below holds the head of
        // the chain of thunks that we are working on.
        //
