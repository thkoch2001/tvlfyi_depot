                ...
                match inner_repr {
                    ...
                    ThunkRepr::Native(native) => {
                        ...
                        return Ok(Trampoline {
                            ...
                            continuation: Some(Box::new(move |vm| {
                                Thunk::force_trampoline(vm, Value::Thunk(self_clone))
                                    .map_err(|kind| Error::new(kind, todo!("b/238")))
                            })),
                        });
                    }
        ...
        loop {
            if !self.is_suspended() {
                ...
                match *thunk {
                    ...
                    {
                        let inner_repr = todo!(); // inner_thunk.0.borrow().clone();
                        drop(thunk);
                ...
                match self.0.replace(ThunkRepr::Blackhole) {
                    ThunkRepr::Suspended {
                        ...
                        // Construct a trampoline that will be executed by the
                        // VM. This trampoline enters the frame of the contained
                        // thunk and, after the VM has run it to completion,
                        // populates the thunk with it.
                        return Ok(Trampoline {
                            // Ask the VM to enter the suspended code ...
                            action: Some(TrampolineAction::EnterFrame {
                                lambda,
                                upvalues,
                                arg_count: 0,
                            ...
                            // ... and handle the result which will be left on
                            // the stack afterwards.
                            continuation: Some(Box::new(move |vm| {
                                let should_be_blackhole =
                                    self_clone.0.replace(ThunkRepr::Evaluated(vm.pop()));
                                assert!(matches!(should_be_blackhole, ThunkRepr::Blackhole));
                                Self::force_trampoline(vm, Value::Thunk(self_clone))
                                    .map_err(|kind| Error::new(kind, light_span.span()))
                            })),
                        });
                    }
