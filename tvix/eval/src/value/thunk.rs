...
/// Every reference cycles involving `Value`s will contain at least
/// one `Thunk`.
#[derive(Clone, Debug, PartialEq)]
...
impl Thunk {
    ...
    /// a non-suspended value is returned.
    ///
    ...
    pub fn force(&self, vm: &mut VM) -> Result<(), ErrorKind> {
        loop {
            ...
            match *thunk_mut {
                ...
                // and the [`OpFinalize`] branch of [`VM::run_op()`].
    ...
    pub fn is_unforced_recursive_closure(&self) -> bool {
        match self.0.as_ref().borrow().deref() {
            ThunkRepr::RecursiveClosure(_) => true,
            _ => false,
        }
    }
    ...
    pub fn upvalues_mut(&self) -> RefMut<'_, Upvalues> {
        RefMut::map(self.0.borrow_mut(), |thunk| match thunk {
            ThunkRepr::Suspended { upvalues, .. } => upvalues,
            ThunkRepr::RecursiveClosure(Closure { upvalues, .. }) => upvalues,
            ThunkRepr::Blackhole => panic!("upvalues() on a blackhole"),
            ThunkRepr::Evaluated(v) => panic!("upvalues() on already-evaluated thunk {v}"),
        })
    }
}

impl From<Closure> for Thunk {
