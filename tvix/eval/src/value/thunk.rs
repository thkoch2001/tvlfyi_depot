...
impl Thunk {
    ...
    /// Force a thunk from a context that can't handle trampolineoline
    /// continuations.  Calling `force_trampoline()` instead should be
    ...
    pub fn force_trampoline(vm: &mut VM) -> Result<Trampoline, ErrorKind> {
        match vm.pop() {
            Value::Thunk(thunk) /*if !thunk.is_forced()*/ =>
                return thunk.force_trampoline_self(vm),
            v => {
                vm.push(v);
