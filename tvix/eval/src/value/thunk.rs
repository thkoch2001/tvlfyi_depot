...
impl Thunk {
    ...
    pub fn upvalues_mut(&self) -> RefMut<'_, Upvalues> {
        RefMut::map(self.0.borrow_mut(), |thunk| match thunk {
            ...
            ThunkRepr::Evaluated(Value::Closure(c)) => {
                Rc::get_mut(&mut Rc::get_mut(c).unwrap().upvalues)
    ...
    pub(crate) fn ptr_eq(&self, other: &Self, vm:&mut VM, check:bool) -> bool {
        if Rc::ptr_eq(&self.0, &other.0) {
            return true;
        }
        match &*self.0.borrow() {
            ThunkRepr::Evaluated(Value::Closure(c1)) =>
                match &*other.0.borrow() {
                    ThunkRepr::Evaluated(Value::Closure(c2)) => Rc::ptr_eq(c1, c1),
                    _ => false,
                }
            _ => false,
        }
