...
mod pure_builtins {
    ...
    #[builtin("placeholder")]
    async fn builtin_placeholder(_: GenCo, #[lazy] _x: Value) -> Result<Value, ErrorKind> {
    ...
    async fn builtin_to_path(co: GenCo, s: Value) -> Result<Value, ErrorKind> {
        ...
        Ok(path.neo_coerce_to_string(co, CoercionKind::Weak).await?)
    }
...
async fn bgc_insert_key(co: &GenCo, key: Value, done: &mut Vec<Value>) -> Result<bool, ErrorKind> {
    for existing in done.iter() {
        if generators::check_equality(co, existing.clone(), key.clone()).await? {
            return Ok(false);
        }
    }
