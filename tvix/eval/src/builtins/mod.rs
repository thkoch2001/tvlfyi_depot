        ...
        Builtin::new(
            ...
            |args: Vec<Value>, vm: &mut VM| {
                ...
                let attrs = attrs.update(NixAttrs::from_map(BTreeMap::from([
                    ("allowedRequisites".into(), Value::Null),
                ])));
                ...
                let outputs = attrs.select("outputs").unwrap_or(&Value::from(vec![Value::from("out")])).clone().force(vm)?.to_list()?.clone();
                ...
                let attrs = attrs.update(NixAttrs::from_map(BTreeMap::from([
                    ("drvAttrs".into(), Value::Attrs(Rc::new(attrs_orig))),
                ])));
