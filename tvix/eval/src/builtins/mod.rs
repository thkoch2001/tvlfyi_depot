...
fn placeholders() -> Vec<Builtin> {
    vec![
        ...
        Builtin::new("derivation", &[true], |args: Vec<Value>, vm: &mut VM| {
            ...
            // We do not implement derivations yet, so this function
            // sets mock values on the fields that a real derivation
            // would contain.
            //
            // Crucially this means we do not yet *validate* the
            // values either.
            let attrs = unwrap_or_clone_rc(args[0].to_attrs()?);
            let attrs = attrs.update(NixAttrs::from_map(BTreeMap::from([
                ("outPath".into(), "/nix/store/mock-0000000".into()),
                ("drvPath".into(), "/nix/store/mock-0000000.drv".into()),
