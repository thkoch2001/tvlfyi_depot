...
use builtin_macros::{builtin, builtins};
use regex::Regex;
...
fn builtin_foldl(vm: &mut VM, op: Value, mut nul: Value, list: Value) -> Result<Value, ErrorKind> {
    let list = list.to_list()?;
    for val in list {
...
fn builtin_substring(_: &mut VM, start: Value, len: Value, s: Value) -> Result<Value, ErrorKind> {
