...
mod pure_builtins {
    ...
    async fn builtin_abort(co: GenCo, message: Value) -> Result<Value, ErrorKind> {
        ...
        // We use contextful string here because we do not care about the context explicitly.
            ...
            let elements: Vec<(String, Value)> = context
                ...
                .map(|ctx_element| match ctx_element {
                    NixContextElement::Plain(spath) => (
                        spath.clone(),
                        Value::attrs(NixAttrs::from_kv("path".into(), true.into())),
                    ),
                    NixContextElement::Single { name, derivation } => (
                        derivation.clone(),
                        Value::attrs(NixAttrs::from_kv(
                            "outputs".into(),
                    ...
                    NixContextElement::SelfReference => (
                        s.as_str().to_string(),
                        Value::attrs(NixAttrs::from_kv("allOutputs".into(), true.into())),
                    ),
                })
                .collect();
    ...
    async fn builtin_replace_strings(
        ...
        let string = s.to_contextful_str()?;

        let mut res = String::new();

        let mut i: usize = 0;
        let mut empty_string_replace = false;
        let mut context = string.context().cloned();
            ...
            for elem in std::iter::zip(from.iter(), to.iter()) {
                ...
                if &string[i..i + from.len()] == from.as_str() {
                    ...
                    if let Some(ref mut ctx) = context {
                        if let Some(other_ctx) = to.context_mut() {
                            ctx.merge(other_ctx);
                        }
                    } else {
                        context = to.context().cloned();
                    }
        ...
        Ok(Value::String((res, context).into()))
    }
