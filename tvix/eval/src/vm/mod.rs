...
use crate::{
    ...
    upvalues::Upvalues,
    value::{
        Builtin, BuiltinResult, Closure, CoercionKind, Lambda, NixAttrs, NixList, Thunk, Value,
    },
    vm::generators::GenCo,
...
impl<'o> VM<'o> {
        ...
        while let Some(frame) = self.frames.pop() {
            match frame {
                Frame::CallFrame { call_frame, span } => {
                    ...
                    self.observer
                        .observe_enter_frame(0, &call_frame.lambda, frame_id);
                    self.execute_bytecode(span, frame_id, call_frame)?;
                }
    ...
    fn execute_bytecode(&mut self, span: LightSpan, frame_id: usize, mut frame: CallFrame) -> EvalResult<()> {
        loop {
            let op = frame.inc_ip();
            ...
            match op {
                ...
                OpCode::OpReturn => {
                    self.observer.observe_exit_frame(frame_id, &self.stack);
                    return Ok(());
                }
                ...
                OpCode::OpCoerceToString => {
                    ...
                    let gen_frame = Frame::Generator {
                        ...
                        generator: Gen::new(|co| {
                            pin_generator(value.neo_coerce_to_string(co, CoercionKind::Weak))
                        }),
                    };
                ...
                OpCode::OpForce => {
                    if let Some(Value::Thunk(_)) = self.stack.last() {
                        ...
                        let gen_frame = Frame::Generator {
                            span: frame.current_light_span(),
                            state: GeneratorState::Running,
                            generator: Gen::new(|co| pin_generator(thunk.neo_force(co))),
                        };
    ...
    fn tail_call_value(
        ...
        match callable {
            ...
            Value::Closure(closure) => {
                ...
                // With an explicit parent frame (i.e. when tail-calling from
                // bytecode), the stack offset of the parent frame is used as
                // that frame is essentially replaced.
                //
                // Otherwise, the stack offset is taken from the last known call
                // frame. If no frame is present at all, the offset is 0.
                let stack_offset = parent
                    .as_ref()
                    .or_else(|| self.last_call_frame())
            ...
            Value::Attrs(ref attrs) => match attrs.select("__functor") {
                None => todo!(), // Err(self.error(ErrorKind::NotCallable(callable.type_of()))),
                Some(_functor) => {
            ...
            _ => Err(
                todo!(), /*parent.error(ErrorKind::NotCallable(callable.type_of())) */
            ),
        }
    }
...
pub fn run_lambda(
    ...
    vm.frames.push(Frame::Generator {
        span: root_span.into(),
        state: GeneratorState::Running,
        generator: Gen::new(|co| pin_generator(final_deep_force(co))),
    });
