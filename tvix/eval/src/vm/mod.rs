...
use generators::{call_functor, neo_resolve_with, Generator, GeneratorState};
        ...
        loop {
            ...
            match op {
                ...
                OpCode::OpEqual => {
                    ...
                    let gen_span = frame.current_light_span();
                    self.push_call_frame(span, frame);
                    self.enqueue_generator(gen_span, |co| a.nix_eq(b, co, true));
                    return Ok(false);
                }
                ...
                OpCode::OpCoerceToString => {
                    ...
                    self.enqueue_generator(gen_span, |co| {
                        value.neo_coerce_to_string(co, CoercionKind::Weak)
                    });
                ...
                OpCode::OpForce => {
                    if let Some(Value::Thunk(_)) = self.stack.last() {
                        ...
                        self.push_call_frame(span, frame);
                        self.enqueue_generator(gen_span, |co| thunk.neo_force(co));
                        return Ok(false);
                    }
                }
...
pub fn run_lambda(
    ...
    vm.enqueue_generator(root_span.into(), |co| final_deep_force(co));
