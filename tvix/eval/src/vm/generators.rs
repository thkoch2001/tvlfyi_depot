...
pub async fn force_value(co: &GenCo, val: Value) -> Value {
...
// /// "Weakly" coerce a Nix value to a string, only coercing already "stringly"
// /// types like strings and paths, but also attribute sets with a `__toString`
// /// attribute.
// ///
// /// The semantics mirror `!coerceMore` in C++ Nix.
    ...
    let result = match (a, b) {
        ...
        (Value::Attrs(a1), Value::Attrs(a2)) => {
            ...
            match (a1.select("type"), a2.select("type")) {
                (Some(v1), Some(v2)) => {
                    let s1 = force_value(&co, v1.clone()).await.to_str();
                    let s2 = force_value(&co, v1.clone()).await.to_str();
                    ...
                    if let (Ok(s1), Ok(s2)) = (s1, s2) {
                        if s1.as_str() == "derivation" && s2.as_str() == "derivation" {
                            ...
                            let result = force_value(&co, out1.clone()).await.to_str()?
                                == force_value(&co, out2.clone()).await.to_str()?;
