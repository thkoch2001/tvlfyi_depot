...
pub enum GeneratorRequest {
    ...
    NixEquality(Value, Value, PointerEquality),
...
impl Display for GeneratorRequest {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GeneratorRequest::NixEquality(v1, v2, ptr_eq) => {
                write!(f, "nix_eq({}, {}, PointerEquality::{:?})", v1, v2, ptr_eq)
            }
            ...
            match generator.resume_with(message) {
                ...
                genawaiter::GeneratorState::Yielded(request) => {
                    ...
                    match request {
                        ...
                        GeneratorRequest::NixEquality(a, b, ptr_eq) => {
                            self.reenqueue_generator(span.clone(), generator);
                            self.enqueue_generator(span, |co| a.nix_eq(b, co, ptr_eq));
                            return Ok(false);
                        }
...
pub(crate) async fn check_equality(
    ...
    match co.yield_(GeneratorRequest::NixEquality(a, b, ptr_eq)).await {
