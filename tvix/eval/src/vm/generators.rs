...
impl Display for GeneratorRequest {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GeneratorRequest::NixEquality(v1, v2) => write!(f, "nix_eq({}, {})", v1, v2),
...
impl<'o> VM<'o> {
    ...
    pub(crate) fn run_generator(
        ...
        let mut message = match state {
            GeneratorState::Running => GeneratorResponse::Empty,

            // If control returned here, and the generator is
            // awaiting a value, send it the top of the stack.
            GeneratorState::AwaitingValue => GeneratorResponse::Value(self.stack_pop()),
        };
                ...
                genawaiter::GeneratorState::Yielded(request) => {
                    ...
                    match request {
                        ...
                        GeneratorRequest::ForceValue(value) => {
                            ...
                            self.enqueue_generator(span, |co| value.neo_force(co));
                            return Ok(false);
                        }
                        ...
                        GeneratorRequest::NixEquality(a, b) => {
                            self.reenqueue_generator(span.clone(), generator);
                            self.enqueue_generator(span, |co| a.nix_eq(b, co, false));
                            return Ok(false);
                        }

                        GeneratorRequest::StringCoerce(val, kind) => {
                            self.reenqueue_generator(span.clone(), generator);
                            self.enqueue_generator(span, |co| val.neo_coerce_to_string(co, kind));
                            return Ok(false);
                        }
...
pub(crate) async fn check_equality(co: &GenCo, a: Value, b: Value) -> Result<bool, ErrorKind> {
    match co.yield_(GeneratorRequest::NixEquality(a, b)).await {
