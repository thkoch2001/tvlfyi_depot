...
pub trait Syntax {
    ...
    type AttrNameStatic: AttrNameStatic<Self>;
    type AttrNameDynamic: AttrNameDynamic<Self>;
...
/// A static (i.e. not involving interpolation) element of an attribute path.
pub trait AttrNameStatic<Syntax>: Clone + ToSpan + Into<String> + AsRef<str>
where
...
/// A dynamic (i.e. `."${fred+bob}"`) element of an attribute path.
pub trait AttrNameDynamic<Syntax>: Clone + ToSpan
where
    Syntax: self::Syntax + ?Sized,
{
    fn new(expr: Syntax::Expr, span: Span) -> Self;
...
where
    Syntax: self::Syntax + ?Sized,
{
    AttrNameStatic(<Syntax as self::Syntax>::AttrNameStatic),
    AttrNameDynamic(<Syntax as self::Syntax>::AttrNameDynamic),
}
    ...
    Syntax: self::Syntax,
{
    fn clone(&self) -> Self {
        match self {
            AttrName::AttrNameStatic(attrname_static) => {
                AttrName::AttrNameStatic(attrname_static.clone())
            }
            AttrName::AttrNameDynamic(attrname_dynamic) => {
                AttrName::AttrNameDynamic(attrname_dynamic.clone())
            }
        }
    }
}
    ...
    Syntax: self::Syntax + ?Sized,
{
    fn from_cst(c: &Compiler, cst: rnix::ast::Attr) -> Self {
        ...
        if let Some(s) = expr_static_attr_str(&cst) {
            AttrName::AttrNameStatic(<Syntax as self::Syntax>::AttrNameStatic::new(
                s.into(),
                span,
            ))
        } else {
            ...
            AttrName::AttrNameDynamic(<Syntax as self::Syntax>::AttrNameDynamic::new(
                <Syntax as self::Syntax>::Expr::from_cst(c, expr),
                span,
            ))
        }
    }
    ...
    Syntax: syn::Syntax + ?Sized,
{
    fn span_for(&self, file: &codemap::File) -> Span {
        match self {
            AttrName::AttrNameStatic(attrname_static) => attrname_static.span_for(file),
            AttrName::AttrNameDynamic(attrname_dynamic) => attrname_dynamic.span_for(file),
        }
    }
}

/// Least common supertrait of `AttrNameStatic` and `AttrNameDynamic`
impl<Syntax> AttrName<Syntax>
where
    Syntax: self::Syntax,
{
    pub fn as_static(&self) -> Option<&<Syntax as self::Syntax>::AttrNameStatic> {
        match self {
            AttrName::AttrNameStatic(inner) => Some(inner),
            _ => None,
        }
    }
}
