...
/// This trait is a minor kludge to make Rust's painful syntax a bit less
/// painful.  You can think of it as the "back pointer" from each associated
/// type of a Syntax instance (i.e. from the type that implements `Ident`,
/// from the type that implements `Attr`, etc) to the common shared type that
/// implements `Syntax`.
///
/// Having `trait Attr : IsSyntax<Syntax>` instead of `trait Attr { type
/// Syntax : self::Syntax; }` lets us avoid long and hard-to-read `<<<X as Y> as Z>
/// as Q>` chains.
pub trait IsSyntax<Syntax>
where
