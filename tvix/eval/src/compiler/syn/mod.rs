...
where
    Syntax: syn::Syntax + ?Sized + 'static,
{
    ...
    Syntax: self::Syntax + ?Sized + 'static,
{
    ...
    fn attrs<'a>(&'a self) -> impl Iterator<Item = &'a AttrName<Syntax>>
    where
        Syntax: 'static;
    fn new(
        namespace: Option<Syntax::Expr>,
        attrs: impl Iterator<Item = AttrName<Syntax>>,
        span: Span,
    ) -> Self;
}
...
where
    Syntax: self::Syntax + ?Sized + 'static,
{
    fn inherits(&self, c: &Compiler) -> impl Iterator<Item = Syntax::Inherit>;
    fn attributes<'a>(&self, c: &'a Compiler) -> impl Iterator<Item = Syntax::AttrPathValue>;
}
