...
impl<N: rnix::ast::HasEntry> HasEntries<SyntaxImpl> for N {
    fn inherits(&self, c: &Compiler) -> impl Iterator<Item = InheritImpl> {
        rnix::ast::HasEntry::inherits(self).map(|inherit| InheritImpl::from_cst(c, inherit))
    }

    fn attributes<'a>(&self, c: &'a Compiler) -> impl Iterator<Item = AttrPathValueImpl> {
...
impl Inherit<SyntaxImpl> for InheritImpl {
    ...
    fn attrs<'a>(&'a self) -> impl Iterator<Item = &'a AttrName<SyntaxImpl>> {
        self.attrs.iter()
    }
    fn new(
        namespace: Option<<SyntaxImpl as Syntax>::Expr>,
        attrs: impl Iterator<Item = AttrName<SyntaxImpl>>,
        span: Span,
    ) -> Self {
        InheritImpl {
            namespace,
