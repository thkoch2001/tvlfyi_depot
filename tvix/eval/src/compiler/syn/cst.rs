...
impl FromCst<rnix::ast::Attrpath> for AttrPathImpl {
    fn from_cst(entry: rnix::ast::Attrpath) -> Self {
        let coll: Vec<_> = entry.attrs().map(<AttrNameImpl>::from_cst).collect();
...
impl FromCst<rnix::ast::Inherit> for InheritImpl {
    fn from_cst(entry: rnix::ast::Inherit) -> Self {
        ...
        let namespace = entry
            ...
            .map(ExprImpl::from_cst);
        let attrs: Vec<IdentImpl> = entry
            .attrs()
            .filter_map(|cst| Some(IdentImpl::Attr(cst)))
            .collect();
        InheritImpl {
            inherit_token,
            namespace,
...
impl Inherit<SyntaxImpl> for InheritImpl {
    ...
    fn attrs(&self) -> impl Iterator<Item = &<SyntaxImpl as Syntax>::Ident> {
        self.attrs.iter()
    }
    fn new(
...
impl HasEntries<SyntaxImpl> for LetInImpl {
    fn iter(&self) -> impl Iterator<Item = EntryImpl> {
        rnix::ast::HasEntry::entries(&self.cst).map(EntryImpl::from_cst)
    }
}
