...
///! would make the syn::Syntax API extremely awkward.  Two examples of why
///! this is the case:
///!
///! 1. rnix::ast types do not carry a Span, and in order to compute a Span
///!    from them you need a *mutable* reference to a Compiler, because the
///!    Compiler handles errors by mutating its own internal list of errors
///!    rather than returning a Result.  This is why FromCst::from_cst() needs
///!    a `&mut Compiler` argument.  That, in turn, is why none of the
///!    accessor methods (like LetIn::body()) can call FromCst -- because then
///!    those accessor methods would need to take a `&mut Compiler` argument,
///!    which seemed like a bad choice (although perhaps it isn't -- I'm open
///!    to reconsidering this).  Therefore anything that needs to produce a
///!    Span has to be done "up front" at FromCst::from_cst()-time, rather
///!    than lazily on-demand.
///!
///! 2. When implementing a trait method which returns `&Option<T>`, you can't
