        ...
        for tracked_formal in (&entries).into_iter() {
            ...
            match tracked_formal {
                TrackedFormal::WithDefault {
                    ...
                    // The tricky bit about compiling a formal argument with a
                    // default value is that the default may be a thunk that may
                    // depend on the value of other formal arguments, i.e. may
                    // need to be finalised. This finalisation can only happen
                    // if we are actually using the default value—otherwise
                    // OpFinalise will crash on an already finalised (or
                    // non-thunk) value.
                    //
                    // Thus we use an additional local to track whether we wound
                    // up defaulting or not. `FinaliseRequest(false)` indicates
                    // that we should not finalise, as we did not default.
                    //
                    // We are being wasteful with VM stack space in case of default
                    // expressions that don't end up needing to be finalised.
                    // Unfortunately we only know better after compiling the
                    // default expression, so avoiding unnecessary locals would
                    // mean we'd need to modify the chunk after the fact.
                ...
                match tracked_formal {
                    ...
                    TrackedFormal::WithDefault { finalise_request_idx, .. } => {
                        ...
                        let jump_over_finalise =
                            self.push_op(OpCode::OpJumpIfNoFinaliseRequest(JumpOffset(0)),
                                                                           pattern);
                        self.push_op(
