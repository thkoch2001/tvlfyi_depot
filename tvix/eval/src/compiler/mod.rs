...
impl Compiler<'_> {
    ...
    fn compile_param_pattern(&mut self, pattern: &ast::Pattern) -> Formals {
        ...
        for entry in pattern.pat_entries() {
            ...
            let needs_finalise_idx = entry.default().map(|default_expr| {
                let span = self.span_for(&default_expr);
                self.scope_mut().declare_phantom(span, false)
            });
            entries.push((idx, needs_finalise_idx, entry));
            arguments.insert(ident.into(), needs_finalise_idx.is_some());
        }
        ...
        for (idx, needs_finalise_idx, entry) in (&entries).into_iter() {
            ...
            if let Some(default_expr) = entry.default() {
                ...
                self.emit_constant(Value::Bool(false), &default_expr);
                ...
                self.emit_constant(Value::Bool(true), &default_expr);
            ...
            if let Some(fidx) = *needs_finalise_idx {
                self.scope_mut().mark_initialised(fidx);
            }
        }

        for (idx, needs_finalise_idx, _) in (&entries).into_iter() {
            if self.scope()[*idx].needs_finaliser {
                let needs_finalise_stack_idx =
                    self.scope().stack_index((*needs_finalise_idx).unwrap());
                let stack_idx = self.scope().stack_index(*idx);
                self.push_op(OpCode::OpGetLocal(needs_finalise_stack_idx), pattern);
                let jump_over_finalise =
                    self.push_op(OpCode::OpJumpIfFalse(JumpOffset(0)), pattern);
