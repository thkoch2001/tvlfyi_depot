...
impl Compiler<'_> {
    ...
    /// Doc-comments can appear in two places for functions
    ///
    /// ```nix
    /// # (1)
    /// {
    ...
    /// More precisely before the NODE_ATTRPATH_VALUE
    ///
    fn get_lambda_docs(&mut self, lambda: &ast::Expr) -> Option<ast::Comment> {
        match self.get_doc_comment(lambda.syntax()) {
            // Found in place (2)
            Some(doc) => Some(doc),
            // Look in place (1)
            None => match lambda.syntax().parent() {
                Some(ref node) => {
                    match_ast! {
                        match node {
        ...
        F: FnOnce(&mut Compiler, LocalIdx),
    {
        ...
        if let Some(lambda) = expr {
            match self.get_lambda_docs(lambda) {
                Some(d) => {
                    self.context_mut().lambda.doc = Some(SmolStr::from(d.doc_text()));
                }
                _ => (),
            };
        }
