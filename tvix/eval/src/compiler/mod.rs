...
pub(crate) type GlobalsMap = HashMap<&'static str, Rc<dyn Fn(&mut Compiler, rnix::ast::Ident)>>;

pub(crate) struct Compiler<'observer, 'globals> {
...
impl<'observer, 'globals> Compiler<'observer, 'globals> {
    pub(crate) fn new(
        ...
        file: Arc<codemap::File>,
        globals: &'globals HashMap<&'static str, Value>,
        observer: &'observer mut dyn Observer,
    ) -> EvalResult<Self> {
    ...
    pub fn compile_toplevel(&self, expr: &ast::Expr) -> EvalResult<Rc<Lambda>> {
