...
use super::syn::{
    Attr, AttrPath, AttrPathValue, Expr, FromCst, HasEntry, Inherit, StaticOrDynamic,
};
...
impl<Syntax> HasEntry for Binding<Syntax>
where
    Syntax: syn::Syntax,
{
    type Syntax = Syntax;
    ...
    Syntax: syn::Syntax,
{
        ...
        match self {
            ...
            Binding::Plain { expr } => match expr {
                ast::Expr::AttrSet(existing) => {
                    *self = Binding::from_cst_attrset(c, existing);
            ...
            // down.
            Binding::Set { entries, .. } if remaining_path.first().is_some() => entries.push(
                Syntax::AttrPathValue::new(span, remaining_path, Syntax::Expr::from_cst(c, value)),
            ),
            ...
            Binding::Set {
                ...
                if let ast::Expr::AttrSet(new) = value {
                    inherits.extend(
                        ast::HasEntry::inherits(&new)
                            .map(|entry| Syntax::Inherit::from_cst(c, entry)),
                    );
                    entries.extend(ast::HasEntry::attrpath_values(&new).map(|entry| {
                        let span = c.span_for(&entry);
                        Syntax::AttrPathValue::from_cst(c, entry)
                    }));
                } else {
    ...
    fn from_cst_attrset(c: &Compiler, node: &ast::AttrSet) -> Self {
        Binding::Set {
            ...
            inherits: ast::HasEntry::inherits(node)
                .map(|inherit| Syntax::Inherit::from_cst(c, inherit))
                .collect(),
    ...
    Syntax: syn::Syntax,
{
    ...
    fn try_merge(
        ...
        if remaining_path.first().is_none() && !matches!(value, ast::Expr::AttrSet(_)) {
            return false;
        }
...
impl Compiler<'_, '_> {
    ...
    where
        Syntax: syn::Syntax,
        N: ToSpan + HasEntry<Syntax = Syntax>,
    {
        ...
        for entry in node.attributes(self) {
            ...
            *count += 1;
            ...
            let key_slot = match key.attr() {
                StaticOrDynamic::Static(name) if kind.is_attrs() => KeySlot::Attr {
                    attr: key,
                    slot: self.scope_mut().declare_phantom(key_span, false),
                },

                StaticOrDynamic::Static(name) => KeySlot::None {
                    name: name.clone(),
                },

                StaticOrDynamic::Dynamic(dynamic) if kind.is_attrs() => KeySlot::Attr {
                    attr: key,
                    slot: self.scope_mut().declare_phantom(key_span, false),
                },
    ...
    pub(super) fn compile_attr_set(&mut self, slot: LocalIdx, node: &ast::AttrSet) {
        ...
        self.compile_bindings::<_, syn::cst::SyntaxImpl>(slot, kind, node);
            ...
            match binding.key_slot {
                ...
                KeySlot::Attr { slot, attr } => match attr.attr() {
                    ...
                    StaticOrDynamic::Dynamic(expr) => {
                        let expr: rnix::ast::Expr = expr.clone().to_cst();
    ...
    pub(super) fn compile_let_in(&mut self, slot: LocalIdx, node: &ast::LetIn) {
        self.compile_bindings::<_, syn::cst::SyntaxImpl>(slot, BindingsKind::LetIn, node);
    ...
    pub(super) fn compile_legacy_let(&mut self, slot: LocalIdx, node: &ast::LegacyLet) {
        ...
        self.scope_mut().begin_scope();
        self.compile_bindings::<_, syn::cst::SyntaxImpl>(slot, BindingsKind::RecAttrs, node);
