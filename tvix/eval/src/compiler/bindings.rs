...
{
    fn into_iter(&self) -> impl Iterator<Item = Syntax::Entry> {
        (match self {
            Binding::Set { entries, .. } => {
                Some(entries.clone().into_iter().map(|entries| entries.into()))
            }
            _ => None,
        })
        .into_iter()
    ...
    Syntax: syn::Syntax,
{
        ...
        match self {
            ...
            Binding::Set { entries, .. } => {
                if let Some(attrset) = value.as_attrset() {
                    entries.extend(attrset.into_iter());
                } else {
    ...
    fn from_attrset(c: &mut Compiler, attrset: Syntax::AttrSet) -> Self {
        Binding::Set {
            ...
            entries: attrset.into_iter().collect(),
        }
    }
}
    ...
    Syntax: syn::Syntax + 'static,
{
    ...
    fn try_merge(
        ...
        binding
            .binding
            .merge(c, span, Syntax::AttrPath::from_iter(remaining_path), value);
...
impl Compiler<'_, '_> {
        ...
        N: ToSpan + HasEntries<Syntax>,
    {
            ...
            .into_iter()
        {
            ...
            if bindings.try_merge(self, span, &path, value.clone()) {
