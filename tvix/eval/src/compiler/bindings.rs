...
{
    fn inherits(&self, _c: &Compiler) -> impl Iterator<Item = Syntax::Inherit> {
        (match self {
    ...
    fn attributes<'a>(&self, _c: &'a Compiler) -> impl Iterator<Item = Syntax::AttrPathValue> {
        (match self {
                ...
                None => {
                    for attr in inherit.attrs() {
                        ...
                        *count += 1;
                        ...
                        if kind.is_attrs() {
                            let name: String = name.clone().ident().to_string();
                        ...
                        self.compile_identifier_access(slot, name.ident(), attr);
                        ...
                        let idx = if kind == BindingsKind::Attrs {
                            ...
                            self.declare_local(attr, name.ident())
                        };
                    ...
                    for attr in inherit.attrs() {
                        ...
                        *count += 1;
                        inherit_froms.push(Syntax::Inherit::new(
                            Some(from.clone()),
                            Some(AttrName::Static(Syntax::Ident::new(
        ...
        for inherit in inherit_froms {
            ...
            for attr in inherit.attrs() {
                ...
                let key_slot = if kind.is_attrs() {
                    ...
                    KeySlot::None {
                        name: attr.as_static().unwrap().ident().into(),
                    }
                };
                ...
                let value_slot = match kind {
                    ...
                    BindingsKind::LetIn | BindingsKind::RecAttrs => {
                        self.declare_local(&span, attr.as_static().unwrap().ident())
                    }
        ...
        for entry in node.attributes(self).collect::<Vec<_>>().iter() {
            ...
            *count += 1;
            ...
            let key_slot = match key {
                AttrName::Static(ref _name) if kind.is_attrs() => KeySlot::Attr {
                    attr: key,
                    slot: self.scope_mut().declare_phantom(key_span, false),
                },

                AttrName::Static(ref name) => KeySlot::None {
                    name: name.ident().into(),
                },

                AttrName::Dynamic(ref _dynamic) if kind.is_attrs() => KeySlot::Attr {
                    attr: key,
                    slot: self.scope_mut().declare_phantom(key_span, false),
                },
