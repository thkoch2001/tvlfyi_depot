...
use super::syn::{
    AttrName, AttrPath, AttrPathValue, AttrSet, Entry, Expr, HasEntries, Ident, Inherit, LetIn,
};
    ...
    Syntax: syn::Syntax,
{
    ...
    fn merge(
        &mut self,
        c: &mut Compiler,
        remaining_path: <Syntax as syn::Syntax>::AttrPath,
        value: Syntax::Expr,
    ) {
        match self {
            ...
            // and recurse.
            Binding::Plain { expr } => {
                if let Some(existing) = expr.clone().as_attrset() {
                    *self = Binding::from_attrset(c, existing.clone());
                    self.merge(c, remaining_path, value);
                } else {
            ...
            Binding::Set { entries, .. } if remaining_path.first().is_some() => {
                entries.push(Syntax::AttrPathValue::new(remaining_path, value).into())
            }

            Binding::Set { entries, .. } => {
                if let Some(attrset) = value.as_attrset() {
                    entries.extend(attrset.iter());
                } else {
    ...
    Syntax: syn::Syntax + 'static,
{
    ...
    fn try_merge(
        ...
        let mut remaining_path = remaining_path.clone().into_iter().peekable();
        let attr = remaining_path.next().unwrap();

        // If the path has no more entries, and if the entry is not an
        // attribute set literal, the entry can not be merged.
        if remaining_path.peek().is_none() && value.as_attrset().is_none() {
            return false;
        }
        ...
        binding
            .binding
            .merge(c, Syntax::AttrPath::from_iter(remaining_path), value);
...
impl Compiler<'_, '_> {
        ...
        N: ToSpan + HasEntries<Syntax>,
    {
        ...
        for inherit in node
            .iter()
            .filter_map(|e| e.as_inherit().cloned())
            .collect::<Vec<_>>()
            .iter()
        {
                    ...
                    for name in inherit.attrs() {
                        ...
                        *count += 1;
                        ...
                        if kind.is_attrs() {
                            ...
                            self.emit_constant(name.ident().to_string().into(), &span);
                        ...
                        let span = self.span_for(inherit);
                        self.compile_identifier_access(slot, &name.ident(), &span);
        ...
        N: ToSpan + HasEntries<Syntax>,
    {
        for entry in node
            .iter()
            .filter_map(|e| e.as_attr_path_value().cloned())
            .collect::<Vec<_>>()
            .into_iter()
        {
            let span = entry.span_for(self.file);
            let path = entry.path().clone();
            let value = entry.into_value();

            if bindings.try_merge(self, &path, value.clone()) {
                // Binding is nested, or already exists and was merged, move on.
                continue;
            }

            let mut remaining_path = path.clone().into_iter().peekable();
            let key = remaining_path.next().unwrap();
            *count += 1;

            let key_span = self.span_for(&key);
            let key_slot = if let Some(name) = key.as_static() {
                if kind.is_attrs() {
                    KeySlot::Attr {
                        attr: key,
            ...
            let binding = if remaining_path.peek().is_some() {
                Binding::Set {
                    span,
                    kind: BindingsKind::Attrs,
                    entries: vec![Syntax::AttrPathValue::new(
                        Syntax::AttrPath::from_iter(remaining_path),
                        value,
                    )
                    .into()],
                }
                ...
                Binding::Inherit(inherit_from) => {
                    if let Some(namespace) = inherit_from.namespace() {
                        ...
                        self.thunk(binding.value_slot, namespace, |c, s| {
                            ...
                            let name = attrs.next().unwrap().clone().ident().to_string().into();
