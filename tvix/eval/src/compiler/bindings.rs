...
use super::syn::{
    AttrName, AttrNameDynamic, AttrNameStatic, AttrPath, AttrPathValue, AttrSet, Expr, HasEntries,
    Ident, Inherit, LetIn,
};
    ...
    Syntax: syn::Syntax,
{
    ...
    fn matches(&self, key: &str) -> bool {
        match &self.key_slot {
            ...
            KeySlot::Attr { slot: _, attr } => match attr {
                AttrName::AttrNameStatic(s) => {
    ...
    Syntax: syn::Syntax,
{
    ...
    fn try_merge(
        ...
        // can not be merged.
        let name = match attr {
            AttrName::AttrNameStatic(name) => name,
            _ => return false,
        };
        ...
        // `find` here
        let binding = match self.bindings.iter_mut().find(|b| b.matches(name.as_ref())) {
            Some(b) => b,
            None => return false,
        };
            ...
            match inherit.namespace() {
                ...
                None => {
                    for attr in inherit.attrs().iter() {
                        ...
                        if kind == BindingsKind::LetIn
                            && matches!(
                                self.scope_mut().resolve_local(name.as_ref()),
                        ...
                        *count += 1;
                        ...
                        if kind.is_attrs() {
                            ...
                            self.emit_constant(name.into(), attr);
                        ...
                        self.compile_identifier_access(slot, name.as_ref(), attr);
                ...
                Some(from) => {
                    for attr in inherit.attrs() {
                        let name = match attr {
                            AttrName::AttrNameStatic(ref name) => name,
                            _ => {
                                self.emit_error(&attr, ErrorKind::DynamicKeyInScope("inherit"));
                                continue;
                            }
                        ...
                        *count += 1;
                        inherit_froms.push(Syntax::Inherit::new(
                            Some(from.clone()),
                            vec![AttrName::AttrNameStatic(Syntax::AttrNameStatic::new(
        ...
        for inherit in inherit_froms {
            ...
            for attr in inherit.attrs() {
                ...
                let key_slot = if kind.is_attrs() {
                    ...
                    KeySlot::None {
                        name: attr.as_static().unwrap().as_ref().into(),
                    }
                };
                ...
                let value_slot = match kind {
                    ...
                    BindingsKind::LetIn | BindingsKind::RecAttrs => {
                        self.declare_local(&span, attr.as_static().unwrap().clone())
                    }
        ...
        for entry in node.attributes(self) {
            ...
            *count += 1;
            ...
            let key_slot = match key {
                AttrName::AttrNameStatic(ref _name) if kind.is_attrs() => KeySlot::Attr {
                    attr: key,
                    slot: self.scope_mut().declare_phantom(key_span, false),
                },

                AttrName::AttrNameStatic(ref name) => KeySlot::None {
                    name: name.as_ref().into(),
                },

                AttrName::AttrNameDynamic(ref _dynamic) if kind.is_attrs() => KeySlot::Attr {
                    attr: key,
                    slot: self.scope_mut().declare_phantom(key_span, false),
                },
                ...
                BindingsKind::LetIn | BindingsKind::RecAttrs => match &key_slot {
                    ...
                    KeySlot::Attr { slot: _, attr } => {
                        if let Some(name) = attr.as_static() {
                            self.declare_local(&key_span, name.as_ref())
                        } else {
        ...
        Syntax: syn::Syntax,
    {
        ...
        for binding in bindings.bindings.into_iter() {
            ...
            match binding.key_slot {
                ...
                KeySlot::Attr { slot, attr } => match attr {
                    AttrName::AttrNameStatic(name) => {
                        let span = self.scope()[slot].span;
                        self.emit_constant(name.as_ref().into(), &span);
                        self.scope_mut().mark_initialised(slot);
                    }
                    AttrName::AttrNameDynamic(expr) => {
