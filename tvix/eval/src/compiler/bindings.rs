...
{
    fn into_iter(&self, _c: &mut Compiler) -> impl Iterator<Item = Syntax::Entry> {
        (match self {
            Binding::Set { entries, .. } => {
    ...
    Syntax: syn::Syntax,
{
    ...
    fn merge(
        ...
        match self {
            ...
            Binding::Set { entries, .. } if remaining_path.first().is_some() => {
                entries.push(Syntax::AttrPathValue::new(span, remaining_path, value).into())
            }

            Binding::Set { entries, .. } => {
                if let Some(attrset) = value.as_attrset() {
                    entries.extend(attrset.into_iter(c));
                } else {
    ...
    fn from_attrset(c: &mut Compiler, attrset: Syntax::AttrSet) -> Self {
        Binding::Set {
            ...
            entries: attrset.into_iter(c).collect(),
        }
    }
}
    ...
    Syntax: syn::Syntax + 'static,
{
    ...
    fn try_merge(
        ...
        let mut remaining_path = remaining_path.into_iter().peekable();
...
impl Compiler<'_, '_> {
        ...
        N: ToSpan + HasEntries<Syntax>,
    {
            ...
            .iter()
        {
            ...
            if bindings.try_merge(self, entry.span_for(self.file), entry.path(), value.clone()) {
                // Binding is nested, or already exists and was merged, move on.
                continue;
            }

            let mut remaining_path = entry.path().into_iter().peekable();
