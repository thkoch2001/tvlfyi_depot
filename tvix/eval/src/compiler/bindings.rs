...
impl Binding {
    ...
    fn merge(&mut self, c: &mut Compiler, span: Span, mut path: PeekableAttrs, value: ast::Expr) {
        match self {
            ...
            // down.
            Binding::Set(existing) if path.peek().is_some() => {
                existing.entries.push((span, path, value))
            }
...
impl TrackedBindings {
    ...
    fn try_merge(
        ...
        binding.binding.merge(c, span, path, value);
