// SPDX-License-Identifier: MIT
// Copyright © 2022 The Tvix Authors

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.32.0
// 	protoc        (unknown)
// source: tvix/store/protos/rpc_pathinfo.proto

package storev1

import (
	castore_go "code.tvl.fyi/tvix/castore-go"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// The parameters that can be used to lookup a (single) PathInfo object.
// Currently, only a lookup by output hash is supported.
type GetPathInfoRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Types that are assignable to ByWhat:
	//
	//	*GetPathInfoRequest_ByOutputHash
	ByWhat isGetPathInfoRequest_ByWhat `protobuf_oneof:"by_what"`
}

func (x *GetPathInfoRequest) Reset() {
	*x = GetPathInfoRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetPathInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPathInfoRequest) ProtoMessage() {}

func (x *GetPathInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPathInfoRequest.ProtoReflect.Descriptor instead.
func (*GetPathInfoRequest) Descriptor() ([]byte, []int) {
	return file_tvix_store_protos_rpc_pathinfo_proto_rawDescGZIP(), []int{0}
}

func (m *GetPathInfoRequest) GetByWhat() isGetPathInfoRequest_ByWhat {
	if m != nil {
		return m.ByWhat
	}
	return nil
}

func (x *GetPathInfoRequest) GetByOutputHash() []byte {
	if x, ok := x.GetByWhat().(*GetPathInfoRequest_ByOutputHash); ok {
		return x.ByOutputHash
	}
	return nil
}

type isGetPathInfoRequest_ByWhat interface {
	isGetPathInfoRequest_ByWhat()
}

type GetPathInfoRequest_ByOutputHash struct {
	// The output hash of a nix path (20 bytes).
	// This is the nixbase32-decoded portion of a Nix output path, so to substitute
	// /nix/store/xm35nga2g20mz5sm5l6n8v3bdm86yj83-cowsay-3.04
	// this field would contain nixbase32dec("xm35nga2g20mz5sm5l6n8v3bdm86yj83").
	ByOutputHash []byte `protobuf:"bytes,1,opt,name=by_output_hash,json=byOutputHash,proto3,oneof"`
}

func (*GetPathInfoRequest_ByOutputHash) isGetPathInfoRequest_ByWhat() {}

// The parameters that can be used to lookup (multiple) PathInfo objects.
// Currently no filtering is possible, all objects are returned.
type ListPathInfoRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *ListPathInfoRequest) Reset() {
	*x = ListPathInfoRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListPathInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPathInfoRequest) ProtoMessage() {}

func (x *ListPathInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPathInfoRequest.ProtoReflect.Descriptor instead.
func (*ListPathInfoRequest) Descriptor() ([]byte, []int) {
	return file_tvix_store_protos_rpc_pathinfo_proto_rawDescGZIP(), []int{1}
}

// Message type to specify the parameters for the CalculateDigest request.
// Only the NAR_* and FLAT_* enum kinds are supported for "type".
type CalculateDigestRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The hashing type to use.
	Type NARInfo_CA_Hash `protobuf:"varint,1,opt,name=type,proto3,enum=tvix.store.v1.NARInfo_CA_Hash" json:"type,omitempty"`
	// Types that are assignable to Source:
	//
	//	*CalculateDigestRequest_RootNode
	//	*CalculateDigestRequest_BlobDigest
	Source isCalculateDigestRequest_Source `protobuf_oneof:"source"`
}

func (x *CalculateDigestRequest) Reset() {
	*x = CalculateDigestRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CalculateDigestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalculateDigestRequest) ProtoMessage() {}

func (x *CalculateDigestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalculateDigestRequest.ProtoReflect.Descriptor instead.
func (*CalculateDigestRequest) Descriptor() ([]byte, []int) {
	return file_tvix_store_protos_rpc_pathinfo_proto_rawDescGZIP(), []int{2}
}

func (x *CalculateDigestRequest) GetType() NARInfo_CA_Hash {
	if x != nil {
		return x.Type
	}
	return NARInfo_CA_NAR_SHA256
}

func (m *CalculateDigestRequest) GetSource() isCalculateDigestRequest_Source {
	if m != nil {
		return m.Source
	}
	return nil
}

func (x *CalculateDigestRequest) GetRootNode() *castore_go.Node {
	if x, ok := x.GetSource().(*CalculateDigestRequest_RootNode); ok {
		return x.RootNode
	}
	return nil
}

func (x *CalculateDigestRequest) GetBlobDigest() []byte {
	if x, ok := x.GetSource().(*CalculateDigestRequest_BlobDigest); ok {
		return x.BlobDigest
	}
	return nil
}

type isCalculateDigestRequest_Source interface {
	isCalculateDigestRequest_Source()
}

type CalculateDigestRequest_RootNode struct {
	// The root node identifying the contents (for NAR_* hashing type enum
	// kinds).
	// The name of the root_node is ignored.
	RootNode *castore_go.Node `protobuf:"bytes,2,opt,name=root_node,json=rootNode,proto3,oneof"`
}

type CalculateDigestRequest_BlobDigest struct {
	// The blake3 digest of the raw blob contents (for FLAT_* hashing type enum
	// kinds)
	BlobDigest []byte `protobuf:"bytes,3,opt,name=blob_digest,json=blobDigest,proto3,oneof"`
}

func (*CalculateDigestRequest_RootNode) isCalculateDigestRequest_Source() {}

func (*CalculateDigestRequest_BlobDigest) isCalculateDigestRequest_Source() {}

// CalculateDigestResponse is the response returned by the CalculateDigest
// request.
type CalculateDigestResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The calculated digest for the requested hashing type.
	Digest []byte `protobuf:"bytes,1,opt,name=digest,proto3" json:"digest,omitempty"`
	// The NAR size, in bytes. Only populated/nonzero if NAR_* hashing types are
	// used.
	NarSize uint64 `protobuf:"varint,2,opt,name=nar_size,json=narSize,proto3" json:"nar_size,omitempty"`
}

func (x *CalculateDigestResponse) Reset() {
	*x = CalculateDigestResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CalculateDigestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalculateDigestResponse) ProtoMessage() {}

func (x *CalculateDigestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalculateDigestResponse.ProtoReflect.Descriptor instead.
func (*CalculateDigestResponse) Descriptor() ([]byte, []int) {
	return file_tvix_store_protos_rpc_pathinfo_proto_rawDescGZIP(), []int{3}
}

func (x *CalculateDigestResponse) GetDigest() []byte {
	if x != nil {
		return x.Digest
	}
	return nil
}

func (x *CalculateDigestResponse) GetNarSize() uint64 {
	if x != nil {
		return x.NarSize
	}
	return 0
}

var File_tvix_store_protos_rpc_pathinfo_proto protoreflect.FileDescriptor

var file_tvix_store_protos_rpc_pathinfo_proto_rawDesc = []byte{
	,
}

var (
	file_tvix_store_protos_rpc_pathinfo_proto_rawDescOnce sync.Once
	file_tvix_store_protos_rpc_pathinfo_proto_rawDescData = file_tvix_store_protos_rpc_pathinfo_proto_rawDesc
)

func file_tvix_store_protos_rpc_pathinfo_proto_rawDescGZIP() []byte {
	file_tvix_store_protos_rpc_pathinfo_proto_rawDescOnce.Do(func() {
		file_tvix_store_protos_rpc_pathinfo_proto_rawDescData = protoimpl.X.CompressGZIP(file_tvix_store_protos_rpc_pathinfo_proto_rawDescData)
	})
	return file_tvix_store_protos_rpc_pathinfo_proto_rawDescData
}

var file_tvix_store_protos_rpc_pathinfo_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_tvix_store_protos_rpc_pathinfo_proto_goTypes = []interface{}{
	(*GetPathInfoRequest)(nil),      // 0: tvix.store.v1.GetPathInfoRequest
	(*ListPathInfoRequest)(nil),     // 1: tvix.store.v1.ListPathInfoRequest
	(*CalculateDigestRequest)(nil),  // 2: tvix.store.v1.CalculateDigestRequest
	(*CalculateDigestResponse)(nil), // 3: tvix.store.v1.CalculateDigestResponse
	(NARInfo_CA_Hash)(0),            // 4: tvix.store.v1.NARInfo.CA.Hash
	(*castore_go.Node)(nil),         // 5: tvix.castore.v1.Node
	(*PathInfo)(nil),                // 6: tvix.store.v1.PathInfo
}
var file_tvix_store_protos_rpc_pathinfo_proto_depIdxs = []int32{
	4, // 0: tvix.store.v1.CalculateDigestRequest.type:type_name -> tvix.store.v1.NARInfo.CA.Hash
	5, // 1: tvix.store.v1.CalculateDigestRequest.root_node:type_name -> tvix.castore.v1.Node
	0, // 2: tvix.store.v1.PathInfoService.Get:input_type -> tvix.store.v1.GetPathInfoRequest
	6, // 3: tvix.store.v1.PathInfoService.Put:input_type -> tvix.store.v1.PathInfo
	2, // 4: tvix.store.v1.PathInfoService.CalculateDigest:input_type -> tvix.store.v1.CalculateDigestRequest
	1, // 5: tvix.store.v1.PathInfoService.List:input_type -> tvix.store.v1.ListPathInfoRequest
	6, // 6: tvix.store.v1.PathInfoService.Get:output_type -> tvix.store.v1.PathInfo
	6, // 7: tvix.store.v1.PathInfoService.Put:output_type -> tvix.store.v1.PathInfo
	3, // 8: tvix.store.v1.PathInfoService.CalculateDigest:output_type -> tvix.store.v1.CalculateDigestResponse
	6, // 9: tvix.store.v1.PathInfoService.List:output_type -> tvix.store.v1.PathInfo
	6, // [6:10] is the sub-list for method output_type
	2, // [2:6] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_tvix_store_protos_rpc_pathinfo_proto_init() }
func file_tvix_store_protos_rpc_pathinfo_proto_init() {
	if File_tvix_store_protos_rpc_pathinfo_proto != nil {
		return
	}
	file_tvix_store_protos_pathinfo_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetPathInfoRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListPathInfoRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CalculateDigestRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CalculateDigestResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[0].OneofWrappers = []interface{}{
		(*GetPathInfoRequest_ByOutputHash)(nil),
	}
	file_tvix_store_protos_rpc_pathinfo_proto_msgTypes[2].OneofWrappers = []interface{}{
		(*CalculateDigestRequest_RootNode)(nil),
		(*CalculateDigestRequest_BlobDigest)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_tvix_store_protos_rpc_pathinfo_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_tvix_store_protos_rpc_pathinfo_proto_goTypes,
		DependencyIndexes: file_tvix_store_protos_rpc_pathinfo_proto_depIdxs,
		MessageInfos:      file_tvix_store_protos_rpc_pathinfo_proto_msgTypes,
	}.Build()
	File_tvix_store_protos_rpc_pathinfo_proto = out.File
	file_tvix_store_protos_rpc_pathinfo_proto_rawDesc = nil
	file_tvix_store_protos_rpc_pathinfo_proto_goTypes = nil
	file_tvix_store_protos_rpc_pathinfo_proto_depIdxs = nil
}
