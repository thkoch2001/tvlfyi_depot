...
// TODO: well ...
lazy_static! {
    pub static ref KNOWN_PATHS: Arc<RwLock<KnownPaths>> = Arc::new(RwLock::new(Default::default()));
}
...
mod derivation_builtins {
    ...
    fn builtin_derivation_strict(vm: &mut VM, input: Value) -> Result<Value, ErrorKind> {
        ...
        let mut refscan = KNOWN_PATHS.read().unwrap().reference_scanner();
        ...
        let mut known_paths = KNOWN_PATHS.write().unwrap();
        ...
        known_paths.add_replacement_string(derivation_path.clone(), &actual_replacement_str);
        ...
        known_paths.drv(derivation_path.clone(), &output_names);

        for (output_name, output) in &drv.outputs {
            known_paths.output(&output.path, output_name, &derivation_path);
        }
