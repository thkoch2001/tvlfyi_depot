...
mod tests {
    ...
    fn populate_inputs_all() {
        ...
        paths.plain("/nix/store/1234-foo");
        paths.drv("/nix/store/2345-bar.drv", &["out"]);
        paths.output("/nix/store/3456-bar", "out", "/nix/store/2345-bar.drv");

        let inputs: Vec<String> = vec![
            "/nix/store/1234-foo".into(),
            "/nix/store/2345-bar.drv".into(),
            "/nix/store/3456-bar".into(),
        ];

        populate_inputs(&mut drv, &paths, inputs);

        assert_eq!(drv.input_sources.len(), 1);
        assert!(drv.input_sources.contains("/nix/store/1234-foo"));

        assert_eq!(drv.input_derivations.len(), 1);
        assert!(drv
            .input_derivations
            .contains_key("/nix/store/2345-bar.drv"));
    }
}
