...
impl TvixStoreIO {
    ...
    // [tvix_store::proto::node::Node] specified by `sub_path`.
    ...
    /// [tvix_store::proto::PathInfo] describing the path, that was sent to
    /// [PathInfoService].
            ...
            StorePath::from_absolute_path_full(&path.to_string_lossy())
        {
            if let Some(node) = self.store_path_to_root_node(&store_path, &sub_path)? {
                match node {
                        ...
                        if let Some(directory) = self.directory_service.get(&digest)? {
                            ...
                            for node in directory.nodes() {
                                children.push(match node {
                                    tvix_store::proto::node::Node::Directory(e) => {
                                        (e.name, FileType::Directory)
                                    }
                                    tvix_store::proto::node::Node::File(e) => {
                                        (e.name, FileType::Regular)
                                    }
                                    tvix_store::proto::node::Node::Symlink(e) => {
                    ...
                    tvix_store::proto::node::Node::File(_file_node) => {
                    ...
                    tvix_store::proto::node::Node::Symlink(_symlink_node) => Err(io::Error::new(
                        io::ErrorKind::Unsupported,
