//! Simple scanner for non-overlapping, known references of Nix store
//! paths in a given string.
//!
//! This is used for determining build references (see
//! //tvix/eval/docs/build-references.md for more details).
//!
//! The scanner itself is an Aho-Corasick automaton, using the
//! `aho-corasick` crate.

use aho_corasick::AhoCorasick;
use std::collections::BTreeSet;

/// Represents a "primed" reference scanner with an automaton that
/// knows the set of store paths to scan for.
...
impl<'c, 's> ReferenceScanner<'c, 's> {
    /// Construct a new `ReferenceScanner` that knows how to scan for
    /// the given candidate store paths.
    ...
    /// Scan the given string for all non-overlapping matches and
    /// collect them in the scanner.
