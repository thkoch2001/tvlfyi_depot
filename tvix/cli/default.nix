...
let
  ...
  mkNixpkgsEvalTest = attrpath: expectedPath:
    let
      name = "tvix-cli-eval-test-${attrpath}";
    in
    (pkgs.writeShellScript "tvix-eval-${builtins.replaceStrings [".drv"] ["-drv"] attrpath}"
      ''
      ...
      (finalAttrs: previousAttrs: {
        passthru = (previousAttrs.passthru or { }) // {
          ci = {
            label = ":nix: evaluate nixpkgs.${attrpath} in tvix";
            needsOutput = true;
            command = "${finalAttrs.finalPackage}/bin/${finalAttrs.meta.mainProgram}";
          };
        };
...
{
  meta = {
    ci.extraSteps = (builtins.attrNames benchmarks) ++ (builtins.attrNames evalTests);
  };

  # Expose benchmarks and evalTests as standard CI targets.
  passthru.ci = benchmarks.passthru.ci // evalTests.passthru.ci;
})
