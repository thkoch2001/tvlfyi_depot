...
let
  ...
  mkNixpkgsEvalTest =
    ...
    , expectedPath # The expected store path that should match one of the above.
      ...
      let
        name = "tvix-eval-test-${builtins.replaceStrings [".drv"] ["-drv"] (if expr != null then expr else attrPath)}";
      in
      (pkgs.runCommand name { } ''
        export SSL_CERT_FILE=/dev/null
  ...
  evalTests = {
    ...
    eval-nixpkgs-nixos-minimal-installer-drvpath = (mkNixpkgsEvalTest {
      expr = "(import ${pkgs.path}/nixos/release.nix { }).iso_minimal.${pkgs.system}.drvPath";
      expectedPath = (import "${pkgs.path}/nixos/release.nix" { }).iso_minimal.${pkgs.system}.drvPath;
    });
    eval-nixpkgs-nixos-minimal-installer-outpath = (mkNixpkgsEvalTest {
      expr = "(import ${pkgs.path}/nixos/release.nix { }).iso_minimal.${pkgs.system}.outPath";
      expectedPath = (import "${pkgs.path}/nixos/release.nix" { }).iso_minimal.${pkgs.system}.outPath;
    });
    ...
    eval-nixpkgs-nixos-plasma-installer-drvpath = (mkNixpkgsEvalTest {
      expr = "(import ${pkgs.path}/nixos/release.nix { }).iso_plasma6.${pkgs.system}.drvPath";
      expectedPath = (import "${pkgs.path}/nixos/release.nix" { }).iso_plasma6.${pkgs.system}.drvPath;
    });
    eval-nixpkgs-nixos-plasma-installer-outpath = (mkNixpkgsEvalTest {
      expr = "(import ${pkgs.path}/nixos/release.nix { }).iso_plasma6.${pkgs.system}.outPath";
      expectedPath = (import "${pkgs.path}/nixos/release.nix" { }).iso_plasma6.${pkgs.system}.outPath;
    });
  };
in
{
