...
let
  mkNixpkgsEvalCheck = attrset: expectedPath: {
    ...
    command = pkgs.writeShellScript "tvix-eval-${builtins.replaceStrings [ ".drv" ] [ "-drv" ] attrset}" ''
      ...
      EXPECTED='${
        # the verbatim expected Tvix output:
...
(depot.tvix.crates.workspaceMembers.tvix-cli.build.override { runTests = true; }).overrideAttrs (
  ...
  let
    ...
    mkExprBenchmark =
      { expr, description }:
      let
        name = "tvix-cli-benchmark-${description}";
      in
    ...
    meta = {
      ci.extraSteps = {
        ...
        eval-nixpkgs-cross-stdenv-outpath = (
          mkNixpkgsEvalCheck "pkgsCross.aarch64-multiplatform.stdenv.outPath" pkgs.pkgsCross.aarch64-multiplatform.stdenv.outPath
        );
        eval-nixpkgs-cross-hello-outpath = (
          mkNixpkgsEvalCheck "pkgsCross.aarch64-multiplatform.hello.outPath" pkgs.pkgsCross.aarch64-multiplatform.hello.outPath
        );
      };
