...
(depot.tvix.crates.workspaceMembers.tvix-cli.build.override {
  ...
  let
    ...
    mkExprBenchmark = { expr, description }:
      let name = "tvix-cli-benchmark-${description}";
    ...
    mkNixpkgsEvalTest = attrpath: expectedPath:
      let
        name = "tvix-eval-test-${
            builtins.replaceStrings [ ".drv" ] [ "-drv" ] attrpath
          }";
      in (pkgs.runCommand name { } ''
        export SSL_CERT_FILE=/dev/null
        TVIX_OUTPUT=$(${tvix-cli}/bin/tvix -E '(import ${pkgs.path} {}).${attrpath}')
        EXPECTED='${ # the verbatim expected Tvix output:
    ...
    benchmarks = {
      ...
      benchmark-cross-hello =
        (mkNixpkgsBenchmark "pkgsCross.aarch64-multiplatform.hello.outPath");
      benchmark-firefox = (mkNixpkgsBenchmark "firefox.outPath");
      benchmark-cross-firefox =
        (mkNixpkgsBenchmark "pkgsCross.aarch64-multiplatform.firefox.outPath");
      # Example used for benchmarking LightSpan::Delayed in commit bf286a54bc2ac5eeb78c3d5c5ae66e9af24d74d4
      benchmark-nixpkgs-attrnames = (mkExprBenchmark {
    ...
    evalTests = {
      eval-nixpkgs-stdenv-drvpath =
        (mkNixpkgsEvalTest "stdenv.drvPath" pkgs.stdenv.drvPath);
      eval-nixpkgs-stdenv-outpath =
        (mkNixpkgsEvalTest "stdenv.outPath" pkgs.stdenv.outPath);
      eval-nixpkgs-hello-outpath =
        (mkNixpkgsEvalTest "hello.outPath" pkgs.hello.outPath);
      eval-nixpkgs-firefox-outpath =
        (mkNixpkgsEvalTest "firefox.outPath" pkgs.firefox.outPath);
      eval-nixpkgs-firefox-drvpath =
        (mkNixpkgsEvalTest "firefox.drvPath" pkgs.firefox.drvPath);
      eval-nixpkgs-cross-stdenv-outpath =
        (mkNixpkgsEvalTest "pkgsCross.aarch64-multiplatform.stdenv.outPath"
          pkgs.pkgsCross.aarch64-multiplatform.stdenv.outPath);
      eval-nixpkgs-cross-hello-outpath =
        (mkNixpkgsEvalTest "pkgsCross.aarch64-multiplatform.hello.outPath"
          pkgs.pkgsCross.aarch64-multiplatform.hello.outPath);
    };
  in {
    meta = {
