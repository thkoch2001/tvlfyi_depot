...
`)
	...
	// Set up a very barebones environment.
	if err := os.MkdirAll("/etc", os.ModePerm); err != nil {
		log.Printf("Failed to create /etc: %v\n", err)
	}
	...
	if _, ok := cmdlineFields["tvix.find"]; ok {
		...
		// Prepare /fs/nix/store
		if err := mountTvixStore("/fs/nix/store"); err != nil {
			log.Fatalf("Failed to mount tvix store: %v\n", err)
		}

		// Prepare /fs/dev
		if err := os.Mkdir("/fs/dev", os.ModePerm); err != nil {
			log.Fatalf("Failed to mkdir /fs/dev: %s\n", err)
		}
		if err := run("mount", "-t", "devtmpfs", "devtmpfs", "/fs/dev"); err != nil {
			log.Printf("Failed to mount /fs/dev: %v\n", err)
		}

		// Prepare /fs/proc
		if err := os.Mkdir("/fs/proc", os.ModePerm); err != nil {
			log.Fatalf("Failed to mkdir /fs/proc: %s\n", err)

		}
		if err := run("mount", "-t", "proc", "none", "/proc"); err != nil {
			log.Printf("Failed to mount /fs/proc: %v\n", err)
		}

		// Prepare /fs/sys
		os.Mkdir("/fs/sys", os.ModePerm)
		if err := run("mount", "-t", "sysfs", "none", "/fs/sys"); err != nil {
			log.Printf("Failed to mount /sys: %v\n", err)
		}
