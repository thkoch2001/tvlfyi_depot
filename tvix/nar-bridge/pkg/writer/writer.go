...
// If one of them is nil, the other one is picked.
// If both are nil, nil is returned.
func smallerNode(a interface{}, b interface{}) interface{} {
	if a == nil {
		// if a is nil, b wins
		return b
	} else {
		// if a is not nil, check b
		if b == nil {
			// if it's nil, a wins
			return a
		} else {
			// a and b are not nil, actually check their names
			if getNodeName(a) < getNodeName(b) {
				return a
			} else {
				return b
			}
...
// getNodeName returns the Name of a node, no matter if it's a directory, file
// or symlink in case nil is passed, or something else, we panic.
func getNodeName(n interface{}) string {
	switch v := (n).(type) {
	case *storev1pb.DirectoryNode:
		return v.GetName()
	case *storev1pb.FileNode:
	...
	case *storev1pb.SymlinkNode:
		return v.GetName()
	default:
		panic("invalid type passed for getName")
	}
}
