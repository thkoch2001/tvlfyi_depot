...
func registerNarGet(s *Server) {
	s.handler.Get("/nar/{narhash:^(["+nixbase32.Alphabet+"]{52}|["+nixbase32.Alphabet+"]{103})$}.nar", func(w http.ResponseWriter, r *http.Request) {
		...
		if pathInfo == nil {
			...
			w.Write([]byte("narHash not found"))
		...
		if pathInfoDirectory, isDirectory := pathInfo.GetNode().(*storev1pb.PathInfo_Directory); isDirectory {
			...
			directoryStream, err := s.directoryServiceClient.Get(ctx, &storev1pb.GetDirectoryRequest{
				ByWhat: &storev1pb.GetDirectoryRequest_RootDigest{
					RootDigest: rootDirectoryDigest,
				},
				Recursive: true,
			})
			if err != nil {
				...
				w.Write([]byte("unable to query directory stream"))
				return
			}
			...
			for {
				...
				if err != nil {
					...
					w.WriteHeader(http.StatusInternalServerError)
					w.Write([]byte("unable to receive from directory stream"))
					return
				}
				...
				if err != nil {
					...
					w.WriteHeader(http.StatusInternalServerError)
					w.Write([]byte("unable to calculate directory digest"))
					return
				}
		...
		writer.Export(
			...
			func(blobRef []byte) (io.ReadCloser, error) {
				...
				if err != nil {
					return nil, fmt.Errorf("unable to receive from directory stream: %w", err)
