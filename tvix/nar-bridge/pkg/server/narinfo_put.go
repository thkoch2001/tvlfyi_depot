...
func registerNarinfoPut(s *Server) {
	s.handler.Put("/{outputhash:^["+nixbase32.Alphabet+"]{32}}.narinfo", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, narinfoSig := range narInfo.Signatures {
			// narinfoSig.KeyName
			// // TODO: improve narinfo signature parser?
			// narinfoSigSegments := strings.Split(narinfoSig.String(), ":")
			// if len(narinfoSigSegments) != 2 {
			// 	log.WithField("narinfo_signature", narinfoSig.String()).Error("unable to parse narinfo signature")
			// 	w.WriteHeader(http.StatusBadRequest)
			// 	log.WithError(err).Error("unable to parse narinfo signature")
			// }
