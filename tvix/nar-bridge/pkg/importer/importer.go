	...
	for {
		...
		default:
			...
			if hdr.Type == nar.TypeRegular {
				...
				var fileNodeOp *op[*castorev1pb.FileNode]
				if hdr.Size < asyncUploadThreshold {
					...
					fileNodeOp = doAsync(func() (*castorev1pb.FileNode, error) {
						defer asyncUploadSem.Release(hdr.Size)

						return uploadBlob(bytes.NewReader(b))
					})
				} else {
					fileNodeOp = doSync(func() (*castorev1pb.FileNode, error) {
				...
				if len(stack) > 0 {
					topOfStack := stack[len(stack)-1]
					topOfStack.files = append(topOfStack.files, fileNodeOp)
				} else {
					rootFileOp = fileNodeOp
				}
			}
