...
pub async fn head(
    ...
    let digest = nix_http::parse_narinfo_str(&narinfo_str).ok_or_else(|| StatusCode::NOT_FOUND)?;
...
pub async fn put(
    ...
    let _narinfo_digest =
        nix_http::parse_narinfo_str(&narinfo_str).ok_or_else(|| StatusCode::UNAUTHORIZED);
    Span::current().record("path_info.digest", &narinfo_str[0..32]);
