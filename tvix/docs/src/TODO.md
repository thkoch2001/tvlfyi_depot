 ...
 - On the trait side, currently there's no way to distinguish reading a
   ...
   Design idea (@flokli): split these two concerns into two separate traits:
    - a `ChunkService` dealing with retrieving individual chunks, by their
      ...
      memory
    - a `BlobService` storing metadata about blobs. We might require it to also
      implement `ChunkService`, though most implementations could become
      metadata only, and require the `ChunkService` to proxy through to in their
      constructor.

   Individual stores would not need to implement `BlobReader` anymore, but that
   could be global thing with access to the whole store composition layer, which
   should make it easier to reuse chunks from other backends. Unclear if the
   write path should be structured the same way. At least for some backends, we
   want the remote end to decide about chunking.
