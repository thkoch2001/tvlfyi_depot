...
### Store composition
 ...
 - Store composition hierarchies (@yuka). URL format too one-dimensional, but on the
   user-facing side, we might get quite far with a similar "substituters"
   concept that Nix uses, to construct the composed stores.
   - Current design idea:
     ...
     - Making URLs the primary data format would get ugly quite easily (hello
       ...
       syntax to our new config format on the fly and then use one codepath to
       instantiate/assemble. Similarly, something like the "substituters
       concept" from Nix could also be converted to such a config format under
       the hood.
