...
impl<'a> NixConfig<'a> {
    /// Parses configuration from a file like `/etc/nix/nix.conf`, updating
    /// `&self` with all values contained in there.
    /// Can be run multiple times, though merging is currently not implemented
    /// (existing values are overwritten).
    pub fn parse(&mut self, input: &'a str) -> Result<(), Error> {
...
mod tests {
    ...
    pub fn test_parse() {
        let config = {
            ...
            config.parse(include_str!("nix.conf")).expect("must parse");
            config
        };
        ...
        assert_eq!(
            NixConfig {
                ...
                substituters: Some(vec!["https://cache.tvl.fyi", "https://cache.nixos.org/"]),
                ...
                trusted_public_keys: Some(vec![
                    ...
                    PubKey::parse("cache.tvl.su:kjc6KOMupXc1vHVufJUoDUYeLzbwSr9abcAKdn/U1Jk=")
                        .expect("failed to parse pubkey")
                ]),
