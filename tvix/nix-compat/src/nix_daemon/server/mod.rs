            ...
            if let Ok(operation) = TryInto::<Operation>::try_into(op_code) {
                match operation {
                    ...
                    Operation::QueryPathFromHashPart => {
                        ...
                        self.handle(io.query_path_from_hash_part(hash).map_err(|r| r.into()))
                            .await?
                    }
                    Operation::QueryValidDerivers => {
                        let path: StorePath = self.reader.read_value().await?;
                        self.handle(async move {
                            let result = io.query_path_info(&path).await?.value;
                            let result: Vec<StorePath> = result
                                .into_iter()
                                .map(|info| info.deriver.value)
                                .flatten()
                                .collect();
                            Ok(result)
                        })
                    ...
                    Operation::QueryValidPaths => {
                        let request: QueryValidPaths = self.reader.read_value().await?;
                        self.handle(async move {
                            let result: Vec<std::io::Result<Option<StorePath>>> =
                                join_all(request.paths.into_iter().map(|p| async {
                                    let info = io.query_path_info(&p).await?;
                                    Ok(info.value.map(|_| p))
                                }))
                                .await;
                            let result: Vec<StorePath> =
                                result.into_iter().flatten().into_iter().flatten().collect();
                            Ok(result)
                        })
                        .await?
                    }
                    ...
                    Operation::QueryReferrers | Operation::QueryRealisation => {
                        let _: StorePath = self.reader.read_value().await?;
                        self.handle(async move { Ok(Vec::<String>::new()) }).await?
                    }
                    _ => {
                        return Err(std::io::Error::other(
