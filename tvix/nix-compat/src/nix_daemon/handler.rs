...
/// and determines the [ProtocolVerson] to use for the remainder of the session.
///
/// Once initialized, [`handle_client`] needs to be called to handle the rest of the session,
/// it delegates all operation handling to instance of [NixDaemonIO]
///
        ...
        T: NixSerialize + Send,
    {
        ...
        match result {
            Ok(r) => {
                ...
                // by sending STDERR_LAST and then send the response
                writer.write_number(STDERR_LAST).await?;
