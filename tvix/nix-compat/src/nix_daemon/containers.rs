    ...
    T: NixSerialize + Sync + Send,
{
        ...
        W: NixWrite + Send,
    {
        match &self.value {
            Some(inner) => {
                writer.write(&true).await?;
                writer.write(inner).await
            }
            None => writer.write(&false).await,
        }
    }
}
    ...
    T: NixSerialize + Default + Sync + Send,
{
    ...
    {
        match &self.value {
            Some(inner) => writer.write(inner).await,
            None => writer.write(&T::default()).await,
        }
    }
}
