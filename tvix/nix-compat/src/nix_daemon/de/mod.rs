...
impl Error for io::Error {
    fn custom<T: fmt::Display>(msg: T) -> Self {
        Self::new(io::ErrorKind::Other, msg.to_string())
    }
    ...
    fn invalid_data<T: fmt::Display>(msg: T) -> Self {
        Self::new(io::ErrorKind::InvalidData, msg.to_string())
    }

    fn missing_data<T: fmt::Display>(msg: T) -> Self {
        Self::new(io::ErrorKind::UnexpectedEof, msg.to_string())
    }
}
    ...
    fn read_number(&mut self) -> impl Future<Output = Result<u64, Self::Error>> + Send + '_ {
        async move {
            self.try_read_number()
                .await?
                .ok_or_else(|| Self::Error::missing_data("unexpected end-of-file"))
        }
    }
...
pub trait NixDeserialize: Sized {
    ...
    {
        async move {
            Self::try_deserialize(reader)
                .await?
                .ok_or_else(|| R::Error::missing_data("unexpected end-of-file"))
        }
    }
}
