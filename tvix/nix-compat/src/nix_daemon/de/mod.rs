...
/// Like serde the `Error` trait allow `NixRead` implementations to make
/// custom handling of different error situations the `NixDeserialize`
/// might come across.
...
pub trait NixRead: Send {
    ...
    /// You also specify a size limit on the returned bytes.
    /// This returns an Option to support graceful shutdown.
