...
#[derive(Debug, PartialEq, Primitive, NixSerialize, Clone, Copy)]
#[nix(into = "u64")]
pub enum Verbosity {
    LvlError = 0,
    LvlWarn = 1,
...
impl From<Verbosity> for tracing::Level {
    fn from(value: Verbosity) -> Self {
        match value {
            Verbosity::LvlError => tracing::Level::ERROR,
            Verbosity::LvlWarn => tracing::Level::WARN,
            Verbosity::LvlNotice => tracing::Level::INFO,
            Verbosity::LvlInfo => tracing::Level::INFO,
            Verbosity::LvlTalkative => tracing::Level::INFO,
            Verbosity::LvlChatty => tracing::Level::DEBUG,
            Verbosity::LvlDebug => tracing::Level::DEBUG,
            Verbosity::LvlVomit => tracing::Level::TRACE,
        }
    }
}

// Implementing manually, since nix client can send illegal verbosity values, i.e. when using `nix-buid -vvvvv`
impl NixDeserialize for Verbosity {
    async fn try_deserialize<R>(reader: &mut R) -> Result<Option<Self>, R::Error>
    where
        R: ?Sized + super::de::NixRead + Send,
    {
        let value = reader.try_read_number().await?;
        Ok(value.map(|v| TryFrom::try_from(v).unwrap_or(Self::LvlVomit)))
    }
}

/// Settings requested by the client. These settings are applied to a
/// connection to between the daemon and a client.
#[derive(Debug, Default, PartialEq, NixDeserialize, NixSerialize)]
