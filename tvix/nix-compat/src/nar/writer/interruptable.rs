...
impl<'a, W: Write> Node<'a, W> {
    ...
    pub fn file_skip(mut self, executable: bool, size: u64) -> io::Result<(&'a mut W, FileSkip)> {
        self.write(if executable {
            &wire::TOK_EXE
        } else {
            &wire::TOK_REG
...
impl FileSkip {
    pub fn cont<W: Write>(self, writer: &mut W) -> io::Result<Node<W>> {
        let mut node = Node { writer };
        node.pad(self.size)?;
