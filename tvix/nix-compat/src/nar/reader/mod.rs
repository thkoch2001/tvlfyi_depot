...
use crate::nar::wire::{self, MAX_NAME_LEN, MAX_TARGET_LEN, TOK_NAR, TOK_NOD, TOK_PAR};
...
impl<'a, 'r> Node<'a, 'r> {
    fn new(reader: &'a mut Reader<'r>) -> io::Result<Self> {
        Ok(match read::tag(reader)? {
            wire::Node::Sym => {
                let target = read::bytes(reader, MAX_TARGET_LEN)?;

                if target.is_empty() || target.contains(&0) {
                    return Err(InvalidData.into());
                }

                read::token(reader, &TOK_PAR)?;
...
impl<'a, 'r> FileReader<'a, 'r> {
    ...
    fn read_end(&mut self) -> io::Result<()> {
        ...
        read::token(self.reader, &TOK_PAR)
    }
...
impl<'a, 'r> DirReader<'a, 'r> {
    ...
    pub fn next(&mut self) -> io::Result<Option<Entry>> {
        ...
        let name = read::bytes(self.reader, MAX_NAME_LEN)?;
        ...
        read::token(self.reader, &TOK_NOD)?;
