...
mod tests {
    ...
    async fn test_read_client_settings_without_overrides() {
        // Bits captured from a Nix 2.3.17 run w/ sockdump (protocol version 21).
        let wire_bits = hex!(
        ...
        let settings = read_client_settings(&mut mock, 21)
            .await
            .expect("should read the settings");
    ...
    async fn test_read_client_settings_with_overrides() {
        ...
        let overrides = HashMap::from([
            (
                Vec::from("allowed-uris"),
                Vec::from("https://bordeaux.guix.gnu.org/"),
            ),
            (Vec::from("allowed-users"), Vec::from("jean pierre")),
        ]);
        let expected = ClientSettings {
            keep_failed: false,
            keep_going: false,
