            ...
            match **dbg!(&this.state) {
                ...
                BytesPacketPosition::Payload(pos) => {
                    ...
                    if pos + bytes_read as u64 == signalled_size {
                        ...
                        // state, then return.
                        // As we did read something into the buffer (otherwise
                        // we'd have bubbled up the `Poll::Pending` and not ended up here),
                        // we're not "EOF" yet.
                        // It's ok to return now and assume to get called again
                        // (waking up in the padding state).
                        // If the payload length is zero, returning here without
                        // writing anything to `buf` (signalling EOF) is also
                        // okay, as an empty packet has zero padding.
                        *this.state = BytesPacketPosition::Padding(0);
                    } else {
