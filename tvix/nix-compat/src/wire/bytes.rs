    ...
    S: RangeBounds<u64>,
{
    ...
    let len = match u32::try_from(primitive::read_u64(r).await?) {
        Err(_) => None,
        Ok(len) if !allowed_size.contains(&(len as u64)) => None,
        Ok(len) => Some(len),
    }
    .ok_or_else(|| {
        std::io::Error::new(
    ...
    let padded_len = (padding(len as u64) as usize) + (len as usize);
    let mut limited_reader = r.take(padded_len as u64);
...
pub async fn write_bytes<W: AsyncWriteExt + Unpin>(w: &mut W, b: &[u8]) -> std::io::Result<()> {
    ...
    w.write_all(b).await?;
    let padding = padding(len as u64);
    if padding != 0 {
...
///
/// The len input is exspressed as a u32 because it's sometimes
/// provided by the Nix client as a u32. It'd be unsafe to convert it
/// to usize. For the output, we're sure it won't ever exceed u64:
/// it's a 8-modulo.
fn padding(len: u64) -> u8 {
    let modulo = len % 8;
    if modulo == 0 {
        0
    } else {
