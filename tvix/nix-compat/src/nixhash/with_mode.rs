...
impl NixHashWithMode {
    ...
    /// This is necessary to do such shenigans because we have external consumers
    /// who would like to know whether we have a invalid or a missing NixHashWithMode structure
    /// in another structure, e.g. Output.
    ...
    /// solution.
    /// Now this is said, let's explain how this works.
    ///
    ...
    pub(crate) fn from_map<'de, D>(map: &Map<String, Value>) -> Result<Option<Self>, D::Error>
        ...
        let digest: Vec<u8> = {
            if let Some(v) = map.get("hash") {
                ...
                return Err(serde::de::Error::missing_field("couldn't extract 'hash' key"));
            }
        };
        ...
        if let Some(v) = map.get("hashAlgo") {
            if let Some(s) = v.as_str() {
                match s.strip_prefix("r:") {
                    Some(rest) => Ok(Some(NixHashWithMode::Recursive(NixHash::new(
                        HashAlgo::try_from(rest).map_err(|e| {
                            serde::de::Error::invalid_value(Unexpected::Other(&e.to_string()),
                            &"one of ...") // TODO: say which algos are supported.
                        })?,
                        digest,
                    )))),
                    None => Ok(Some(NixHashWithMode::Flat(NixHash::new(
            ...
            Err(serde::de::Error::missing_field("couldn't extract 'hashAlgo' key"))
        }
    }
}
