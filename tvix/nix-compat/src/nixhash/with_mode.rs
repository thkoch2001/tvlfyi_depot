...
impl NixHashWithMode {
    ...
    /// This is necessary to do such shenigans because we have external consumers, which are, so
    /// far:
    ...
    pub(crate) fn from_map<'de, D>(map: &Map<String, Value>) -> Result<Option<Self>, D::Error>
        ...
        let digest: Vec<u8> = {
            if let Some(v) = map.get("hash") {
                ...
                return Err(serde::de::Error::missing_field("couldn't extract `hash` key but `hashAlgo` key present"));
            }
        };
        ...
        if let Some(v) = map.get("hashAlgo") {
            ...
            Err(serde::de::Error::missing_field("couldn't extract `hashAlgo` key, but `hash` key present"))
        }
    }
}
