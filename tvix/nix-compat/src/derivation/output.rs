        ...
        where
            D: serde::Deserializer<'de> {
                ...
                Ok(Self {
                    path: fields
                            .get("path")
                            .ok_or(serde::de::Error::missing_field(&"`path` is missing and required for outputs"))?
                            .as_str()
                            .ok_or(serde::de::Error::invalid_type(serde::de::Unexpected::Other(&"certainly not a string"), &"a string"))?
                            .to_owned(),
...
#[test]
fn deserialize_input_addressed_output() {
    let json_bytes = r#"
    {
      "path": "/nix/store/blablabla"
    }"#;
    let output: Output = serde_json::from_str(&json_bytes).expect("JSON was not well-formatted");

    assert!(!output.is_fixed(), "Output is fixed output, expected input-addressed");
}

#[test]
fn deserialize_fixed_output() {
    let json_bytes = r#"
    {
    ...
    assert!(output.is_fixed(), "Output is not fixed output, expected fixed output");
}

#[test]
fn deserialize_with_error_invalid_length_fixed_output() {
    let json_bytes = r#"
    {
        "path": "/nix/store/blablablabla",
        "hash": "IAMOFINVALIDLENGTH",
        "hashAlgo": "r:sha256"
    }"#;
    let output: Result<Output, _> = serde_json::from_str(&json_bytes);

    assert!(output.is_err(), "Output was supposed to be broken, still deserialized without any error");
}
