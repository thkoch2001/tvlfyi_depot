        ...
        Ok(Self {
            path: fields
                ...
                .ok_or(serde::de::Error::missing_field(
                    &"`path` is missing and required for outputs",
                ))?
                .as_str()
...
fn deserialize_input_addressed_output() {
    ...
    assert!(
        !output.is_fixed(),
        "Output is fixed output, expected input-addressed"
    );
}
...
fn deserialize_fixed_output() {
    ...
    assert!(
        output.is_fixed(),
        "Output is not fixed output, expected fixed output"
    );
}
...
fn deserialize_with_error_invalid_length_fixed_output() {
    ...
    assert!(
        output.is_err(),
        "Output was supposed to be broken, still deserialized without any error"
    );
}
