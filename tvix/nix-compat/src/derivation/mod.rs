...
pub struct PreDerivation<DrvPath> {
    #[serde(rename = "args")]
    ...
    #[serde(bound(serialize = "DrvPath: Serialize"))]
    #[serde(bound(deserialize = "DrvPath: Deserialize<'de> + Ord"))]
...
impl<DrvHash> Default for PreDerivation<DrvHash> {
    fn default() -> Self {
        PreDerivation {
...
impl<DrvPath: Ord + Serialize> PreDerivation<DrvPath> {
...
pub type Derivation = PreDerivation<StorePath>;
...
impl Derivation {
        ...
        F: Fn(&StorePath) -> NixHash,
    {
        ...
        let digest = self.fod_digest().unwrap_or({
            ...
            let replaced_derivation = PreDerivation::<String> {
                input_derivations: replaced_input_derivations,
                // other fields, unfortunately need to do manually because type change
