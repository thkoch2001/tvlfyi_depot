...
use super::parse_error::{self, into_nomerror, ErrorKind, NomError, NomResult};
...
/// it to a BTreeMap.
/// We don't use parse_map here, as it's dealing with 2-tuples, and these are
/// 4-tuples.
            ...
            for (output_name, output) in outputs_lst.into_iter() {
                if outputs.contains_key(&output_name) {
                    return Err(nom::Err::Failure(NomError {
                        input: i,
                        code: parse_error::ErrorKind::DuplicateMapKey(output_name),
                    }));
                }
...
fn parse_input_derivations(i: &[u8]) -> NomResult<&[u8], BTreeMap<String, Vec<String>>> {
    parse_map::<Vec<String>, _>(aterm::parse_str_list)(i)
}
    ...
    preceded(
        ...
        delimited(
            ...
            tuple((
                ...
                |i| terminated(aterm::parse_str_list, nomchar(','))(i).map_err(into_nomerror),
                // parse environment
                parse_map::<Vec<u8>, _>(aterm::parse_bstr_field),
            )),
            nomchar(')'),
        )
        .map(
            |(
                ...
                // All values in environment need to be converted from Vec<u8>
                // to BString.
                let mut environment_new: BTreeMap<String, BString> = BTreeMap::new();
                for (k, v) in environment.into_iter() {
                    environment_new.insert(k, v.into());
                }
...
mod tests {
    ...
    lazy_static! {
        ...
        static ref EXP_AB_MAP: BTreeMap<String, Vec<u8>> = {
            let mut b = BTreeMap::new();
    ...
    fn parse_map(input: &'static [u8], expected: &BTreeMap<String, Vec<u8>>, exp_rest: &[u8]) {
        let (rest, parsed) = super::parse_map::<Vec<u8>, _>(crate::aterm::parse_bstr_field)(input)
            .expect("must parse");
