...
fn parse_input_derivations(i: &[u8]) -> NomResult<&[u8], BTreeMap<String, BTreeSet<String>>> {
    ...
    for (input_derivation, outputs) in input_derivations_list {
        let mut new_output_names = BTreeSet::new();
        for output in outputs.into_iter() {
            if !new_output_names.insert(output.clone()) {
                return Err(nom::Err::Failure(NomError {
                    input: i,
                    code: ErrorKind::DuplicateInputDerivationOutputName(output.to_string()),
                }));
            }
        }
