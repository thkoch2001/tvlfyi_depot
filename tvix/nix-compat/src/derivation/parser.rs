    ...
    preceded(
        ...
        delimited(
            ...
            tuple((
                ...
                |i| terminated(aterm::parse_str_list, nomchar(','))(i).map_err(into_nomerror),
                // parse environment
                parse_map::<BString, _>(aterm::parse_bstr_field),
            )),
            nomchar(')'),
        )
        .map(
...
mod tests {
    ...
    fn parse_map(input: &'static [u8], expected: &BTreeMap<String, BString>, exp_rest: &[u8]) {
        let (rest, parsed) = super::parse_map::<BString, _>(crate::aterm::parse_bstr_field)(input)
            .expect("must parse");
