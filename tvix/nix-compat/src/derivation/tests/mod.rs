...
fn output_paths(name: &str, drv_path: &str) {
    ...
    // calculate the hash derivation modulo.
    // We don't expect the lookup function to be called for most derivations.
    let calculated_hash_derivation_modulo = derivation.hash_derivation_modulo(|parent_drv_path| {
                    ...
                    && parent_drv_path == "/nix/store/ss2p4wmxijn652haqyd7dckxwl4c7hxx-bar.drv"))
        {
            ...
            // calculate hash_derivation_modulo. This may not trigger any subsequent requests, as they're both FOD.
    ...
    derivation
        .calculate_output_paths(name, &calculated_hash_derivation_modulo)
        .unwrap();
...
fn output_path_construction() {
    ...
    let foo_calc_result = foo_drv.calculate_output_paths(
        ...
        &foo_drv.hash_derivation_modulo(|drv_path| {
            ...
            bar_drv_hash_derivation_modulo.clone()
        }),
    );
