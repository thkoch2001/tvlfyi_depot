...
fn cp1252() {
    ...
    outputs.insert(
        "out".to_string(),
        Output {
            path: "/nix/store/drr2mjp9fp9vvzsf5f9p0a80j33dxy7m-cp1252".to_string(),
            ..Default::default()
        },
    );
    ...
    environment.insert("chars".to_string(), vec![0xc5, 0xc4, 0xd6].into());
    environment.insert("name".to_string(), "cp1252".into());
    environment.insert(
        "out".to_string(),
        "/nix/store/drr2mjp9fp9vvzsf5f9p0a80j33dxy7m-cp1252".into(),
    );
    ...
    let actual_drv_path = derivation.calculate_derivation_path("cp1252").unwrap();
    assert_eq!(
        "/nix/store/m1vfixn8iprlf0v9abmlrz7mjw1xj8kp-cp1252.drv".to_string(),
        actual_drv_path.to_absolute_path()
    );
    ...
    // path.
    {
        ...
        derivation
            .calculate_output_paths("cp1252", &calculated_derivation_or_fod_hash)
            .unwrap();

        assert_eq!(
            "/nix/store/drr2mjp9fp9vvzsf5f9p0a80j33dxy7m-cp1252".to_string(),
    ...
    // Construct the ATerm representation and compare with our fixture.
    {
        let data = read_file(&format!(
            "{}/{}",
            RESOURCES_PATHS, "m1vfixn8iprlf0v9abmlrz7mjw1xj8kp-cp1252.drv"
        ));
        assert_eq!(
            data,
