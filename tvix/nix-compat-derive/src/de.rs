...
fn nix_deserialize_impl(
    ...
    quote! {
        ...
        impl #impl_generics #crate_path::nix_daemon::NixDeserialize for #ty #ty_generics
            #where_clause
        {
            #[allow(clippy::manual_async_fn)]
            fn try_deserialize<R>(reader: &mut R) -> impl ::std::future::Future<Output=Result<Option<Self>, R::Error>> + Send + '_
                where R: ?Sized + #crate_path::nix_daemon::NixRead + Send,
            {
                #body
            }
        }
