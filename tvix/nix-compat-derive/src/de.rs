...
fn nix_deserialize_body_from(
    ...
    } else if let Some(type_try_from) = attrs.type_try_from.as_ref() {
        Some(nix_deserialize_try_from(crate_path, type_try_from))
    } else {
        None
    }
}
...
fn nix_deserialize_struct(style: Style, fields: &Vec<Field<'_>>) -> TokenStream {
...
fn nix_deserialize_enum(variants: &Vec<Variant<'_>>) -> TokenStream {
