...
fn nix_serialize_impl(
    ...
    quote! {
        ...
        impl #impl_generics #crate_path::nix_daemon::NixSerialize for #ty #ty_generics
            #where_clause
        {
            async fn serialize<W>(&self, writer: &mut W) -> std::result::Result<(), W::Error>
                where W: #crate_path::nix_daemon::NixWrite
            {
                use #crate_path::nix_daemon::SerializationError as _;
                #body
            }
        }
    }
...
fn nix_serialize_try_into(crate_path: &Path, ty: &Type) -> TokenStream {
    quote_spanned! {
        ty.span() =>
        {
            use #crate_path::nix_daemon::SerializationError;
            let other : #ty = <Self as Clone>::clone(self).try_into().map_err(SerializationError::unsupported_data)?;
            writer.write_value(&other).await
        }
    }
}
