...
pub fn main_args(args: Vec<String>) -> clap::Result<NixResult> {
  let matches = clap::App::new("nix-store")
   ...
   if let Some(add) = matches.subcommand_matches("--add") {
     ...
     let file_contents = std::fs::read_to_string(file).expect(&format!("file {} does not exist", file));
     Ok(NixResult::FileAddedToStore{ content: file_contents })
   }
   else {
...
mod integration_tests {
  ...
  fn run_nix_command(cmd: &str, args: Vec<String>) -> NixOutput {
    let out = std::process::Command::new(cmd).args(args).output().expect("could not run nix-store");
  ...
  fn nix_nix_store<'a>(args: Vec<String>) -> NixResult {
    ...
    match run_nix_command("nix-store", args) {
      ...
      NixOutput::Ok{stdout, ..} => {
        ...
        NixResult::FileAddedToStore{ content:
          std::fs::read_to_string(&stdout).expect(&format!("cannot open {} as store file", stdout))
  ...
  fn tvix_nix_store<'a>(args: Vec<String>) -> NixResult {
    ...
    super::main_args(Vec::from(args)).unwrap_or_else(|e| panic!("clap command line parsing failed:\n{}", e))
  }
  ...
  fn test_realise() {
    ...
    tempfile.write_all(file_content.as_bytes()).expect("could not write to tempfile");
    assert_eq!(
      tvix_nix_store(vec!["--add".to_string(), tempfile.path().as_os_str().to_string_lossy().into_owned()]),
      nix_nix_store(vec!["--add".to_string(), tempfile.path().as_os_str().to_string_lossy().into_owned()]),
