...
pub fn from_str_impure<'code, T>(src: &'code str, io_handle: Box<dyn EvalIO>) -> Result<T, Error>
where
    T: serde::Deserialize<'code>,
{
    from_str_with_config_impure(src, |_| /* no extra config */ (), io_handle)
}
