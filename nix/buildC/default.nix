...
let
  ...
  # TODO(sterni): full freestanding support
  # TODO(sterni): combat SIGKILL flakiness
  # feat(sterni): generate release tarball buildable with GNU make
  ...
  library =
    ...
    , name
      # name of the library to build and install: libName should
      # be the same as the argument passed to -l in order to link
      # against this library, i. e. the resulting library is
      # installed to $out/lib/lib${libName}.{a, so}
    , libName ? name
    ...
    , CFLAGS ? []
      # Extra LDFLAGS to be used when linking this into a shared library.
      # Has no effect if buildConf.static is true.
      # TODO(sterni): remove this as I don't know an usecase for this atm.
    , LDFLAGS ? []
