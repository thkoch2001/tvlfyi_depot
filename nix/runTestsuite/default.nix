...
let
  ...
  bins = depot.nix.getBins pkgs.coreutils [ "printf" ]
    // depot.nix.getBins pkgs.s6-portable-utils [
      "s6-touch"
      "s6-false"
      "s6-cat"
    ];
  ...
  AssertErrorContext = sum "AssertErrorContext" {
  ...
  AssertResult = sum "AssertResult" {
    yep = struct "yep" { test = string; };
    nope = struct "nope" {
      test = string;
  ...
  assertEq = defun [ string any any AssertResult ] (desc: left: right:
    let context = { not-equal = { inherit left right; }; };
    in assertBoolContext context desc (left == right));

  # assert that the expression throws when `deepSeq`-ed
  assertThrows = defun [ string any AssertResult ] (desc: expr:
    let context = { should-throw = { inherit expr; }; };
    in assertBoolContext context desc (throws expr));

  # assert that the expression does not throw when `deepSeq`-ed
  assertDoesNotThrow = defun [ string any AssertResult ] (desc: expr:
  ...
  runTestsuite = defun [ string (list ItResult) drv ] (name: itResults:
    let
      goodAss = ass:
      ...
      res = partitionTests (it: (partitionTests goodAss it.asserts).err == [ ])
        itResults;
...
in { inherit assertEq assertThrows assertDoesNotThrow it runTestsuite; }
