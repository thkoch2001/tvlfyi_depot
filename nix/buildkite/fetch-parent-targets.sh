...
function latest_drvmap_url {
    ...
    curl 'https://graphql.buildkite.com/v1' \
         ...
         -d "{\"query\": \"{ pipeline(slug: \\\"tvl/depot\\\") { builds(first: 50, branch: [\\\"%default\\\"], state: [RUNNING, PASSED]) { edges { node { jobs(passed: true, first: 1, type: [COMMAND], step: {key: [\\\"pipeline-gen\\\"]}) { edges { node { ... on JobTypeCommand { url artifacts { edges { node { downloadURL path }}}}}}}}}}}}\"}" | tee out.json | \
