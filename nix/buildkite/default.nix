...
in rec {
    ...
    targetToSteps = target: let
      ...
      splitExtraSteps = partition ({ postStep, ... }: postStep)
       (attrValues (mapAttrs (name: value: {
         inherit name value;
         postStep = (value ? prompt) || (value ? postBuild && value.postBuild);
  ...
  #     which this step should be allowed to run. Format is a string
  #     with refs separated by spaces.
  #
  ...
  # steps.
  mkGatedStep = { step, label, parent, prompt, branches }: {
    inherit branches;
    group = label;
  ...
  in if (isString prompt)
    then mkGatedStep {
      inherit step label parent prompt branches;
    }
    else step;
}
