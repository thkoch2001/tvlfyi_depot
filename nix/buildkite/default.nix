...
rec
  ...
  shouldSkip =
    ...
    drvPath:
    if
      ( hasAttr label parentTargetMap ) && parentTargetMap."${ label }".drvPath == drvPath
    then
  ...
  makePipelineChunk =
    ...
    chunkId:
    chunk:
    rec { filename = "${ name }-chunk-${ chunkId }.json"; path = writeText filename ( toJSON { steps = chunk; } ); };
  ...
  pipelineChunks = name: steps: attrValues ( mapAttrs ( makePipelineChunk name ) ( chunksOf 256 steps ) );
                ...
                attrValues
                  (
                    mapAttrs
                      (
                        name:
                        value: { inherit name value; postStep = ( value ? prompt ) || ( value.postBuild or false ); }
                      )
                      ( target.meta.ci.extraSteps or { } )
                  )
              );
          mkExtraStep' = { name, value, ... }: mkExtraStep step name value;
          extraBuildSteps = map mkExtraStep' splitExtraSteps.wrong;
          # 'wrong' -> no prompt
          extraPostSteps = map mkExtraStep' splitExtraSteps.right;
          # 'right' -> has prompt
        in
        { buildSteps = [ step ] ++ extraBuildSteps; postSteps = extraPostSteps; };
      # Combine all target steps into separate build and post-build step lists.
      steps =
        foldl'
          ( acc: t: { buildSteps = acc.buildSteps ++ t.buildSteps; postSteps = acc.postSteps ++ t.postSteps; } )
                  ...
                  (
                    target:
                    {
                      ...
                      value = {
                        ...
                        attrPath = target.__readTree ++ lib.optionals ( target ? __subtarget ) [ target.__subtarget ];
                      };
                    }
                  )
                  drvTargets
  ...
  mkExtraStep =
    ...
    in
    if ( isString prompt ) then mkGatedStep { inherit step label parent prompt; } else step;
}
