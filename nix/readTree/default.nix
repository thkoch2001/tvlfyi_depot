...
let
  ...
  importFile = args: scopedArgs: path: parts: filter:
    let
      importedFile = if scopedArgs != { } then
        builtins.scopedImport scopedArgs path
      else
        import path;
      pathType = builtins.typeOf importedFile;
    in if pathType != "lambda" then
      builtins.throw "readTree: trying to import ${
        toString path
  ...
  readTree = { args, initPath, rootDir, parts, argsFilter, scopedArgs }:
    let
      ...
      self = if rootDir then {
        ...
        importFile args scopedArgs initPath parts argsFilter;
      ...
      children = if hasAttr ".skip-subtree" dir then
        [ ]
      else
        map (c: {
          name = c;
    ...
    in if isAttrs nodeValue then
      nodeValue // allChildren // (marker parts allChildren)
    else
      nodeValue;
...
in {
  ...
  __functor = _:
    { path, args, filter ? (_parts: x: x), scopedArgs ? { } }:
    readTree {
