...
let
  inherit
    ...
    readDir
    substring;
  argsWithPath = args: parts: let meta.locatedAt = parts; in meta // ( if isAttrs args then args else args meta );
  readDirVisible =
    path:
    let
    ...
    in
    listToAttrs ( map ( name: { inherit name; value = children.${ name }; } ) names );
  # Create a mark containing the location of this attribute and
  # a list of all child attribute names added by readTree.
  marker = parts: children: { __readTree = parts; __readTreeChildren = builtins.attrNames children; };
  ...
  importFile =
    ...
    filter:
    let
      importedFile = if scopedArgs != { } then builtins.scopedImport scopedArgs path else import path;
  ...
  nixFileName = file: let res = match "(.*)\\.nix" file; in if res == null then null else head res;
  readTree =
    { args
    , initPath
    , rootDir
    ...
    let
      ...
      self = if rootDir then { __readTree = [ ]; } else importFile args scopedArgs initPath parts argsFilter;
      ...
      # Import Nix files
      nixFiles =
        if hasAttr ".skip-subtree" dir then [ ] else filter ( f: f != null ) ( map nixFileName ( attrNames dir ) );
      nixChildren =
        map
          (
            c:
            ...
            in
            { name = c; value = if isAttrs imported then imported // marker childParts { } else imported; }
          )
          nixFiles;
    ...
    in
    if isAttrs nodeValue then nodeValue // allChildren // ( marker parts allChildren ) else nodeValue;
...
in
{
  ...
  __functor =
    ...
    readTree { inherit args scopedArgs; argsFilter = filter; initPath = path; rootDir = true; parts = [ ]; };
  ...
  restrictFolder =
    ...
    args:
    if
      ( elemAt parts 0 ) == folder || elem parts exceptions
    then
      args
    else
      args
