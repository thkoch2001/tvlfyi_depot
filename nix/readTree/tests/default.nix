...
let
  ...
  example = it "corresponds to the README example" [
    (assertEq "third_party attrset" (lib.isAttrs tree-ex.third_party
      && (!lib.isDerivation tree-ex.third_party)) true)
    (assertEq "third_party attrset other attribute"
      tree-ex.third_party.favouriteColour "orange")
    (assertEq "rustpkgs attrset aho-corasick"
      tree-ex.third_party.rustpkgs.aho-corasick "aho-corasick")
    (assertEq "rustpkgs attrset serde" tree-ex.third_party.rustpkgs.serde
      "serde")
  ...
  traversal-logic = it "corresponds to the traversal logic in the README" [
    ...
    (assertEq "skip subtree a/default.nix is skipped" (tree-tl.skip-subtree ? a)
      false)
    (assertEq "skip subtree b/c.nix is skipped" (tree-tl.skip-subtree ? b)
      false)
    (assertEq "skip subtree a/default.nix would be read without .skip-subtree"
      (tree-tl.no-skip-subtree.a) "am I subtree yet?")
    (assertEq "skip subtree b/c.nix would be read without .skip-subtree"
      (tree-tl.no-skip-subtree.b.c) "cool")
    ...
    (assertEq "default.nix means sibling isn’t read"
      (tree-tl.default-nix ? sibling) false)
    (assertEq
      "default.nix means subdirs are still read and merged into default.nix"
      (tree-tl.default-nix.subdir.a) "but I’m picked up")
    ...
    (assertEq "default.nix is not an attrset -> children are not merged"
      (tree-tl.default-nix.no-merge ? subdir) false)
    ...
    (assertEq
      ...
      tree-tl.default-nix.can-be-drv.subdir.a "Picked up through the drv")
  ...
  wrong = it "cannot read these files and will complain" [
    (assertThrows "this file is not a function" (depot.nix.readTree {
...
in runTestsuite "readTree" [ example traversal-logic wrong markers ]
