...
let
  ...
  assertIsTag = tag:
    let res = verifyTag tag;
    in assert res.isTag || throw res.errmsg; { inherit (res) name val; };
  ...
  discrDef = defTag: fs: v:
    let res = lib.findFirst (t: t.val v) null (map assertIsTag fs);
    in if res == null then { ${defTag} = v; } else { ${res.name} = v; };

  # Like `discrDef`, but fail if there is no match.
  discr = fs: v:
    let res = discrDef null fs v;
    in assert lib.assertMsg (res != null)
      "tag.discr: No predicate found that matches ${
  ...
  match = sum: matcher:
    ...
    in assert let len = builtins.length cases;
    in lib.assertMsg (len == 1) ("match: an instance of a sum is an attrset "
      + "with exactly one element, yours had ${toString len}"
      + ", namely: ${lib.generators.toPretty { } cases}");
    let case = builtins.head cases;
    in assert lib.assertMsg (matcher ? ${case})
      (''match: "${case}" is not a valid case of this sum, ''
        + "the matcher accepts: ${
          lib.generators.toPretty { } (builtins.attrNames matcher)
        }");
  ...
  tests = import ./tests.nix {
    inherit depot lib verifyTag discr discrDef match matchLam;
  };

in { inherit verifyTag tagName tagValue discr discrDef match matchLam tests; }
