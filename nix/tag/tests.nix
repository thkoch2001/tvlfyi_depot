...
let
  ...
  discr-test = it "can discr things" [
    ...
    (assertEq "bools here, ints there" (discr [
      { bool = lib.isBool; }
      { int = lib.isInt; }
    ] 25) { int = 25; })
    (assertEq "bools here, ints there 2" (discr [
      { bool = lib.isBool; }
      { int = lib.isInt; }
    ] true) { bool = true; })
    (assertEq "fallback to default" (discrDef "def" [
      { bool = lib.isBool; }
      { int = lib.isInt; }
    ] "foo") { def = "foo"; })
    (assertThrows "throws failing to match" (discr [ { fish = x: x == 42; } ] 21))
  ];
