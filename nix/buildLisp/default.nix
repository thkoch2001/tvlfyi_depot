...
let
  ...
  allDeps =
    ...
    let
      ...
      # (mainly bundled).
      deps' =
        builtins.map ( dep: dep."${ impl.name }" or ( dep.overrideLisp ( _: { implementation = impl; } ) ) ) deps;
    in
  ...
  makeOverridable = f: orig: ( f orig ) // { overrideLisp = new: makeOverridable f ( orig // ( new orig ) ); };
              ...
              passthru =
                ( old.passthru or { } )
                  // {
                    ...
                    meta = ( old.passthru.meta or { } ) // { inherit targets; };
                  }
                  // builtins.listToAttrs
                    (
                      builtins.map
                        (
                          impl: { inherit ( impl ) name; value = self.overrideLisp ( _: { implementation = impl; } ); }
                        )
  ...
  testSuite =
    ...
    runCommandNoCC
      name
      { LD_LIBRARY_PATH = lib.makeLibraryPath lispNativeDeps; LANG = "C.UTF-8"; }
      ''
      echo "Running test suite ${ name }"

      ${ implementation.runScript } ${ implementation.genTestLisp { inherit name expression; srcs = filteredSrcs; deps = lispDeps; } } | tee $out
      ...
      ( name: v: { inherit name; } // v )
      {
        ...
        ccl = {
          ...
          faslExt =
            /**/
            if
              targetPlatform.isPowerPC && targetPlatform.is32bit
            then
              "pfsl"
            else
              if
                targetPlatform.isPowerPC && targetPlatform.is64bit
              then
                "p64fsl"
              else
                if
                ...
                then
                  ...
                  if
                    targetPlatform.isx86_32 && targetPlatform.isLinux
                  then
                    "lx32fsl"
                  else
                    if
                      targetPlatform.isAarch32 && targetPlatform.isLinux
                    then
                      "lafsl"
                    else
                      if
                        targetPlatform.isx86_32 && targetPlatform.isDarwin
                      then
                        "dx32fsl"
                      else
                        if
                          targetPlatform.isx86_64 && targetPlatform.isDarwin
                        then
                          "dx64fsl"
                        else
                          if
                          ...
                          then
                            ...
                            if
                              targetPlatform.isx86_32 && targetPlatform.isFreeBSD
                            then
                              "fx32fsl"
                            else
                              if
                                targetPlatform.isx86_64 && targetPlatform.isFreeBSD
                              then
                                "fx64fsl"
                              else
                                if
                                  targetPlatform.isx86_32 && targetPlatform.isWindows
                                then
                                  "wx32fsl"
                                else
                                  if
                                    targetPlatform.isx86_64 && targetPlatform.isWindows
                                  then
                                    "wx64fsl"
                                  else
  ...
  library =
    ...
    , brokenOn ? [ ]
      # TODO(sterni): make this a warning
    , srcs
    , deps ? [ ]
    , native ? [ ]
    , tests ? null
      ...
      (
        ...
        runCommandNoCC
          "${ name }-cllib"
          {
            ...
            LANG = "C.UTF-8";
            passthru =
              passthru // { inherit lispNativeDeps lispDeps; lispName = name; lispBinary = false; tests = testDrv; };
          }
          ''
          ...
          ${ implementation.runScript } ${ implementation.genCompileLisp { srcs = filteredSrcs; inherit name; deps = lispDeps; } }
          ''
      );
  ...
  program =
    ...
    let
      ...
      selfLib = ( makeOverridable library ) { inherit name native brokenOn; deps = lispDeps; srcs = filteredSrcs; };
      testDrv =
        if
          !isNull tests
        then
      ...
      (
        ...
        runCommandNoCC
          ...
          (
            ...
            ${ implementation.runScript } ${ implementation.genDumpLisp { inherit name main; deps = ( [ selfLib ] ++ lispDeps ); } }
            ''
              + lib.optionalString
                implementation.wrapProgram
                ''
