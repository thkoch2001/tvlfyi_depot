...
let
  ...
  inherit (pkgs)
    lib runCommandNoCC makeWrapper writeText writeShellScriptBin sbcl ecl-static
    ccl;
  ...
  #  ];
  implFilter = impl: xs:
    let isFilterSet = x: builtins.isAttrs x && !(lib.isDerivation x);
    in builtins.map
    (x: if isFilterSet x then x.${impl.name} or x.default else x)
    (builtins.filter (x: !(isFilterSet x) || x ? ${impl.name} || x ? default)
      xs);
  ...
  genTestLispGeneric = impl:
    { name, srcs, deps, expression }:
    writeText "${name}.lisp" ''
      ;; Dependencies
  ...
  allDeps = impl: deps:
    let
      ...
      # (mainly bundled).
      deps' = builtins.map (dep:
        dep."${impl.name}" or (dep.overrideLisp
          (_: { implementation = impl; }))) deps;
    in (lib.toposort dependsOn
      (lib.unique (lib.flatten (deps' ++ (map (d: d.lispDeps) deps'))))).result;
      ...
      drv.overrideLisp (old:
        ...
        in {
          passthru = (old.passthru or { }) // {
            ...
            inherit (impl) name;
            value = self.overrideLisp (_: { implementation = impl; });
  ...
  testSuite =
    { name, expression, srcs, deps ? [ ], native ? [ ], implementation }:
    let
  ...
  impls = lib.mapAttrs (name: v: { inherit name; } // v) {
      ...
      lispWith = deps:
        ...
        in writeShellScriptBin "sbcl" ''
          export LD_LIBRARY_PATH="${
            lib.makeLibraryPath (allNative [ ] lispDeps)
          }"
          export LANG="C.UTF-8"
          exec ${sbcl}/bin/sbcl ${
    ...
    ccl = {
      ...
      faslExt = if targetPlatform.isPowerPC && targetPlatform.is32bit then
        "pfsl"
      else if targetPlatform.isPowerPC && targetPlatform.is64bit then
        "p64fsl"
  ...
  library = { name, implementation ? defaultImplementation
    , brokenOn ? [ ] # TODO(sterni): make this a warning
    , srcs, deps ? [ ], native ? [ ], tests ? null, passthru ? { } }:
    let
    ...
    in lib.fix (self:
      runCommandNoCC "${name}-cllib" {
        ...
        ${if !isNull testDrv then
          "echo 'Test ${testDrv} succeeded'"
        else
          "echo 'No tests run'"}
  ...
  program = { name, implementation ? defaultImplementation
    , brokenOn ? [ ] # TODO(sterni): make this a warning
    , main ? "${name}:main", srcs, deps ? [ ], native ? [ ], tests ? null
    , passthru ? { } }:
    let
      filteredSrcs = implFilter implementation srcs;
      filteredDeps = implFilter implementation deps;
      ...
      testDrv = if !isNull tests then
        testSuite {
          name = tests.name or "${name}-test";
          srcs = ( # testSuite does run implFilter as well
