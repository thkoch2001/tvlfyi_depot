...
let
  ...
  # TODO(sterni): make allDeps unavailable
  ...
  testSuite = { name, expression, srcs, deps ? [], native ? [], impl }:
    let
      ...
      lispDeps = allDepsForImpl impl (implFilter impl deps);
  ...
  impls = lib.mapAttrs (name: v: { inherit name; } // v) {
    sbcl = {
      ...
      lispWith = deps:
        let lispDeps = filter (d: !d.lispBinary) (allDepsForImpl impls.sbcl deps);
  ...
  library =
    ...
    let
      ...
      lispDeps = allDepsForImpl impl filteredDeps;
...
in {
  ...
  sbclWith = deps: impls.sbcl.lispWith (allDepsForImpl impls.sbcl deps);
}
