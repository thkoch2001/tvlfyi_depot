...
let
  ...
  #
  # Type: path -> int
  depth = path: lib.pipe path [ toString ( builtins.split "/" ) ( builtins.filter builtins.isList ) builtins.length ];
  ...
  makeSymlink =
    ...
    in
    assert toString root == prefix; { src = contextPath; dst = belowRoot; };
...
pkgs.runCommandNoCC
  "sparse-${ builtins.baseNameOf root }"
  { }
  (
