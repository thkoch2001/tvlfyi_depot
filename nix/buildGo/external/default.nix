...
let
  inherit ( builtins ) elemAt foldl' fromJSON head length listToAttrs readFile replaceStrings tail throw;
  ...
  stdlibPackages =
    runCommand
      ...
      '';
  analyser =
    program { name = "analyser"; srcs = [ ./main.go ]; x_defs = { "main.stdlibList" = "${ stdlibPackages }"; }; };
  mkset = path: value: if path == [ ] then { gopkg = value; } else { "${ head path }" = mkset ( tail path ) value; };
    ...
    let
      ...
      foreignDeps =
        map
          ...
          entry.foreignDeps;
      args = { srcs = map ( f: src + ( "/" + f ) ) entry.files; deps = localDeps ++ foreignDeps; };
      libArgs =
        args
          // {
      ...
      binArgs = args // { name = ( last ( ( lib.splitString "/" path ) ++ entry.locator ) ); };
    in
...
let
  ...
  depMap = listToAttrs ( map ( d: { name = d.goImportPath; value = d; } ) ( map ( d: d.gopkg ) deps ) );
