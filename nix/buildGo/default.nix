...
let
  inherit (builtins)
    attrNames baseNameOf dirOf elemAt filter listToAttrs map match readDir
  ...
  srcBasename = src:
    elemAt (match "([a-z0-9]{32}-)?(.*.go)" (baseNameOf src)) 1;
  ...
  xFlags = x_defs:
    spaceOut (map (k: "-X ${k}=${x_defs."${k}"}") (attrNames x_defs));
  ...
  program = { name, srcs, deps ? [ ], x_defs ? { } }:
    ...
    in runCommand name { } ''
      ${go}/bin/go tool compile -o ${name}.a -trimpath=$PWD -trimpath=${go} ${
        includeSources uniqueDeps
      } ${spaceOut srcs}
      mkdir -p $out/bin
      export GOROOT_FINAL=go
      ${go}/bin/go tool link -o $out/bin/${name} -buildid nix ${
        xFlags x_defs
      } ${includeLibs uniqueDeps} ${name}.a
    '';
  ...
  package = { name, srcs, deps ? [ ], path ? name, sfiles ? [ ] }:
    let
      ...
      asmBuild = ifAsm ''
        ${go}/bin/go tool asm -trimpath $PWD -I $PWD -I ${go}/share/go/pkg/include -D GOOS_linux -D GOARCH_amd64 -gensymabis -o ./symabis ${
          spaceOut sfiles
        }
        ${go}/bin/go tool asm -trimpath $PWD -I $PWD -I ${go}/share/go/pkg/include -D GOOS_linux -D GOARCH_amd64 -o ./asm.o ${
          spaceOut sfiles
        }
      '';
      ...
      gopkg = (runCommand "golib-${name}" { } ''
        ...
        ${asmBuild}
        ${go}/bin/go tool compile -pack ${asmLink} -o $out/${path}.a -trimpath=$PWD -trimpath=${go} -p ${path} ${
          includeSources uniqueDeps
        } ${spaceOut srcs}
        ${asmPack}
      '') // {
  ...
  protoLibs = import ./proto.nix { inherit external; };

  # Build a Go library out of the specified protobuf definition.
  proto = { name, proto, path ? name, goPackage ? name, extraDeps ? [ ] }:
    (makeOverridable package) {
      inherit name path;
      ...
      srcs = lib.singleton (runCommand "goproto-${name}.pb.go" { } ''
        ...
        ${protobuf}/bin/protoc --plugin=${protoLibs.goProto.protoc-gen-go.gopkg}/bin/protoc-gen-go \
          --go_out=plugins=grpc,import_path=${baseNameOf path}:. ${
            baseNameOf proto
          }
