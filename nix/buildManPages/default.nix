...
let
  ...
  bins = getBins mandoc [ "mandoc" ] // getBins gzip [ "gzip" ] // getBins coreutils [ "mkdir" "ln" "cp" ];
  defaultGzip = true;
  basename = gzip: { name, section, ... }: "${ name }.${ toString section }${ lib.optionalString gzip ".gz" }";
        ...
        [
          ...
          ++ (
            if
              gzip
            then
              [ "redirfd" "-w" "1" "$out" bins.gzip "-c" source ]
            else
...
in
{ __functor = _: buildManPages; single = buildManPage; }
