...
let
  bins =
    getBins pkgs.execline [ "execlineb" { use = "if"; as = "execlineIf"; } "redirfd" "importas" "exec" ]
      // getBins pkgs.s6-portable-utils [ "s6-cat" "s6-grep" "s6-touch" "s6-test" "s6-chmod" ];
in
  ...
  (
    derivationArgs
      // {
        ...
        _runExeclineScript = let in escapeExecline execline;
        _runExeclineStdin = stdin;
        passAsFile = [ "_runExeclineScript" "_runExeclineStdin" ] ++ derivationArgs.passAsFile or [ ];
        # the default, exec acts as identity executable
        builder = builderWrapper;
        args = [
          bins.importas
          # import script file as $script
          "-ui"
          # drop the envvar afterwards
          "script"
          # substitution name
          "_runExeclineScriptPath"
          # passed script file
          bins.importas
          ...
          # read the file
          "0"
          # into the stdin of execlineb
          "$stdin"
          # that was given via stdin
          bins.execlineb
          ...
          "-S0"
          # set $@ inside the execline script
          "-W"
          # die on syntax error
          "$script"
          # substituted by importas
        ];
      }
  )
