...
let
  bins =
    getBins coreutils [ "mv" ]
      // getBins pkgs.execline [ "execlineb" { use = "if"; as = "execlineIf"; } "redirfd" "importas" ]
      // getBins pkgs.s6-portable-utils [ "s6-chmod" "s6-grep" "s6-touch" "s6-cat" "s6-test" ];
  ...
  basic = runExeclineLocal "run-execline-test-basic" { } [ "importas" "-ui" "out" "out" "${ bins.s6-touch }" "$out" ];
  ...
  wrapWithVar =
    runExeclineLocal
      "run-execline-test-wrap-with-var"
      {
        builderWrapper =
          writeScript
            "var-wrapper"
