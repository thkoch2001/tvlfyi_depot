...
let
  ...
  linkTo = name: path:
    depot.nix.runExecline.local name { } [
      "importas"
      "out"
      "out"
      bins.s6-ln
  ...
  rustSimple = args@{ name, ... }:
    src:
    linkTo name "${rustSimpleBin args src}/bin/${name}";

  # Like `rustSimple`, but put the binary in `$out/bin/`.
  rustSimpleBin = { name, dependencies ? [ ], doCheck ? true, }:
    src:
    (if doCheck then testRustSimple else pkgs.lib.id) (pkgs.buildRustCrate ({
      pname = name;
      version = "1.0.0";
      crateName = name;
      crateBin = [ name ];
  ...
  rustSimpleLib = { name, dependencies ? [ ], doCheck ? true, }:
    src:
    (if doCheck then testRustSimple else pkgs.lib.id) (pkgs.buildRustCrate ({
      pname = name;
      version = "1.0.0";
...
in { inherit rustSimple rustSimpleBin rustSimpleLib; }
