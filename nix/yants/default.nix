  ...
  typedef' =
    ...
    }:
    {
      ...
      __functor =
        ...
        in
        if checkToBool result then value else throw (toError value result);
    };

  typeError =
    type: val: "expected type '${type}', but value '${prettyPrint val}' is of type '${typeOf val}'";
    ...
    typedef' {
      ...
      checkType =
        ...
        let
          res = check v;
        in
        { ok = res; } // (lib.optionalAttrs (!res) { err = typeError name v; });
    };
  ...
  checkEach =
    ...
    foldl'
      (
        ...
        {
          ok = acc.ok && isT;
          err = if isT then acc.err else acc.err + "${prettyPrint e}: ${t.toError e res}\n";
        }
      )
      {
        ok = true;
...
lib.fix (self: {
  ...
  function = typedef "function" (
    x: isFunction x || (isAttrs x && x ? "__functor" && isFunction x.__functor)
  );
      ...
      plain =
        ...
        typedef' {
          ...
          checkType = (
            ...
            in
            if isAttrs x && length (attrNames x) == 1 && hasAttr variant def then
              let
                t = def."${variant}";
                v = x."${variant}";
                res = t.checkType v;
              in
              if t.checkToBool res then
                { ok = true; }
              else
                {
                  ok = false;
                  err = "while checking '${name}' variant '${variant}': " + t.toError v res;
                }
            else
              {
                ok = false;
