  ...
  typedef' =
    ...
    }:
    {
      ...
      __functor =
        self:
        value:
        let result = self.checkType value; in if checkToBool result then value else throw ( toError value result );
    };
    ...
    typedef'
      {
        inherit name;
        checkType =
          v: let res = check v; in { ok = res; } // ( lib.optionalAttrs ( !res ) { err = typeError name v; } );
      };
  checkEach =
    name:
    t:
    ...
    foldl'
      (
        ...
        in
        { ok = acc.ok && isT; err = if isT then acc.err else acc.err + "${ prettyPrint e }: ${ t.toError e res }\n"; }
      )
...
lib.fix
  (
    self:
    {
      ...
      function = typedef "function" ( x: isFunction x || ( isAttrs x && x ? "__functor" && isFunction x.__functor ) );
        ...
        typedef
          ...
          (
            ...
            isAttrs x
              ...
              && hasAttr "checkType" x
              && self.function.check x.checkType
              && hasAttr "checkToBool" x
          ...
          rec
            ...
            checkType =
              ...
              in
              {
                ...
                err = "expected type ${ name }, but value does not conform to '${ t.name }': " + t.toError v res;
              };
          };
      eitherN =
        tn:
        typedef
          "either<${ concatStringsSep ", " ( map ( x: x.name ) tn ) }>"
      ...
      list =
        ...
        typedef'
          rec
            ...
            name = "list<${ t.name }>";
            checkType =
              v: if isList v then checkEach name ( self.type t ) v else { ok = false; err = typeError name v; };
          };
      attrs =
        t:
        typedef'
        ...
        let
            ...
            then
              ...
              then
                ...
                checkExtraneous
                  def
                  ( tail has )
                  { ok = false; err = acc.err + "unexpected struct field '${ head has }'\n"; };
          ...
          checkStruct =
            ...
            let
              ...
              checkedFields =
                map
                  ( n: let v = if hasAttr n value then value."${ n }" else null; in checkField def."${ n }" n v )
                  ( attrNames def );
              combined =
                foldl'
                  ( acc: res: { ok = acc.ok && res.ok; err = if !res.ok then acc.err + res.err else acc.err; } )
                  init
                  checkedFields;
            in
            { ok = combined.ok && extraneous.ok; err = combined.err + extraneous.err; };
          struct' =
            name:
            def:
            typedef'
              {
                ...
                checkType =
                  ...
                  then
                    ...
                    { ok = false; err = typeError name value; };
            ...
            typedef'
              {
                ...
                checkType = (
                  ...
                  in
                  if
                    isAttrs x && length ( attrNames x ) == 1 && hasAttr variant def
                  then
                    let
                      t = def."${ variant }";
                      v = x."${ variant }";
                    ...
                    then
                      ...
                      { ok = false; err = "while checking '${ name }' variant '${ variant }': " + t.toError v res; }
                  else
                    { ok = false; err = typeError name x; }
                );
              };
          sum' =
            name:
      ...
      defun =
        let
          mkFunc =
            ...
            f:
            {
              inherit sig;
              __toString =
                self: foldl' ( s: t: "${ s } -> ${ t.name }" ) "λ :: ${ ( head self.sig ).name }" ( tail self.sig );
              __functor = _: f;
            };
          defun' =
            sig:
        ...
        in
        sig:
        func: if length sig < 2 then ( throw "Signature must at least have two types (a -> b)" ) else defun' sig func;
      # Restricting types
      #
            ...
            checkType =
              ...
              then
                ...
                if
                  isBool iok
                then
                  { ok = iok; err = "${ prettyPrint v } does not conform to restriction '${ restriction }'"; }
                else
                  # use throw here to avoid spamming the build log
                  throw
                    "restriction '${ restriction }' predicate returned unexpected value '${ prettyPrint iok }' instead of boolean";
          };
    }
  )
