...
let
  ...
  storePathName =
    ...
    then
      ...
      then
        ...
        if
          builtins.isString p || ( builtins.isAttrs p && ( p ? outPath || p ? __toString ) )
        then
          let
            strPath = toString p;
  ...
  pathType =
    ...
    in
    {
      ${ thisPathType } =
        /**/
        if thisPathType != "symlink" then true else if isSymlinkDir then "directory" else "regular-or-missing";
    };
  pathType' =
    path:
    let
  ...
  /*
   ...
   Type: path(-like) -> bool
   */
  realPathIsDirectory = path: let pt = pathType' path; in pt ? directory || pt.symlink or null == "directory";
  /*
...
in
{ inherit storePathName pathType isDirectory realPathIsDirectory isRegularFile isSymlink; }
