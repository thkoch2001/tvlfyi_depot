...
let
  ...
  mergePatch = target: patch:
    if lib.isAttrs patch then
      ...
      in foldlAttrs (acc: patchEl:
        if patchEl.value == null then
          removeAttrs acc [ patchEl.name ]
        else
          acc // {
            ${patchEl.name} =
              mergePatch (acc.${patchEl.name} or "unnused") patchEl.value;
          }) target' patch
    else
      patch;
  ...
  tests = let
    ...
    nonAttrs = it "one side is a non-attrset value" [
      (assertEq "target is a value means the value is replaced by the patch"
        (mergePatch 42 testPatch) (mergePatch { } testPatch))
      (assertEq "patch is a value means it replaces target alltogether"
        (mergePatch testTarget 42) 42)
    ];
    ...
    rfcTests = let
      r = index: target: patch: res:
        (assertEq "test number ${toString index}" (mergePatch target patch)
          res);
  ...
  in runTestsuite "mergePatch" [ emptyPatch nonAttrs rfcExamples rfcTests ];
