...
let
  bins =
    depot.nix.getBins pkgs.coreutils [ "printf" "echo" "cat" "printenv" "tee" ]
  ...
  debugExec =
    ...
    depot.nix.writeExecline
      "debug-exec"
      { }
      [ "if" [ "fdmove" "-c" "1" "2" "if" [ bins.printf "%s: " msg ] "if" [ bins.echo "$@" ] ] "$@" ];
  # Print stdin to stderr and stdout
  eprint-stdin = depot.nix.writeExecline "eprint-stdin" { } [ "pipeline" [ bins.multitee "0-1,2" ] "$@" ];
...
in
{ inherit debugExec eprint-stdin eprint-stdin-netencode eprintenv split-stdin runInEmptyEnv; }
