...
let
  ...
  */
  toINI =
    ...
    let
      ...
      mkSections = lib.strings.concatMapStringsSep "\n" ({ name, value }: mkSection name value) sections;
      mkGlobalSection =
        if globalSection == [ ] then "" else toKeyValue { inherit mkKeyValue; } globalSection + "\n";
    in
    mkGlobalSection + mkSections;
  ...
  */
  toKeyValue =
    ...
    in
    nameValues:
    lib.strings.concatStrings (lib.concatLists (map ({ name, value }: mkLines name value) nameValues));
in
toINI
