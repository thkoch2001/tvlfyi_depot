...
let
  rev =
    depot.nix.writeExecline
      ...
      { }
      [ "pipeline" [ "${ pkgs.curl }/bin/curl" "-L" "https://packdeps.haskellers.com/reverse" ] rev-hs ];
  rev-hs =
    pkgs.writers.writeHaskell
      "revers-haskell-deps-hs"
      { libraries = [ pkgs.haskellPackages.nicify-lib pkgs.haskellPackages.tagsoup ]; }
