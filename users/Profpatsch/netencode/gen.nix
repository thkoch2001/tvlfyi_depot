...
let
  netstring = tag: suffix: s: "${ tag }${ toString ( builtins.stringLength s ) }:${ s }${ suffix }";
  unit = "u,";
    ...
    let
      match = {
        ...
        "set" =
          ...
          if
            attrs.type or "" == "derivation"
          then
            text "${ attrs }"
          else
            record ( lib.mapAttrsToList ( k: v: { key = k; val = dwim v; } ) attrs );
        "list" = l: list ( map dwim l );
      };
    in
    match.${ builtins.typeOf val } val;
in
{ inherit unit n1 n3 n6 n7 i3 i6 i7 text binary tag record list dwim; }
