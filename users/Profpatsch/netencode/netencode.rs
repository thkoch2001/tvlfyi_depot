...
pub fn t_from_stdin_or_die_user_error_with_rest<'a>(prog_name: &'_ str, initial_bytes: &[u8]) -> Option<(Vec<u8>, T)> {
        ...
        match parse::t_t(&parser_vec) {
            ...
            Err(nom::Err::Incomplete(Needed)) => {
                if was_empty {
                    exec_helpers::die_user_error(
                        prog_name,
                        &format!("unable to parse netencode from stdin, input incomplete: {:?}", parser_vec)
                    );
                }
