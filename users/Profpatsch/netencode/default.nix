...
let
  netencode-rs =
    depot.nix.writers.rustSimpleLib
      {
        name = "netencode";
        dependencies = [ depot.third_party.rust-crates.nom depot.users.Profpatsch.execline.exec-helpers ];
      }
      ( builtins.readFile ./netencode.rs );
  gen = import ./gen.nix { inherit lib; };
  pretty-rs =
    depot.nix.writers.rustSimpleLib
      { name = "netencode-pretty"; dependencies = [ netencode-rs ]; }
      ( builtins.readFile ./pretty.rs );
  pretty =
    depot.nix.writers.rustSimple
      {
        name = "netencode-pretty";
        dependencies = [ netencode-rs pretty-rs depot.users.Profpatsch.execline.exec-helpers ];
      }
      ''
    ...
    depot.nix.writers.rustSimple
      {
        ...
        dependencies = [ depot.users.Profpatsch.arglib.netencode.rust netencode-rs depot.third_party.rust-crates.mustache ];
      }
  ...
  record-get =
    depot.nix.writers.rustSimple
      {
        name = "record-get";
        dependencies = [ netencode-rs depot.users.Profpatsch.execline.exec-helpers depot.users.Profpatsch.arglib.netencode.rust ];
      }
      ''
      extern crate netencode;
  ...
  record-splice-env =
    depot.nix.writers.rustSimple
      { name = "record-splice-env"; dependencies = [ netencode-rs depot.users.Profpatsch.execline.exec-helpers ]; }
      ''
      extern crate netencode;
      extern crate exec_helpers;
  ...
  env-splice-record =
    depot.nix.writers.rustSimple
      { name = "env-splice-record"; dependencies = [ netencode-rs depot.users.Profpatsch.execline.exec-helpers ]; }
      ''
      extern crate netencode;
      extern crate exec_helpers;
      use netencode::{T};
...
depot.nix.readTree.drvTargets
  { inherit netencode-rs pretty-rs pretty netencode-mustache record-get record-splice-env env-splice-record gen; }
