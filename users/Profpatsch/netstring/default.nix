...
let
  toNetstring = s: "${ toString ( builtins.stringLength s ) }:${ s },";
  toNetstringList = xs: lib.concatStrings ( map toNetstring xs );
  toNetstringKeyVal =
    attrs: lib.concatStrings ( lib.mapAttrsToList ( k: v: toNetstring ( toNetstring k + toNetstring v ) ) attrs );
  python-netstring =
    depot.users.Profpatsch.writers.python3Lib
      { name = "netstring"; }
      ''
...
depot.nix.readTree.drvTargets
  { inherit toNetstring toNetstringList toNetstringKeyVal python-netstring rust-netstring; }
