...
let
  bins =
    depot.nix.getBins pkgs.s6-portable-utils [ "s6-mkdir" "s6-cat" "s6-ln" "s6-ls" "s6-touch" ]
  ...
  FlakeError =
    restrict "flake error" ( s: lib.any ( prefix: ( builtins.substring 0 1 s ) == prefix ) [ "E" "W" ] ) string;
  ...
  python3Lib =
    ...
    pkgs.python3Packages.buildPythonPackage
      { inherit name; src = srcTree; propagatedBuildInputs = libraries pkgs.python3Packages; doCheck = false; };
in
{ inherit python3 python3Lib; }
