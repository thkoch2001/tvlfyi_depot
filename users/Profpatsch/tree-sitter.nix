...
let
  bins =
    depot.nix.getBins pkgs.coreutils [ "head" "printf" "cat" ]
  ...
  print-ast =
    depot.nix.writers.rustSimple
      { name = "print-ast"; dependencies = with depot.third_party.rust-crates; [ libloading tree-sitter ]; }
      ''
    ...
    depot.nix.writers.rustSimple
      {
        ...
        dependencies = [ depot.third_party.rust-crates.inotify depot.users.Profpatsch.netstring.rust-netstring ];
      }
      ''
...
in
depot.nix.readTree.drvTargets { inherit print-ast tree-sitter-nix print-nix-file-on-update watch-file-modified; }
