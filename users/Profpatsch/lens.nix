...
let
  ...
  lens =
    strong: get: set: pab:
    lensP strong (s: tuple (get s) (b: set s b)) pab;

  # lensP :: (s -> (a, b -> t)) -> Lens s t a b
  lensP =
    strong: to: pab:
    strong.dimap to ({ fst, snd }: snd fst) (strong.firstP pab);
  ...
  optic =
    accessors: profunctorSubclass:
    builtins.foldl' comp id (map (accessor: accessor profunctorSubclass) accessors);
in
{
  inherit
    id
