...
let
  ...
  lens = strong: get: set: pab: lensP strong ( s: tuple ( get s ) ( b: set s b ) ) pab;
  # lensP :: (s -> (a, b -> t)) -> Lens s t a b
  lensP = strong: to: pab: strong.dimap to ( { fst, snd }: snd fst ) ( strong.firstP pab );
  ...
  field = name: strong: lens strong ( attrs: attrs.${ name } ) ( attrs: a: attrs // { ${ name } = a; } );
  # Setter :: (->) a b -> (->) s t
  # Setter :: (a -> b) -> (s -> t)
  ...
  optic =
    accessors: profunctorSubclass: builtins.foldl' comp id ( map ( accessor: accessor profunctorSubclass ) accessors );
in
{ inherit id _ const comp Profunctor profunctorFun Strong strongFun iso lens optic _1 _2 field tuple swap over set; }
