      ...
      os =
        depot.ops.nixos.nixosFor
          (
            _:
            {
              imports = [ "${ pkgs.path }/nixos/modules/virtualisation/google-compute-image.nix" configuration ];
              networking.hostName = name;
              fileSystems."/nix" = { device = "/dev/disk/by-label/google-${ name }-disk"; fsType = "ext4"; };
            }
          );
            ...
            lib.recursiveUpdate
              extraConfig
              {
                ...
                resource.google_compute_instance."${ name }" = {
                  ...
                  tags = [ "http-server" "https-server" "${ name }-firewall" ];
                  boot_disk = {
                    device_name = "boot";
                    initialize_params = { size = 10; image = "projects/nixos-cloud/global/images/${ nixosImage.name }"; };
                  };
                  attached_disk = { source = "\${google_compute_disk.${ name }.id}"; device_name = "${ name }-disk"; };
                  network_interface = { network = "default"; subnetwork = "default"; access_config = { }; };
                ...
                resource.tls_private_key."${ name }" = { algorithm = "ECDSA"; ecdsa_curve = "P384"; };
                ...
                resource.google_compute_disk."${ name }" = { inherit zone; name = "${ name }-disk"; size = 100; };
                resource.null_resource.deploy_nixos = {
                  ...
                  connection = { host = "\${google_compute_instance.${ name }.network_interface[0].access_config[0].nat_ip}"; };
                  provisioner = [
