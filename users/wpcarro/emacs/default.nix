...
let
  ...
  emacsBinPath =
    makeBinPath ( wpcarro.common.shell-utils ++ ( with pkgs; [ clipmenu ispell nix pass scrot xorg.xset ] ) );
  ...
  vendorDir = path { path = ./.emacs.d/vendor; name = "emacs-vendor"; };
  # TODO(wpcarro): byte-compile these by packaging each as an Elisp library.
  wpcDir = path { path = ./.emacs.d/wpc; name = "emacs-libs"; };
  wpcPackageEl = path { path = ./.emacs.d/wpc/wpc-package.el; name = "wpc-package.el"; };
  initEl = path { path = ./.emacs.d/init.el; name = "init.el"; };
  loadPath =
    concatStringsSep
      ":"
      [
  ...
  runScript =
    ...
    writeShellScript
      ...
      exec ${ wpcarrosEmacs }/bin/emacs \
        ...
        "$@"
      '';
  nixos = withEmacsPath { emacsBin = "${ wpcarrosEmacs }/bin/emacs"; };
  meta.targets = [ "nixos" ];
}
