...
let make (size : int) (seed : 'a) : 'a vec =
  { length = size; capacity = size; xs = Array.make size seed }

let create () = { length = 0; capacity = 0; xs = [||] }

let from_array (xs : 'a array) : 'a vec =
  { length = Array.length xs; capacity = Array.length xs; xs }
...
let append (x : 'a) (v : 'a vec) =
  if v.capacity = 0 then (
    ...
    let new_cap =
      v.capacity |> float_of_int |> Float.mul 1.25 |> ceil |> int_of_float
    in
...
let get (i : int) (v : 'a vec) : 'a option =
  if i >= v.length then None else Some v.xs.(i)

let get_unsafe (i : int) (v : 'a vec) : 'a = v.xs.(i)

let set (i : int) (x : 'a) (v : 'a vec) : unit =
  if i < v.length then Array.set v.xs i x

let length (v : 'a vec) : int = v.length

let update (i : int) (f : 'a -> 'a) (v : 'a vec) : unit =
  match get i v with None -> () | Some x -> set i (f x) v
