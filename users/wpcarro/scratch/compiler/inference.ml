 ...
 *
 * Wish List:
 * - TODO Debug inference for function application ((-> x x) 10)
 *
 * Example Usage:
...
let rec infer (ctx : context) (x : Types.value) : inference =
  ...
  | ValueVarApplication _ -> todo "Support ValueVarApplication"
...
let rec source (x : Types.value) : string =
  ...
  | ValueVarApplication (x, y) ->
     Printf.sprintf "(%s %s)" x (source y)
  | ValueBinder (x, y, z) ->
      Printf.sprintf "(let %s %s %s)" x (source y) (source z)
