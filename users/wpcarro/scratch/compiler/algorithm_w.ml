(*
  ...
  - Fix broken type inference:
    - actual   `((-> x x) 10) => a0`
    - expected `((-> x x) 10) => Integer`
  - Support a Lambda Calculus expression language.
  - Support a REPL for playing around.
...
(* (:: id (-> a a)) *)
let id : value = ValueFunction ("x", ValueVariable "x")

(* (:: const (-> a (-> b a))) *)
let const : value = ValueFunction ("x", ValueFunction ("y", ValueVariable "x"))
