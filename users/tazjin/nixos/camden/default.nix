...
lib.fix
  (
    self:
    {
            ...
            fetchTarball
              {
                ...
                url =
                  "https://github.com/NixOS/nixpkgs-channels/archive/75f4ba05c63be3f147bcc2f7bd4ba1f029cedcb1.tar.gz";
      ...
      boot = {
        initrd = {
          availableKernelModules = [ "ahci" "xhci_pci" "usbhid" "usb_storage" "sd_mod" "sdhci_pci" "rtsx_usb_sdmmc" "r8169" ];
      ...
      fileSystems = {
        "/" = { device = "/dev/disk/by-label/camden-root"; fsType = "ext4"; };
      ...
      nix = {
        ...
        binaryCaches = [ "https://tazjin.cachix.org" ];
        binaryCachePublicKeys = [ "tazjin.cachix.org-1:IZkgLeqfOr1kAZjypItHMg1NoBjm4zX9Zzep8oRSh7U=" ];
      };
      powerManagement.cpuFreqGovernor = lib.mkDefault "powersave";
      networking = {
        hostName = "camden";
        interfaces.enp1s0.useDHCP = true;
        interfaces.enp1s0.ipv6.addresses = [ { address = "2a01:4b00:821a:ce02::5"; prefixLength = 64; } ];
      ...
      fonts = { fonts = [ pkgs.jetbrains-mono ]; fontconfig.defaultFonts.monospace = [ "JetBrains Mono" ]; };
      environment.systemPackages =
        # programs from the depot
        ( with depot; [ fun.idual.script fun.idual.setAlarm ] )
          ++
      ...
      users = {
        ...
        users.tazjin = { isNormalUser = true; uid = 1000; extraGroups = [ "git" "wheel" "quassel" "video" ]; shell = pkgs.fish; };
        # Set up a user & group for general git shenanigans
        groups.git = { };
        users.git = { group = "git"; isSystemUser = true; };
      };
      # Services setup
      ...
      systemd.timers.fix-nginx = { wantedBy = [ "multi-user.target" ]; timerConfig = { OnCalendar = "minutely"; }; };
      ...
      security.acme = {
        ...
        certs."quassel.tazj.in" = {
          ...
          user = "nginx";
          # required because of a bug in the ACME module
          group = "quassel";
      ...
      services.depot.quassel = { enable = true; acmeHost = "quassel.tazj.in"; bindAddresses = [ "0.0.0.0" ]; };
      services.bitlbee = {
        enable = false;
        portNumber = 2337;
        # bees
      ...
      services.nginx = {
        ...
        virtualHosts.cgit-old = nginxRedirect { from = "git.tazj.in"; to = "code.tvl.fyi"; acmeHost = "tazj.in"; };
        virtualHosts.cs-old = nginxRedirect { from = "cs.tazj.in"; to = "cs.tvl.fyi"; acmeHost = "tazj.in"; };
      };
