...
let
  ...
  ordMap = builtins.listToAttrs ( lib.imap1 ( i: v: { name = v; value = i; } ) ( string.toChars allChars ) );
  # Note on performance:
  ...
  asciiAlpha = c: let v = ord c; in ( v >= 65 && v <= 90 ) || ( v >= 97 && v <= 122 );
  asciiNum = c: let v = ord c; in v >= 48 && v <= 57;
  asciiAlphaNum = c: asciiAlpha c || asciiNum c;
in
{
  inherit allChars char charval ord chr asciiAlpha asciiNum asciiAlphaNum;
