...
let
  inherit ( depot.users.sterni.nix ) char flow fun int string util;
  /*
  ...
  wellFormedByte =
    ...
    let
      ...
      secondBytePredicate =
        flow.switch
          first
          [
            [ ( int.inRange 194 223 ) defaultRange ]
            # C2..DF
            [ 224 ( int.inRange 160 191 ) ]
            # E0
            [ ( int.inRange 225 236 ) defaultRange ]
            # E1..EC
            [ 237 ( int.inRange 128 159 ) ]
            # ED
            [ ( int.inRange 238 239 ) defaultRange ]
            # EE..EF
            [ 240 ( int.inRange 144 191 ) ]
            # F0
            [ ( int.inRange 241 243 ) defaultRange ]
            # F1..F3
            [ 244 ( int.inRange 128 143 ) ]
            # F4
            [ ( fun.const true ) null ]
          ];
      firstBytePredicate = byte: assert first == byte; first < 128 || secondBytePredicate != null;
    in
    ...
    # return predicate according to Table 3-6.
      ...
      if
        pos == 1
      then
        assert secondBytePredicate != null; secondBytePredicate
  ...
  step =
      ...
      newCode =
        ...
        # ascii character
          ...
          let
            ...
            # See also table 3-6.
            mask = if pos == 0 then int.exp 2 ( 8 - ( count + 1 ) ) - 1 else 63;
    ...
    in
    if
      !( wellFormedByte first pos value )
    then
      builtins.throw illFormedMsg
    else
      if
        pos + 1 == count
      then
        ...
        ( builtins.removeAttrs args [ "result" ] ) // { inherit count first; code = newCode; pos = pos + 1; };
  /*
    ...
    let
      ...
      iterResult =
        builtins.genericClosure
          {
            startSet = [ { key = "start"; stringIndex = -1; state = { }; codepoint = null; } ];
            operator =
              { state
              , stringIndex
              , ...
          ...
          (
            ...
            let
              ...
              earlyEndMsg =
                ...
                then
                  "Missing ${ toString ( with state; count - pos ) } bytes at end of input"
                else
  ...
  /*
   ...
   Type: integer -> string
   */
  formatCodepoint = cp: "U+" + string.fit { width = 4; char = "0"; } ( int.toHex cp );
  encodeCodepoint =
    cp:
    let
      ...
      count =
        flow.switch
          ...
          [
            [ ( int.inRange 0 127 ) 1 ]
            # 00000000 0xxxxxxx
            [ ( int.inRange 128 2047 ) 2 ]
            # 00000yyy yyxxxxxx
            [ ( int.inRange 2048 65535 ) 3 ]
            # zzzzyyyy yyxxxxxx
            [ ( int.inRange 65536 1114111 ) 4 ]
      ...
      components =
        lib.mapAttrs
          ( _: { mask, offset }: int.bitAnd ( int.bitShiftR cp offset ) mask )
          {
            x = { mask = if count > 1 then 63 else 127; offset = 0; };
            y = { mask = if count > 2 then 63 else 31; offset = 6; };
            z = { mask = 15; offset = 12; };
      ...
      (
        builtins.genList
          (
            ...
            in
            if wellFormedByte firstByte i byte then byte else builtins.throw unableToEncodeMessage
          )
          count
      );
  /*
...
in
{ inherit encode decode step formatCodepoint; }
