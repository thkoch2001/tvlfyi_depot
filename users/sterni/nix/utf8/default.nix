  ...
  */
  byteCount =
    ...
    flow.cond [
      ...
      [
        (int.bitAnd i 224 == 192)
        2
      ]
      [
        (int.bitAnd i 240 == 224)
        3
      ]
      [
        (int.bitAnd i 248 == 240)
        4
      ]
      [
        true
        (builtins.throw "Ill-formed first byte ${int.toHex i}")
      ]
    ];
  ...
  */
  wellFormedByte =
    ...
    # return predicate according to Table 3-6.
      ...
      assert secondBytePredicate != null;
      secondBytePredicate
  ...
  */
  step =
    ...
    let
      ...
      newCode =
        if count == 1 then
          int.bitAnd 127 first # ascii character
      ...
      illFormedMsg = "Ill-formed byte ${int.toHex value} at position ${toString pos} in ${toString count} byte UTF-8 sequence";
    in
    if !(wellFormedByte first pos value) then
      builtins.throw illFormedMsg
    else if pos + 1 == count then
      ...
      (builtins.removeAttrs args [ "result" ])
      // {
        inherit count first;
        code = newCode;
        pos = pos + 1;
  ...
  encodeCodepoint =
    ...
    let
      ...
      count = flow.switch cp [
        ...
        [
          (num.inRange 128 2047)
          2
        ] # 00000yyy yyxxxxxx
        [
          (num.inRange 2048 65535)
          3
        ] # zzzzyyyy yyxxxxxx
        [
          (num.inRange 65536 1114111)
          4
        ...
        [
          (fun.const true)
          (builtins.throw invalidCodepointMsg)
        ]
      ];
      ...
      components = lib.mapAttrs (_: { mask, offset }: int.bitAnd (int.bitShiftR cp offset) mask) {
        x = {
