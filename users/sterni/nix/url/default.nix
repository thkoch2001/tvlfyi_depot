...
let
  inherit ( depot.users.sterni.nix ) char int string flow;
  reserved = c: builtins.elem c [ "!" "#" "$" "&" "'" "(" ")" "*" "+" "," "/" ":" ";" "=" "?" "@" "[" "]" ];
  unreserved = c: char.asciiAlphaNum c || builtins.elem c [ "-" "_" "." "~" ];
  percentEncode =
    c:
    if
      unreserved c
    then
      ...
      "%" + ( string.fit { width = 2; char = "0"; side = "left"; } ( int.toHex ( char.ord c ) ) );
  encode =
    { leaveReserved ? false
    }:
    s:
  ...
  decode =
    ...
    let
      ...
      decodeStep =
        ...
        flow.cond
          [
            [ ( builtins.isList s ) { inherit result; inPercent = true; } ]
            [
              inPercent
              {
                inPercent = false;
