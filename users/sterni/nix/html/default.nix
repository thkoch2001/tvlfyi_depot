    ...
    let
      attrs' =
        builtins.concatStringsSep
          ...
          (
            builtins.map
              ( n: " ${ escapeMinimal n }=\"${ escapeMinimal ( toString attrs.${ n } ) }\"" )
              ( builtins.attrNames attrs )
          );
      content' = if builtins.isList content then builtins.concatStringsSep "" content else content;
    in
    if content == null then "<${ tag }${ attrs' }/>" else "<${ tag }${ attrs' }>${ content' }</${ tag }>";
  /*
...
in
{ inherit escapeMinimal renderTag withDoctype; __findFile = _: renderTag; esc = escapeMinimal; }
