...
let
  ...
  inherit (depot.users.sterni.nix) string num;

  inherit (builtins) bitOr bitAnd bitXor;
  ...
  exp =
    ...
    if pow > 0 then
      ...
      1.0 / exp base (num.abs pow)
    else
      1;

  bitShiftR = bit: count: if count == 0 then bit else (bitShiftR bit (count - 1)) / 2;
  ...
  toHex =
    ...
    in
    if int == 0 then "0" else "${sign}${go (num.abs int)}";
