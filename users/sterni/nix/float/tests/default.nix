...
let
  ...
  inherit (depot.users.sterni.nix) float;
  ...
  testsConversionFrom = it "tests integer to float conversion" [
    ...
    (assertEq "float.intToFloat converts ints" (builtins.all (val: builtins.isFloat val) (
  ...
  testsConversionTo = it "tests float to integer conversion" [
    (assertEq "round" (builtins.map float.round exampleFloats) [
      1
      0
      0
      0
    ...
    (assertEq "truncate towards zero" (builtins.map float.truncate exampleFloats) [
      0
      0
      0
      0
