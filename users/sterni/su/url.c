...
s_t urlencode(slice_t input, bool escape_reserved, bool escape_slash) {
    ...
    for(s_int i = 0; i < input.slice_len; i++) {
        ...
        if(needs_escape) {
            ...
            s_append_char(&output, hex_digit((c & 0xf0) >> 4));
            s_append_char(&output, hex_digit(c & 0x0f));
        } else {
    ...
    while(pos < input.slice_len) {
        ...
        if(c == '%') {
            if(pos + 2 < input.slice_len) {
                short a = hex_int(input.slice_buf[++pos]);
                short b = hex_int(input.slice_buf[++pos]);
