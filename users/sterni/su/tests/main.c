...
void check_json_emit(void) {
  ...
  if(memstream_res != -1) {
    ...
    su_check_assert(&chk, "json/emit: written size matches computed size", len_correct);
...
void check_url(void) {
  ...
  if(sizeof(encoded) == sizeof(decoded)) {
    for(size_t i = 0; i < (sizeof(encoded) / sizeof(slice_t)); i++) {
      ...
      su_check_assert(&chk, "url: urlencode matches expected",
          slice_eq(s_slice(tmp_encoded), encoded[i]));

      su_check_assert(&chk, "url: urldecode matches expected",
