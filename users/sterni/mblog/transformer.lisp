...
(defclass apple-note-transformer (hax-proxy-handler)
  ((cid-lookup
   ...
   ;; FIXME(sterni): we also need to record the depth
   (discard-until
...
(defmethod hax:end-element ((h apple-note-transformer) name)
  (with-accessors ((discard-until transformer-discard-until))
      h
      (cond
        ;; If we are discarding and encounter the same tag again (XXX depth),
        ;; we can stop it, but still have to discard the current tag.
