...
(defclass apple-note-transformer (hax-proxy-handler)
  ((attachments
    :initarg :attachments
    :initform '()
    :accessor transformer-attachments)
   (discard-until
...
(defmethod hax:start-element ((h apple-note-transformer) name attrs)
  (with-accessors ((discard-until transformer-discard-until)
                   (attachments transformer-attachments)
        ...
        ((string= name "OBJECT")
         (progn
           ...
           (when-let* ((cid (parse-content-id attrs))
                       (file (assoc cid attachments :test #'string=)))
             (hax:start-element next-handler
                                "IMG"
                                (list (hax:make-attribute "SRC" (cdr file))))
...
(defun apple-note-html-fragment (input attachments)
  (closure-html:parse
   input
   (make-instance 'apple-note-transformer
                  :attachments attachments
                  :next-handler (closure-html:make-string-sink))))
