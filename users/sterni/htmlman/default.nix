...
let
  ...
  markdown =
    md:
    let
      html = runExecline.local "rendered-markdown" { stdin = md; } (
        [
          "importas"
          "-iu"
          "out"
  ...
  deployScript =
    title: drv:
    writers.writeDash "deploy-${title}" ''
      usage() {
  ...
  htmlman =
    ...
    let
      ...
      resolvePath =
        ...
        if path != null then path else "${manDir}/${name}.${toString section}";
          ...
          (
            [
              ...
              (execlineStdoutInto "\${out}/style.css" [
                "if"
                ([ bins.cat ] ++ lib.optional normalizeCss normalizeDrv ++ [ "$style" ])
              ])
