...
let
  inherit ( depot.nix ) getBins runExecline yants;
  inherit ( depot.tools ) cheddar;
  inherit ( pkgs ) mandoc coreutils fetchurl writers;
  bins = getBins cheddar [ "cheddar" ] // getBins mandoc [ "mandoc" ] // getBins coreutils [ "cat" "mv" "mkdir" ];
  normalizeDrv =
    fetchurl
      {
  ...
  execlineStdoutInto = target: line: [ "redirfd" "-w" "1" target ] ++ line;
    ...
    let
      html =
        runExecline.local
          ...
          (
            [ "importas" "-iu" "out" "out" ]
              ++ execlineStdoutInto "$out" [ bins.cheddar "--about-filter" "description.md" ]
          );
    in
  ...
  htmlman =
    ...
    let
      ...
      index = indexTemplate { inherit title description pages; };
      resolvePath =
        { path ? null, name, section }: if path != null then path else "${ manDir }/${ name }.${ toString section }";
      mandocOpts =
        lib.concatStringsSep
          ...
          (
            [ "style=style.css" ]
              ++ linkXrEnum.match
                linkXr
                { all = [ "man=./%N.%S.html" ]; inManDir = [ "man=./%N.%S.html;${ linkXrFallback }" ]; none = [ ]; }
          );
      html =
        runExecline.local
          "htmlman-${ title }"
          { derivationArgs = { inherit index style; passAsFile = [ "index" "style" ]; }; }
          (
            [
              "multisubstitute"
              [
              ...
              (
                execlineStdoutInto
                  "\${out}/style.css"
                  [ "if" ( [ bins.cat ] ++ lib.optional normalizeCss normalizeDrv ++ [ "$style" ] ) ]
              )
              ...
              ++ lib.concatMap
                (
                  ...
                  @ p:
                  execlineStdoutInto
                    "\${out}/${ name }.${ toString section }.html"
                    [ "if" [ bins.mandoc "-mdoc" "-T" "html" "-O" mandocOpts ( resolvePath p ) ] ]
                )
                pages
          );
    in
    html // { deploy = deployScript title html; };
in
htmlman
