...
let
  ...
  bins =
    getBins pkgs.jq [ "jq" ]
      // getBins pkgs.coreutils [ "cat" "printf" "tee" "test" "wc" ]
      // getBins pkgs.gnugrep [ "grep" ]
      // getBins pkgs.cargo-audit [ "cargo-audit" ]
      // getBins pkgs.ansi2html [ "ansi2html" ]
      // { eprintf = depot.tools.eprintf; };
  # list of maintainers we may @mention on GitHub
  maintainerWhitelist =
    builtins.attrValues { inherit ( lib.maintainers ) sternenseemann qyliss jk symphorien erictapen expipiplus1; };
  ...
  recurseInto = path: x: path == [ ] || ( lib.isAttrs x && ( x.recurseForDerivations or false ) );
    ...
    let
      go =
        ...
        if
          doRec
        then
          lib.concatLists ( lib.mapAttrsToList ( n: go ( path ++ [ n ] ) ) x )
        else
          if
            isDrv && isRust
          then
            [ { attr = path; lock = extractCargoLock x; maintainers = x.meta.maintainers or [ ]; } ]
          else
            [ ];
    in
    go [ ];
  ...
  reportFor =
    ...
    then
      ...
      depot.nix.runExecline
        "${ strAttr }-vulnerability-report"
        { }
        [
          "pipeline"
          [ bins.cargo-audit "audit" "--json" "-n" "--db" rustsec-advisory-db "-f" lock ]
          "importas"
          "out"
          "out"
          "redirfd"
  ...
  reportForNixpkgs =
    ...
    @ args:
    let
      reports = builtins.map reportFor ( allLockFiles ( import nixpkgsPath nixpkgsArgs ) );
    in
    depot.nix.runExecline
      "nixpkgs-rust-pkgs-vulnerability-report.md"
      ...
      [
        ...
        "-E"
        "vulnerableCount"
        [ "pipeline" [ bins.grep "^- \\[ \\]" "$out" ] bins.wc "-l" ]
        "if"
        [
          bins.printf
  ...
  singleReport =
    ...
    depot.nix.runExecline
      ...
      [
        ...
        "-N"
        "report"
        [ bins.cargo-audit "audit" "--quiet" "-n" "--db" rustsec-advisory-db "-f" lockFile ]
        "pipeline"
        [
          "ifte"
...
in
{
  ...
  single = singleReport;
  inherit extractCargoLock allLockFiles;
  # simple sanity check, doesn't cover everything, but testing the full report
  # is quite expensive in terms of evaluation.
  testSingle = singleReport { nixpkgsPath = depot.third_party.nixpkgs.path; attr = [ "ripgrep" ]; };
