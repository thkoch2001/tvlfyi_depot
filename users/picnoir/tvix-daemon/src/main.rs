...
#[derive(Debug)]
struct ClientConnection<R: AsyncReadExt + AsyncWriteExt + Unpin + std::fmt::Debug> {
    conn: R,
    version_minor: u64,
    ...
    R: AsyncReadExt + AsyncWriteExt + Unpin + std::fmt::Debug,
{
    ...
    // The client expects us to sent it some logs when
    // we're processing the settings. Sending
    // STDERR_LAST to tell it we're done processing.
