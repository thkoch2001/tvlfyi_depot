...
circle c r
  | r <= memoLength
  = (+ c) <$> memo ! r
  | otherwise = makeCircle c r
    where
      memoLength = 20
      memo = listArray (0, memoLength) $ makeCircle 0 <$> [0..memoLength]

      -- Four initial points, plus the generated points
      makeCircle center@(V2 x₀ y₀) radius
        = (V2 x₀ (y₀ + radius))
        : (V2 x₀ (y₀ - radius))
        : (V2 (x₀ + radius) y₀)
        : (V2 (x₀ - radius) y₀)
      ...
      -- Creates the (x, y) octet offsets, then maps them to absolute points in
      -- all octets.
      points center radius
        = concatMap (generatePoints center) $ unfoldr step $ initialValues radius

      generatePoints (V2 x₀ y₀) (x, y)
        = [ V2 (x₀ `xop` x') (y₀ `yop` y')
          | (x', y') <- [(x, y), (y, x)]
          , xop <- [(+), (-)]
          , yop <- [(+), (-)]
      ...
      initialValues radius = (1 - radius, 1, (-2) * radius, 0, radius)
      ...
      step (f, ddf_x, ddf_y, x, y)
        ...
        where
          ...
          ddf_x' = ddf_x + 2
          x' = x + 1
{-# SPECIALIZE circle :: V2 Int -> Int -> [V2 Int] #-}
{-# SPECIALIZE circle :: V2 Word -> Word -> [V2 Word] #-}
