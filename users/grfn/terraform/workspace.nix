...
let
  ...
  allPlugins = ( p: plugins p ++ ( with p; [ external local tls p.null ] ) );
  tf = pkgs.terraform.withPlugins allPlugins;
  cleanTerraform = filterAttrsRecursive ( k: _: !( builtins.elem k [ "__readTree" "__readTreeChildren" ] ) );
  plugins_tf = {
          ...
          map
            (
              p:
              {
                ...
                value = { source = p.provider-source-address; version = p.version; };
              }
            )
  ...
  module_tf' = module_tf // { inherit ( depot.users.grfn.terraform ) globals; plugins = plugins_tf; };
  module =
    runCommandNoCC
      "module"
      { }
