...
let
  os =
    depot.ops.nixos.nixosFor
      (
        ...
        {
          imports = [ "${ pkgs.path }/nixos/modules/virtualisation/amazon-image.nix" configuration ];
          ec2.hvm = true;
  ...
  securityGroupId' = if isNull securityGroupId then "\${aws_security_group.${ prefix }group.id}" else securityGroupId;
in
recursiveMerge
  [
    (
      lib.optionalAttrs
        ( isNull securityGroupId )
        {
    ...
    rec
      ...
      resource.tls_private_key."${ prefix }key" = { algorithm = "RSA"; };
      ...
      resource.aws_instance."${ prefix }machine" = {
        ...
        root_block_device = { volume_size = rootVolumeSizeGb; tags.Name = name; };
        tags.Name = name;
      };
