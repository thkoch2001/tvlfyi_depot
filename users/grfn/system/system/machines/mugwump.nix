...
with lib;
{
  ...
  fileSystems = {
    ...
    "/boot" = { device = "/dev/disk/by-uuid/7D74-0E4B"; fsType = "vfat"; };
  };
  networking.interfaces = { enp0s25.useDHCP = false; wlp2s0.useDHCP = false; };
  ...
  age.secrets =
    ...
    in
    {
      ...
      buildkite-ssh-key = { file = secret "buildkite-ssh-key"; group = "keys"; mode = "0440"; };
      buildkite-token = { file = secret "buildkite-token"; group = "keys"; mode = "0440"; };
    };
  services.depot.auto-deploy = { enable = true; interval = "1d"; };
  services.fail2ban = { enable = true; ignoreIP = [ "172.16.0.0/16" ]; };
  services.openssh = { allowSFTP = false; passwordAuthentication = false; permitRootLogin = "no"; };
  services.grafana = {
    enable = true;
    port = 3000;
  ...
  security.acme.certs."metrics.gws.fyi" = { dnsProvider = "cloudflare"; credentialsFile = "/run/agenix/cloudflare"; webroot = mkForce null; };
  services.prometheus = {
    enable = true;
    exporters = {
      node = { enable = true; openFirewall = false; enabledCollectors = [ "processes" "systemd" "tcpstat" "wifi" ]; };
      nginx = { enable = true; openFirewall = true; sslVerify = false; constLabels = [ "host=mugwump" ]; };
      blackbox = {
        enable = true;
        openFirewall = true;
        configFile =
    ...
    scrapeConfigs = [
      ...
      }
      {
        ...
        scrape_interval = "1s";
        static_configs = [ { targets = [ "localhost:${ toString config.services.xanthous-server.metricsPort }" ]; } ];
      }
      {
        job_name = "blackbox";
        metrics_path = "/probe";
        ...
        static_configs = [
          {
            targets = [ "https://gws.fyi" "https://windtunnel.ci" "https://app.windtunnel.ci" "https://metrics.gws.fyi" ];
          }
        ];
        relabel_configs = [
          ...
          (
            ...
            rec
              ...
              value = {
                ...
                runtimePackages = with pkgs; [ docker nix gnutar gzip ];
              };
            }
          )
          ( range 1 1 )
      );
  users.users."buildkite-agent-mugwump-1" = { isSystemUser = true; extraGroups = [ "docker" ]; };
}
