...
}:
{
  ...
  boot = {
    initrd = {
      ...
      kernelModules = [ ];
      luks.devices = { "cryptroot".device = "/dev/disk/by-uuid/dcfbc22d-e0d2-411b-8dd3-96704d3aae2e"; };
    };
    kernelPackages = pkgs.linuxPackages_5_15;
    kernelModules = [ "kvm-intel" ];
    blacklistedKernelModules = [ "psmouse" ];
    extraModulePackages = [ config.boot.kernelPackages.digimend ];
    kernelParams = [ "i915.preliminary_hw_support=1" "pcie_aspm=force" ];
  ...
  fileSystems = {
    ...
    "/boot" = { device = "/dev/disk/by-uuid/53A9-248B"; fsType = "vfat"; };
  };
  ...
  hardware.firmware = with pkgs; [ alsa-firmware sof-firmware ];
  ...
  security.pam.loginLimits = [ { domain = "grfn"; type = "soft"; item = "nofile"; value = "65535"; } ];
