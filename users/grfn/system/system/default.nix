...
rec
    ...
    (
        depot.ops.nixos.nixosFor
          (
            ...
            {
              imports = [ ./machines/roswell.nix "${ pkgs.home-manager.src }/nixos" ];
              # Use the same nixpkgs as everything else
              home-manager.useGlobalPkgs = true;
              home-manager.users.grfn =
                { config, lib, ... }: { imports = [ ../home/machines/roswell.nix ]; lib.depot = depot; };
            }
          )
      )
      .system;
  ...
  meta.targets = [ "mugwumpSystem" "roswellSystem" "yerenSystem" "iso" ];
}
