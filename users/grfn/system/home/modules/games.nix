...
let
  ...
  df-full = ( dwarf-fortress-packages.dwarf-fortress-full.override { theme = null; enableIntro = false; enableFPS = true; } );
  init =
    runCommand
      "init.txt"
      { }
  ...
  df =
    runCommand
      ...
      '';
in
mkMerge [ { home.packages = [ crawl xonotic ]; } ( mkIf stdenv.isLinux { home.packages = [ df polymc ]; } ) ]
