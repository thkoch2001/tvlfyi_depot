...
terraform.workspace
  "bbbg"
  { plugins = ( p: with p; [ aws cloudflare ] ); }
  {
    machine =
      terraform.nixosMachine
        {
          name = "bbbg";
          ...
          configuration =
            ...
            {
              imports = [ ./module.nix "${ depot.third_party.agenix.src }/modules/age.nix" ];
              ...
              users.users.grfn = {
                ...
                shell = pkgs.zsh;
                openssh.authorizedKeys.keys = [ depot.users.grfn.keys.main ];
              };
              security.sudo.extraRules = [ { groups = [ "wheel" ]; commands = [ { command = "ALL"; options = [ "NOPASSWD" ]; } ]; } ];
              nix.gc = { automatic = true; dates = "weekly"; options = "--delete-older-than 30d"; };
              age.secrets = { bbbg.file = depot.users.grfn.secrets."bbbg.age"; };
    ...
    dns = {
      data.cloudflare_zone.gws-fyi = { name = "gws.fyi"; };
