    ...
    readTree
      {
        ...
        scopedArgs = { __findFile = _: _: throw "Do not import from NIX_PATH in the depot!"; };
      };
  ...
  (
    ...
    (
      ...
      // {
        ...
        path =
          self.third_party.nixpkgs.lib.cleanSourceWith
            { name = "depot"; src = ./.; filter = self.third_party.nixpkgs.lib.cleanSourceFilter; };
        ...
        ci.gcroot =
          with self.third_party.nixpkgs; makeSetupHook { name = "depot-gcroot"; deps = self.ci.targets; } emptyFile;
      }
  )
