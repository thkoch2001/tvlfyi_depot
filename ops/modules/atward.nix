...
in
{
  options.services.depot.atward = {
    ...
    host =
      lib.mkOption { type = lib.types.str; default = "[::1]"; description = "Host on which atward should listen"; };
    port = lib.mkOption { type = lib.types.int; default = 28973; description = "Port on which atward should listen"; };
  };
  config =
    lib.mkIf
      cfg.enable
