...
let
  ...
  overrideArgs = { logfile = ""; config = "$CREDENTIALS_DIRECTORY/secrets"; };
...
in
{
  options.services.depot.smtprelay = {
    ...
    args = mkOption { type = types.attrsOf types.str; description = "Key value pairs for command line arguments"; };
    secretsFile = mkOption { type = types.str; default = "/run/agenix/smtprelay"; };
  };
  config =
    mkIf
      cfg.enable
