...
let
  ...
  mkFlags =
    flags: concatStringsSep " " ( attrValues ( mapAttrs ( key: value: "-${ key } \"${ toString value }\"" ) flags ) );
  ...
  mkUnit =
    ...
    channel:
    {
      ...
      value = {
        ...
        script =
          "${ depot.fun.clbot }/bin/clbot ${ mkFlags ( cfg.flags // { irc_channel = channel; } ) } -alsologtostderr";
        serviceConfig = { User = "clbot"; EnvironmentFile = cfg.secretsFile; Restart = "always"; };
      };
    };
in
{
  options.services.depot.clbot = {
    ...
    flags = mkOption { type = types.attrsOf types.str; description = "Key value pairs for command line flags"; };
    channels =
      mkOption
        { type = with types; listOf str; description = "Channels in which to post (generates one unit per channel)"; };
    secretsFile =
      mkOption
        {
          type = types.str;
  ...
  config =
    mkIf
      cfg.enable
      {
        ...
        # the module.
        users = { groups.clbot = { }; users.clbot = { group = "clbot"; isSystemUser = true; }; };
