...
in
{
  options.services.prometheus-fail2ban-exporter =
    with lib;
    {
  ...
  config =
    lib.mkIf
      cfg.enable
      {
        ...
        systemd.timers."prometheus-fail2ban-exporter" = { wantedBy = [ "multi-user.target" ]; timerConfig.OnCalendar = cfg.interval; };
