...
in
{
  services.gerrit = {
    ...
    plugins = with depot.third_party.gerrit_plugins; [ owners oauth depot.ops.gerrit-tvl ];
    ...
    settings = {
      ...
      gerrit = { canonicalWebUrl = "https://cl.tvl.fyi"; docUrl = "/Documentation"; };
      ...
      commentlink.panettone = { match = "b/(\\\\d+)"; html = "<a href=\"https://b.tvl.fyi/issues/$1\">b/$1</a>"; };
      # Auto-link other CLs
      commentlink.gerrit = { match = "cl/(\\\\d+)"; html = "<a href=\"https://cl.tvl.fyi/$1\">cl/$1</a>"; };
  ...
  services.depot.restic = { paths = [ "/var/lib/gerrit" ]; exclude = [ "/var/lib/gerrit/tmp" ]; };
}
