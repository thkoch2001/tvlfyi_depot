...
in
{
  options.services.depot.sourcegraph =
    with lib;
    {
      ...
      port = mkOption { description = "Port on which SourceGraph should listen"; type = types.int; default = 3463; };
      cheddarPort =
        mkOption { description = "Port on which cheddar should listen"; type = types.int; default = 4238; };
    };
  config =
    lib.mkIf
      cfg.enable
      {
        ...
        systemd.services.cheddar-server = {
          ...
          script =
            "${ depot.tools.cheddar }/bin/cheddar --listen 0.0.0.0:${ toString cfg.cheddarPort } --sourcegraph-server";
          serviceConfig = { DynamicUser = true; Restart = "always"; };
        };
        virtualisation.oci-containers.containers.sourcegraph = {
          image = "sourcegraph/server:3.31.2";
          ports = [ "127.0.0.1:${ toString cfg.port }:7080" ];
          volumes = [ "/var/lib/sourcegraph/etc:/etc/sourcegraph" "/var/lib/sourcegraph/data:/var/opt/sourcegraph" ];
