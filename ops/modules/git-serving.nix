...
in
{
  options.services.depot.git-serving =
    with lib;
    {
      enable = mkEnableOption "Enable cgit & josh configuration";
      joshPort = mkOption { description = "Port on which josh should listen"; type = types.int; default = 5674; };
    };
  config =
    lib.mkIf
      cfg.enable
      {
        ...
        systemd.services.cgit = {
          ...
          serviceConfig = { Restart = "on-failure"; User = "git"; Group = "git"; };
        };
        ...
        systemd.services.josh = {
          ...
          serviceConfig = {
            ...
            Restart = "always";
            ExecStart =
              "${ depot.third_party.josh }/bin/josh-proxy --no-background --local /var/lib/josh --port ${ toString cfg.joshPort } --remote https://cl.tvl.fyi/";
          };
        };
      };
}
