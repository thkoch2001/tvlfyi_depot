...
var (
	...
	depotRootPath    = flag.String("depot_root", "", "Path to depot root.")
	dumpWorkspaces   = flag.Bool("dump_workspaces", false, "Dump the workspaces.")
...
func (ds *dirStruct) FlattenHasType(root, path string) {
	if len(ds.RootFor) != 0 {
		// Check to see if we have a Nix LSP override here.
		lspOverrides := nixLSPs(root, path)
		for _, lspo := range lspOverrides {
			if ds.RootFor[lspo] {
				ds.NixLSPPresent[lspo] = true
			}
		}
	}
	...
	for c, cds := range ds.Children {
		cds.FlattenHasType(root, filepath.Join(path, c))
		if cds.HasSubRoot {
			ds.HasSubRoot = true
		}
...
func (ds *dirStruct) Roots(path string, m map[string][]Workspace) {
	for ht, cfs := range ds.ChildFiles {
		m[ht] = append(m[ht], Workspace{
			NixLSPPresent: ds.NixLSPPresent[ht],
			Path:          path,
			Files:         cfs,
		})
	}
...
func nixLSPs(root, path string) []string {
	defaultNix := filepath.Join(root, path, "default.nix")
	_, err := os.Stat(defaultNix)
	...
	bits := strings.Split(filepath.ToSlash(path), "/")
	if path == "." {
		bits = nil
	}
	expr := fmt.Sprintf("(builtins.attrNames (import %s/. {}).\"%s\")", root, strings.Join(append(bits, "meta", "lsps"), "\".\""))
	cmd := exec.Command("nix", "eval", "--json", expr)
	o, err := cmd.Output()
	if err != nil {
		return nil
	}
...
func runLSPForWorkspaces(ctx context.Context, logger *zap.SugaredLogger, diagnosticPrefix, depotRoot string, cfg LSPConfiguration, workspaces []Workspace) ([]*protocol.PublishDiagnosticsParams, error) {
	...
	cl := &client{}
	cmd := cfg.Command
	if len(workspaces) == 1 && workspaces[0].NixLSPPresent {
		ws := workspaces[0]
		cmd = append([]string{
			"nix-shell",
			"-A",
			fmt.Sprintf("\"%s\"", strings.Join(append(strings.Split(filepath.ToSlash(ws.Path), "/"), "meta", "lsps", diagnosticPrefix), "\".\"")),
			depotRoot,
			"--run",
		}, cmd...)
		logger.Infof("launching %v", cmd)
	}
	conn, lspClient, err := launchLSP(ctx, logger.Desugar(), cl, cmd)
	if err != nil {
...
waitSettled:
	...
	_ = lspClient.Shutdown(ctx)
	_ = lspClient.Exit(ctx)
...
func runLSPsForWorkspaces(ctx context.Context, root string, workspaces map[string][]Workspace) ([]*protocol.PublishDiagnosticsParams, error) {
	...
	for wsLang, wss := range workspaces {
		...
		var togetherWorkspaces []Workspace
		var separateWorkspaces []Workspace
		for _, ws := range wss {
			if ws.NixLSPPresent || !lsp.SupportsWorkspaces {
				separateWorkspaces = append(separateWorkspaces, ws)
			} else {
				togetherWorkspaces = append(togetherWorkspaces, ws)
			}
		}
		if len(togetherWorkspaces) > 0 {
			work = append(work, func(ctx context.Context) ([]*protocol.PublishDiagnosticsParams, error) {
				return runLSPForWorkspaces(ctx, logger.Named(wsLang), wsLang, root, lsp, togetherWorkspaces)
			})
		}
		for _, ws := range separateWorkspaces {
			ws := ws
			work = append(work, func(ctx context.Context) ([]*protocol.PublishDiagnosticsParams, error) {
...
func FilterWorkspacesForFiles(workspaces map[string][]Workspace, files []string) map[string][]Workspace {
	if len(files) == 0 {
		return workspaces
	}
	...
	for lang, wss := range workspaces {
		for _, ws := range wss {
			anyMatch := false
			for _, f := range ws.Files {
				fullPath := filepath.Join(ws.Path, f)
				if fs[fullPath] {
					anyMatch = true
					break
				}
			...
			out[lang] = append(out[lang], ws)
		}
	}
	return out
}

func FilterDiagnosticsForFiles(diagnostics []*protocol.PublishDiagnosticsParams, root string, files []string) []*protocol.PublishDiagnosticsParams {
	if len(files) == 0 {
		return diagnostics
	}
...
func main() {
	...
	workspaces = FilterWorkspacesForFiles(workspaces, flag.Args())
	if *dumpWorkspaces {
	...
	diagnostics = FilterDiagnosticsForFiles(diagnostics, root, flag.Args())
