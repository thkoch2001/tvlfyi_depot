...
in
{
  ...
  hardware = { enableRedistributableFirmware = true; cpu.amd.updateMicrocode = true; };
  boot = {
    tmpOnTmpfs = true;
    kernelModules = [ "kvm-amd" ];
    supportedFilesystems = [ "zfs" ];
    initrd = {
      availableKernelModules = [ "igb" "xhci_pci" "nvme" "ahci" "usbhid" "usb_storage" "sr_mod" ];
      ...
      network = {
        ...
        ssh = {
          ...
          port = 2222;
          authorizedKeys =
            depot.users.tazjin.keys.all ++ depot.users.lukegb.keys.all ++ [ depot.users.grfn.keys.whitby ];
  ...
  fileSystems = {
    ...
    "/boot" = { device = "/dev/disk/by-uuid/073E-7FBD"; fsType = "vfat"; };
  ...
  networking = {
    ...
    defaultGateway6 = { address = "fe80::1"; interface = "enp196s0"; };
    firewall.allowedTCPPorts = [ 22 80 443 4238 8443 29418 ];
    firewall.allowedUDPPorts = [ 8443 ];
    interfaces.enp196s0.useDHCP = true;
    interfaces.enp196s0.ipv6.addresses = [ { address = "2a01:04f8:0242:5b21::feed:edef:beef"; prefixLength = 64; } ];
  };
  ...
  environment.etc."gitconfig".source =
    pkgs.writeText
      "gitconfig"
      ''
      [gc]
  ...
  nix = {
    ...
    sshServe = {
      enable = true;
      keys = with depot.users; tazjin.keys.all ++ lukegb.keys.all ++ [ grfn.keys.whitby ] ++ sterni.keys.all;
    };
  };
  programs.mtr.enable = true;
  programs.mosh.enable = true;
  services.openssh = { enable = true; passwordAuthentication = false; challengeResponseAuthentication = false; };
  ...
  age.secrets =
    ...
    in
    {
      ...
      buildkite-agent-token = { file = secretFile "buildkite-agent-token"; mode = "0440"; group = "buildkite-agents"; };
      buildkite-graphql-token = { file = secretFile "buildkite-graphql-token"; mode = "0440"; group = "buildkite-agents"; };
      buildkite-besadii-config = { file = secretFile "besadii"; mode = "0440"; group = "buildkite-agents"; };
      gerrit-besadii-config = { file = secretFile "besadii"; owner = "git"; };
      gerrit-secrets = {
      ...
      clbot-ssh = { file = secretFile "clbot-ssh"; owner = "clbot"; };
      # Not actually a secret
      nix-cache-pub = { file = secretFile "nix-cache-pub"; mode = "0444"; };
    };
  ...
  services.depot.buildkite = { enable = true; agentCount = 32; };
  # Start a local SMTP relay to Gmail (used by gerrit)
  services.depot.smtprelay = {
    enable = true;
    args = { listen = ":2525"; remote_host = "smtp.gmail.com:587"; remote_auth = "plain"; remote_user = "tvlbot@tazj.in"; };
  };
  ...
  services.depot = {
    ...
    panettone = { enable = true; dbUser = "panettone"; dbName = "panettone"; irccatChannel = "#tvl"; };
    # Run the first cursed bot (quote bot)
    paroxysm.enable = true;
    # Run the second cursed bot
    owothia = { enable = true; ircServer = "localhost"; ircPort = config.services.znc.config.Listener.l.Port; };
    ...
    restic = { enable = true; paths = [ "/var/backup/postgresql" "/var/lib/grafana" "/var/lib/znc" ]; };
  ...
  services.postgresql = {
    ...
    ensureUsers = [ { name = "panettone"; ensurePermissions = { "DATABASE panettone" = "ALL PRIVILEGES"; }; } ];
  };
  services.postgresqlBackup = { enable = true; databases = [ "keycloak" "panettone" "tvldb" ]; };
  services.nix-serve = { enable = true; port = 6443; secretKeyFile = "/run/agenix/nix-cache-priv"; bindAddress = "localhost"; };
  ...
  services.prometheus = {
    enable = true;
    exporters.node = { enable = true; enabledCollectors = [ "logind" "processes" "systemd" ]; };
    scrapeConfigs = [
      {
        job_name = "node";
        scrape_interval = "5s";
      ...
      let
        options = {
          auth = {
            ...
            anonymous = { enabled = true; org_name = "The Virus Lounge"; org_role = "Viewer"; };
            basic.enabled = false;
        ...
        optionToString =
          value: if ( typeOf value ) == "bool" then if value then "true" else "false" else builtins.toString value;
              ...
              mapAttrsToList
                (
                  ...
                  then
                    ...
                    [ ( nameValuePair ( encodeName ( prefix ++ [ name ] ) ) ( optionToString value ) ) ]
                )
                inp
            );
      in
  ...
  services.keycloak = {
    ...
    database = { type = "postgresql"; passwordFile = "/run/agenix/keycloak-db"; createLocally = false; };
  ...
  users = {
    ...
    users.isomer = { isNormalUser = true; extraGroups = [ "git" ]; openssh.authorizedKeys.keys = depot.users.isomer.keys.all; };
    users.riking = {
      isNormalUser = true;
      extraGroups = [ "git" ];
      openssh.authorizedKeys.keys = depot.users.riking.keys.u2f ++ depot.users.riking.keys.passworded;
    };
    users.edef = { isNormalUser = true; extraGroups = [ "git" ]; openssh.authorizedKeys.keys = depot.users.edef.keys.all; };
    users.qyliss = { isNormalUser = true; extraGroups = [ "git" ]; openssh.authorizedKeys.keys = depot.users.qyliss.keys.all; };
    users.eta = { isNormalUser = true; extraGroups = [ "git" ]; openssh.authorizedKeys.keys = depot.users.eta.keys.whitby; };
    users.cynthia = {
      isNormalUser = true;
      # I'm normal OwO :3
      extraGroups = [ "git" ];
    ...
    users.firefly = { isNormalUser = true; extraGroups = [ "git" ]; openssh.authorizedKeys.keys = depot.users.firefly.keys.whitby; };
    users.sterni = {
      isNormalUser = true;
      extraGroups = [ "git" "wheel" ];
      openssh.authorizedKeys.keys = depot.users.sterni.keys.all;
    };
    users.flokli = { isNormalUser = true; extraGroups = [ "git" ]; openssh.authorizedKeys.keys = depot.users.flokli.keys.all; };
    # Set up a user & group for git shenanigans
    groups.git = { };
    users.git = { group = "git"; isSystemUser = true; createHome = true; home = "/var/lib/git"; };
  };
  security.acme = { acceptTerms = true; defaults.email = "certs@tvl.fyi"; };
