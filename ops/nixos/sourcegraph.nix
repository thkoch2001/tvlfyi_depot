...
let
  ...
  depot = config.depot;
  siteConfigJson = toFile "sourcegraph-site-config.json" (toJSON cfg.siteConfig);
  codeHostConfigJson = toFile "sourcegraph-code-host-config.json" (toJSON cfg.codeHostConfig);
in {
