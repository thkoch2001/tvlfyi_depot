...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.tvlbot = {
      ...
      script = "${pkgs.ii}/bin/ii -s 127.0.0.1 -p 36633 -n tvlbot -f 'TVL Bot' -i /var/lib/tvlbot";
      ...
      postStart = ''
        ...
        # auth to nickserv
        echo '/j nickserv' > $IRCDIR/in
        sleep 3

        echo "identify $TVLBOT_NICKSERV" > $IRCDIR/nickserv/in
        sleep 3
