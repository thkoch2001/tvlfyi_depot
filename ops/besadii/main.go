...
func triggerBuild(cfg *config, log *syslog.Writer, trigger *buildTrigger) error {
	...
	if trigger.changeId != "" && trigger.patchset != "" {
		...
		// group builds), so make it the identifier for the CL. Replacing
		// space " " with "_" from branch as git branch names with space
		// are not supported.
		branch = fmt.Sprintf("%s/%v", strings.Replace(cfg.GenericChangeName, " ", "_", -1), strings.Split(trigger.ref, "/")[3])
	}
