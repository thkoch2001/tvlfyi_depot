...
func mustFixedHostKey(f string) ssh.HostKeyCallback {
	...
	if err != nil {
		panic(fmt.Sprintf("ParsePublicKey(%q): %v", f, err))
	}
	return ssh.FixedHostKey(pk)
}

func mustPublicKey(p string) ssh.AuthMethod {
	pkBytes, err := ioutil.ReadFile(p)
	if err != nil {
		panic(fmt.Sprintf("reading %q: %v", p, err))
	}
	pk, err := ssh.ParsePrivateKey(pkBytes)
	if err != nil {
		panic(fmt.Sprintf("parsing private key from %q: %v", p, err))
	}
...
func main() {
	...
	cfg := &ssh.ClientConfig{
		...
		Auth:            []ssh.AuthMethod{mustPublicKey(*gerritAuthKeyPath)},
