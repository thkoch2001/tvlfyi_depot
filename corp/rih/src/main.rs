    ...
    html! {
      <div class="dropdown">
        <input type="text" class="form-control" id="citizenship" aria-describedby="citizenshipHelp"
            ...
            onblur={link.callback(|_| Msg::BlurCitizenship)}/>
...
fn get_input_element_by_id(id: &str) -> Option<HtmlInputElement> {
    ...
    let input_element = element.dyn_into::<HtmlInputElement>().ok()?;
    Some(input_element)
}
    ...
    fn update(&mut self, _ctx: &Context<Self>, msg: Self::Message) -> bool {
        ...
        let state_change = match msg {
            ...
            Msg::BlurCitizenship => {
                ...
                let _ = window.set_timeout_with_callback_and_timeout_and_arguments_0(
                    &Closure::wrap(Box::new(move || {
                        // add code here to remove the "show" class from the dropdown
                    }) as Box<dyn FnMut()>),
                    100,
                );
            ...
            Msg::SetCitizenship(country) => {
                if let Some(input) = get_input_element_by_id("citizenship") {
                    input.set_value(&country);
                }
