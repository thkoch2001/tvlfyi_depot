...
let
  ...
  # channel bump.
  nixpkgsSrc =
    externalArgs.nixpkgsBisectPath or (
        fetchTarball
          {
  ...
  commitsOverlay = _: _: { nixpkgsCommits = { unstable = unstableHashes.commit; stable = stableHashes.commit; }; };
in
import
  nixpkgsSrc
  {
