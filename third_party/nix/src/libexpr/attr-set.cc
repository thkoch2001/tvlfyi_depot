  ...
  return std::visit(
      overloaded{
        ...
        [](AttributeVector::iterator iter) -> std::pair<const Symbol, Attr>& {
          ...
          static_assert(std::is_standard_layout<std::pair<const Symbol, Attr>>::value);
...
void VectorBindings::merge(Bindings& other) {
  ...
  if (m_it != attributes_.end()) {
    std::copy(
        m_it,
        attributes_.end(),
        std::back_inserter(new_attributes));
  }
...
Bindings::iterator VectorBindings::find(const Symbol& name) {
  return BindingsIterator{std::find_if(
      attributes_.begin(),
      attributes_.end(),
