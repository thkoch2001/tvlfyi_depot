  ...
  if (pid != Pid(-1)) {
    ...
    if (buildUser) {
      /* If we're using a build user, then there is a tricky
         ...
         child. */
      ::kill(-int(pid), SIGKILL); /* ignore the result */
      buildUser->kill();
      pid.wait();
    } else {
      pid.kill();
...
void DerivationGoal::startBuilder() {
  ...
  if (useChroot) {
    ...
    */
    ...
    writeFile("/proc/" + std::to_string(int(pid)) + "/uid_map",
              (format("%d %d 1") % sandboxUid % hostUid).str());

    writeFile("/proc/" + std::to_string(int(pid)) + "/setgroups", "deny");

    writeFile("/proc/" + std::to_string(int(pid)) + "/gid_map",
