...
struct BasicDerivation {
  ...
  explicit BasicDerivation(const nix::proto::Derivation* proto_derivation)
      : outputs(),
        ...
        args(),
        env() {
    outputs.insert(proto_derivation->outputs().begin(),
                   proto_derivation->outputs().end());

    inputSrcs.insert(proto_derivation->input_sources().paths().begin(),
                     proto_derivation->input_sources().paths().end());

    args.insert(args.end(), proto_derivation->args().begin(),
                proto_derivation->args().end());

    env.insert(proto_derivation->env().begin(), proto_derivation->env().end());
  }
