...
struct BasicDerivation {
  ...
  explicit BasicDerivation(const nix::proto::Derivation* proto_derivation)
      : outputs(),
        inputSrcs(),
        platform(proto_derivation->platform()),
        builder(proto_derivation->builder().path()),
        args(),
        env() {
    for (const auto& [key, val] : proto_derivation->outputs()) {
      outputs.try_emplace(key, val);
    }

    for (const auto& path : proto_derivation->input_sources().paths()) {
      inputSrcs.insert(path);
    }

    for (const auto& arg : proto_derivation->args()) {
      args.push_back(arg);
    }

    for (const auto& [key, val] : proto_derivation->env()) {
      env.try_emplace(key, val);
    }
  }
