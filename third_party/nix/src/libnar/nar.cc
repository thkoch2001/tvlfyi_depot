...
class writer {
    ...
    auto operator<< (std::streambuf& src) -> writer& {
        std::streampos pos = src.pubseekoff(0, std::ios_base::cur, std::ios_base::in);
        std::streampos end = src.pubseekoff(0, std::ios_base::end, std::ios_base::in);
...
public:
    ...
    void symlink(std::string_view target) {
        *this << "(" << "type" << "symlink" << "target" << target << ")";
    };
    ...
    public:
        template<class F> void operator() (std::string_view name, F f) {
            writer_ << "entry" << "(" << "name" << name << "node";
            f(writer_);
            writer_ << ")";
        }
    };

    template<class F> void directory(F f) {
        *this << "(" << "type" << "directory";
