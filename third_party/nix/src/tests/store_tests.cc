...
class StoreTest : public ::testing::Test {
 public:
  ...
  std::filesystem::path GetTempDir() {
    constexpr std::string_view kPrefix = "nix-test-";
    constexpr int kRandomByteCnt = 8;
    std::error_code ec;
    std::array<unsigned char, kRandomByteCnt> raw_suffix;
    if (kRandomByteCnt != getrandom(raw_suffix.data(), raw_suffix.size(), 0)) {
      int errnum = errno;
      LOG(FATAL) << "Failed to get randomness for tmpdir: "
                 << (errno == 0 ? "short read" : strerror(errnum));
    }
    std::string suffix = absl::WebSafeBase64Escape(absl::string_view(
        reinterpret_cast<char*>(raw_suffix.data()), raw_suffix.size()));

    std::filesystem::path tmp_dir_parent =
        std::filesystem::temp_directory_path();
    std::filesystem::path dir = (tmp_dir_parent / kPrefix).concat(suffix);
    if (!std::filesystem::create_directories(dir, ec)) {
      LOG(FATAL) << "Failed to create tmpdir for test: " << ec;
    }
...
constexpr absl::string_view kDep1NarCache = "nar/0hfdc95cy6mxi4c15pp0frdf97r7yvd8c141qzvpms2f8x17p2ig.nar.xz";
