...
TEST_F(FileTests, ReadRegularFile) {
  ...
  auto contents = nix::util::readFile_Status(path.string());
  EXPECT_OK(contents);
  EXPECT_EQ(*contents, kFileContents);

  int fd = open(path.c_str(), O_RDONLY);
  contents = nix::util::readFile_Status(fd);
  close(fd);
  EXPECT_OK(contents);
  EXPECT_EQ(*contents, kFileContents);
}
...
TEST_F(FileTests, ReadDevNull) {
  auto contents = nix::util::readFile_Status("/dev/null");
  EXPECT_OK(contents);
  EXPECT_EQ(*contents, "");
...
TEST_F(FileTests, ReadDoesNotExist) {
  ...
  contents = nix::util::readFile_Status(-1);
