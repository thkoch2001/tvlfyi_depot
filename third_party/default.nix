...
}:
{
  ...
  nixos =
    ...
    let
      eval =
        import
          "${ pkgs.path }/nixos/lib/eval-config.nix"
          {
            inherit specialArgs system;
            modules = [ configuration ( import "${ depot.path + "/ops/modules/default-imports.nix" }" ) ];
          };
      ...
      vmConfig =
        (
            import
              ...
              {
                inherit specialArgs system;
                modules = [ configuration "${ pkgs.path }/nixos/modules/virtualisation/qemu-vm.nix" ];
              }
          )
          .config;
    in
