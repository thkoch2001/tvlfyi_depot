...
let
  ...
  */
  respond =
    ...
    bodyArg:
    let
      status = if builtins.isInt statusArg then {
        code = statusArg;
        line = lib.findFirst (line: statusCodes."${line}" == statusArg) null
      ...
      renderedHeaders = lib.concatStrings (lib.mapAttrsToList (n: v: ''
        ${n}: ${toString v}
      '') headers);
      internalError = msg:
        respond 500 { Content-type = "text/plain"; } "bubblegum error: ${msg}";
      body = builtins.tryEval bodyArg;
    in if status.code == null || status.line == null then
      internalError "Invalid status ${lib.generators.toPretty { } statusArg}."
    else if !body.success then
  ...
  */
  ...
  /* Returns the value of the `PATH_INFO` environment
     ...
     for convenience.
  */
  pathInfo = let p = builtins.getEnv "PATH_INFO";
  in if builtins.stringLength p == 0 then "/" else p;
  ...
  */
  ...
  writeCGI = { # if given sets the `PATH` to search for `nix-instantiate`
    ...
    input:
    let
      drvName = if builtins.isString input || args ? name then
        args.name
      else
        utils.storePathName input;
      script = if builtins.isPath input || lib.isDerivation input then
        input
      else if builtins.isString input then
        pkgs.writeText "${drvName}-source" input
      else
        builtins.throw
        "Unsupported input: ${lib.generators.toPretty { } input}";
...
in { inherit respond pathInfo scriptName absolutePath writeCGI; }
