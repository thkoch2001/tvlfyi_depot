...
let
  ...
  link = struct "link" { rel = string; href = string; };
    ...
    defun
      ...
      (
        ...
        <entry>
          ...
          ${ if e ? content then ''        <content type="html">${ escape e.content }</content>'' else "" }
  ...
  mostRecentlyUpdated = defun [ ( list entry ) int ] ( entries: foldl' max 0 ( map ( e: e.updated ) entries ) );
