...
(define-constant +issue-statuses+ (list :open :closed)
  :test #'equal)
...
(defun list-issues (&key status (with '(:num-comments)))
  "Return a list of all issues with the given STATUS (or all if nil), most
recent first. If WITH contains `:NUM-COMMENTS' (the default) each issue will
