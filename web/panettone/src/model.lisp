...
(define-constant +migrations-dir+
    ...
    (or (when (find-package :build)
          (let ((sym (find-symbol "+migrations-dir+" :build)))
            (when (boundp sym)
              (symbol-value sym))))
        "migrations/")
  :test #'string=
  :documentation "The directory where migrations are stored")
...
in the context of a database transaction and should perform the migration."
  (let* ((version (get-universal-time))
         (filename (format nil "~A-~A.lisp"
                           version
                           name))
         (pathname (make-pathname :directory +migrations-dir+
                                  :name filename)))
