...
in Nixlang. It uses a subset of ATerm internally, produces "fingerprints"
containing hashes of these ATerms, which are then hashed again.
...
filesystem-based backends, as well as sled, an embedded K/V database.
...
## Catchables / tryEval
TODO: describe things we did here, a bit on our catchable system, still WIP on
macro and test suite side
...
`callPackage` implementation, but impossible to fix in Nix without breaking
all hashes.
