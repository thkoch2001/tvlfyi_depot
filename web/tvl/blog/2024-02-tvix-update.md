...
Even though we're not yet getting into very fine-grained optimization until
we're sure Tvix evaluates nixpkgs in a compatible fashion, we still want to have
an idea about evaluation performance, and how it changes over time.
To do this, we extended our benchmarks and wired them into
[Windtunnel][windtunnel], which regularly now runs benchmarks and provides
graphs. In the future, we plan to extend this to allow per-CL runs, so it
becomes easier to get see changes in performance implications right in the web
interface, without having to do a manual benchmark locally before and after
the change.
