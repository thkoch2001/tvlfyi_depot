...
Very early, during bootstraping, nixpkgs relies on some `builtin:fetchurl`
...
performance improvements. Expect a detailed blogpost around that soon after
this one!
...
We're still considering these outputs early prototypes, and will refine them as
we go.
...
This have been consolidated into a mdbook (at `//tvix/docs`).

We plan to properly host these as a website, hopefully providing a better intro
...
running some more tests in our test suite.
...
about the Tvix Store Model in the last update, but it didn't go into too much
...
specifying such a composition config, as an alternative to the CLI args for the
three (single) stores.

We're still not 100% sure how (granular we want) to expose this in a
user-friendly fashion, and are not 100% settled on the format yet.

There's also some more combinators and refactors missing), but please let us
know your thoughts!

## Contributors
It's been a lot of progress, which would not have been possible without our
