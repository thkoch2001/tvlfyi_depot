...
in depot.nix.utils.drvTargets(lib.fix (self: {
  ...
  # Create a PNG of the TVL logo with the specified static armchair colour.
  logoPng = colour: pkgs.runCommandNoCC "logo.png" {} ''
    ${pkgs.inkscape}/bin/inkscape --export-area-drawing --export-background-opacity=0 ${self.staticLogoSvg colour} -o $out
  '';
...
// (lib.mapAttrs' (k: v: lib.nameValuePair "${k}Png" (self.logoPng v)) palette)))
