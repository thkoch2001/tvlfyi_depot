...
let
  ...
  renderPost =
    ...
    runCommandNoCC
      ...
      ${ if post ? updated then ''      date --date="@${ toString post.updated }" '+ (updated %Y-%m-%d)' >> $out'' else "" }
      echo '</aside>' >> $out
      ...
      ${
        ...
        then
          ...
          if ( isUnlisted post ) then "cat ${ unlistedWarning } >> $out" else "# Your ads could be here?"
      }
