...
let
  inherit ( pkgs ) jq ripgrep runCommandNoCC writeTextFile;
  inherit ( builtins ) elem filter fromJSON head readFile map;
  inherit ( lib ) concatStringsSep;
  inherit ( depot.nix.yants ) defun int string struct;
  knownUsers = map ( u: u.username ) depot.ops.users;
  todo = struct { file = string; line = int; todo = string; user = string; };
  allTodos =
    fromJSON
      (
        readFile
